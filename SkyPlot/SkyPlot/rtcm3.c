...
static double satwavelen(unsigned char sat, int frq, const nav_t* nav)
{
	...
	if (sys == SYS_GLO) {
		...
		else if (frq == 3) return CLIGHT / FREQB1C_CMP; /* B1C */
		else if (frq == 4) return CLIGHT / FREQB2a_CMP; /* B2a */
		else if (frq == 5) return CLIGHT / FREQB2b_CMP; /* B2b */
	}
	else if (sys == SYS_GAL) {
	...
	for (i = j = 0; i < h->nsat; i++) {
		...
		for (k = 0; k < h->nsig; k++) {
			...
			if (sat && index >= 0 && ind[k] >= 0) {
				...
				rtcm->obs.data[index].LLI[ind[k]] = 0;
...
static int decode_msm5(rtcm_t* rtcm, int sys)
{
	...
	if (i + h.nsat * 36 + ncell * 63 > rtcm->len * 8) {
		trace(4, "rtcm3 %d length error: nsat=%d ncell=%d len=%d\n", type, h.nsat,
			ncell, rtcm->len);
		return -1;
	}
	for (j = 0; j < h.nsat; j++) {
...
extern int decode_rtcm3(rtcm_t* rtcm)
{
	...
	if(type==1074 || type == 1075 || type==1019)
	printf("decode_rtcm3:rcv=%d len=%3d type=%d\n", rtcm->rcv, rtcm->len, type);
	switch (type) {
