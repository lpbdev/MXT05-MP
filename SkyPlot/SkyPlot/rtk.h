...
#define SVN_VERSION 31242
#define TIME_OUTPUT
#define MAXEPH		10240
#define MAXGEPH     5120
#define DEBUG_BUFF_LEN	2048
...
typedef struct {
	...
	int npb;         /* bytes in input peek buffers */
	int buffPtr[2];//buffer pointer
	int navsel;         /* ephemeris select (0:all,1:rover,2:base,3:corr) */
	int prcout;         /* missing observation data count */
	unsigned int tick;  /* start tick */
	thread_t thread;    /* server thread */
	unsigned char* pbuf; /* peek buffers {rov,base,corr} */
	unsigned char* buff; /* input buffers {rov,base,corr} */
	rtcm_t rtcm;
	gtime_t ftime;//download time
	obs_t obs[128];//observation data (rover,base)
	stream_t stream[2];
	int format[2];      /* input format {rov,base,corr} */
	lock_t lock;        /* lock flag */
}rtksvr_t;
...
extern void geph2pos(gtime_t time, const geph_t* geph, double* rs, double* dts,
	double* var);
extern int lambda(rtk_t* rtk, int n, int m, const double* a, const double* Q, double* f, double* s);
...
extern int checkFixP(rtk_t* rtk);
extern int checkFloatP(rtk_t* rtk);
