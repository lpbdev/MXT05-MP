...
#define DEFAULT_MEMBUF_SIZE 4096        
...
static int toinact = 0; /* inactive timeout (ms) */
static int ticonnect = 1000; /* interval to re-connect (ms) */
...
static int readseribuff(serial_t* serial, uint8_t* buff, int nmax)
{
    ...
    if (fptcp)   fprintf(fptcp, "readseribuff: dev=%d\n", serial->dev);
    ...
    unlock(&serial->lock);
    if (fptcp)   fprintf(fptcp, "readseribuff: ns=%d rp=%d wp=%d\n", ns, serial->rp, serial->wp);
    return ns;
}
static int writeseribuff(serial_t* serial, uint8_t* buff, int n)
{
    ...
    if (fptcp)   fprintf(fptcp, "writeseribuff: dev=%d n=%d\n", serial->dev, n);
    ...
    for (ns = 0; ns < n; ns++) {
        ...
        if (wp != serial->rp) serial->wp = wp;
            ...
            if (fptcp)   fprintf(fptcp, "serial buffer overflow: size=%d\n", serial->buffsize);
            break;
        }
    }
    unlock(&serial->lock);
    if (fptcp)   fprintf(fptcp, "writeseribuff: ns=%d rp=%d wp=%d\n", ns, serial->rp, serial->wp);
    return ns;
}
#endif /* WIN32 */
...
static DWORD WINAPI serialthread(void* arg)
{
    ...
    if (fptcp)   fprintf(fptcp, "serialthread:\n");
...
static serial_t* openserial(const char* path, int mode, char* msg)
{
    ...
    int rw = 0;
#endif
    if (fptcp)   fprintf(fptcp, "openserial: path=%s mode=%d\n", path, mode);
    ...
    if (fptcp)   fprintf(fptcp, "openserial: dev=%d\n", serial->dev);
    return serial;
}
/* close serial --------------------------------------------------------------*/
static void closeserial(serial_t* serial)
{
    if (fptcp)   fprintf(fptcp, "closeserial: dev=%d\n", serial->dev);
...
static int readserial(serial_t* serial, uint8_t* buff, int n, char* msg)
{
    ...
    int nr;
#endif
    if (fptcp)   fprintf(fptcp, "readserial: dev=%d n=%d\n", serial->dev, n);
    if (!serial) return 0;
#ifdef WIN32
...
#endif
    if (fptcp)   fprintf(fptcp, "readserial: exit dev=%d nr=%d\n", serial->dev, nr);
...
static int writeserial(serial_t* serial, uint8_t* buff, int n, char* msg)
{
    ...
    if (fptcp)   fprintf(fptcp, "writeserial: dev=%d n=%d\n", serial->dev, n);
...
#endif
    if (fptcp)   fprintf(fptcp, "writeserial: exit dev=%d ns=%d\n", serial->dev, ns);
    return ns;
}
/* get state serial ----------------------------------------------------------*/
...
static int openfile_(file_t* file, gtime_t time, char* msg)
{
    ...
    if (fptcp)   fprintf(fptcp, "openfile_: path=%s time=%s\n", file->path, time_str(time, 0));
    ...
    if (fptcp)   fprintf(fptcp, "openfile_: open file %s (%s)\n", file->openpath, rw);
    ...
    if (file->timetag) { /* output/sync time-tag */
        ...
        if (fptcp)   fprintf(fptcp, "openfile_: open tag file %s (%s)\n", tagpath, rw);
...
{
    if (fptcp)   fprintf(fptcp, "closefile_: path=%s\n", file->path);
...
static file_t* openfile(const char* path, int mode, char* msg)
{
    ...
    if (fptcp)   fprintf(fptcp, "openfile: path=%s mode=%d\n", path, mode);
...
{
    if (fptcp)   fprintf(fptcp, "closefile: fp=%d\n", file->fp);
...
static void swapfile(file_t* file, gtime_t time, char* msg)
{
    ...
    if (fptcp)   fprintf(fptcp, "swapfile: fp=%d time=%s\n", file->fp, time_str(time, 0));
    ...
    if (!strcmp(openpath, file->openpath)) {
        if (fptcp)   fprintf(fptcp, "swapfile: no need to swap %s\n", openpath);
        return;
    }
...
{
    if (fptcp)   fprintf(fptcp, "swapclose: fp_tmp=%d\n", file->fp_tmp);
...
static int readfile(file_t* file, uint8_t* buff, int nmax, char* msg)
{
    ...
    if (fptcp)   fprintf(fptcp, "readfile: fp=%d nmax=%d\n", file->fp, nmax);
    ...
    if (fptcp)   fprintf(fptcp, "readfile: fp=%d nr=%d\n", file->fp, nr);
    return nr;
}
/* write file ----------------------------------------------------------------*/
static int writefile(file_t* file, uint8_t* buff, int n, char* msg)
{
    ...
    if (fptcp)   fprintf(fptcp, "writefile: fp=%d n=%d\n", file->fp, n);
    ...
    if (fptcp)   fprintf(fptcp, "writefile: fp=%d ns=%d tick=%5d fpos=%d\n", file->fp, ns, tick, fpos);
...
static int setsock(socket_t sock, char* msg)
{
    ...
    if (setsockopt(sock, SOL_SOCKET, SO_RCVBUF, (const char*)&bs, sizeof(bs)) == -1 ||
        ...
        if (fptcp)   fprintf(fptcp, "setsock: setsockopt error 2 sock=%d err=%d bs=%d\n", sock, errsock(), bs);
        sprintf(msg, "sockopt error: bufsiz");
    }
    if (setsockopt(sock, IPPROTO_TCP, TCP_NODELAY, (const char*)&mode, sizeof(mode)) == -1) {
        if (fptcp)   fprintf(fptcp, "setsock: setsockopt error 3 sock=%d err=%d\n", sock, errsock());
...
static int recv_nb(socket_t sock, uint8_t* buff, int n)
{
    ...
    FD_ZERO(&rs); FD_SET(sock, &rs);
    ret = select(sock + 1, &rs, NULL, NULL, NULL);
    if (ret <= 0) return ret;
...
static int gentcp(tcp_t* tcp, int type, char* msg)
{
    ...
    if (type == 0) { /* server socket */
        ...
        if (bind(tcp->sock, (struct sockaddr*)&tcp->addr, sizeof(tcp->addr)) == -1) {
            ...
            if (fptcp)   fprintf(fptcp, "gentcp: bind error port=%d err=%d\n", tcp->port, errsock());
        ...
        if (!(hp = gethostbyname(tcp->saddr))) {
            ...
            if (fptcp)   fprintf(fptcp, "gentcp: gethostbyname error addr=%s err=%d\n", tcp->saddr, errsock());
            closesocket(tcp->sock);
            tcp->state = 0;
            tcp->tcon = ticonnect;
            tcp->tdis = tickget();
    ...
    tcp->state = 1;
    tcp->tact = tickget();
    if (fptcp)   fprintf(fptcp, "gentcp: exit sock=%d\n", tcp->sock);
    return 1;
}
/* disconnect tcp ------------------------------------------------------------*/
static void discontcp(tcp_t* tcp, int tcon)
{
    if (fptcp)   fprintf(fptcp, "discontcp: sock=%d tcon=%d\n", tcp->sock, tcon);
...
static tcpsvr_t* opentcpsvr(const char* path, char* msg)
{
    ...
    if (fptcp)   fprintf(fptcp, "opentcpsvr: path=%s\n", path);
    ...
    *tcpsvr = tcpsvr0;
    ...
    if (sscanf(port, "%d", &tcpsvr->svr.port) < 1) {
        ...
        if (fptcp)   fprintf(fptcp, "opentcpsvr: port error port=%s\n", port);
        free(tcpsvr);
        return NULL;
    }
...
static void closetcpsvr(tcpsvr_t* tcpsvr)
{
    ...
    if (fptcp)   fprintf(fptcp, "closetcpsvr:\n");
...
static void updatetcpsvr(tcpsvr_t* tcpsvr, char* msg)
{
    ...
    if (fptcp)   fprintf(fptcp, "updatetcpsvr: state=%d\n", tcpsvr->svr.state);
...
static int accsock(tcpsvr_t* tcpsvr, char* msg)
{
    ...
    if (fptcp)   fprintf(fptcp, "accsock: sock=%d\n", tcpsvr->svr.sock);

    for (i = 0; i < MAXCLI; i++) {
        if (tcpsvr->cli[i].state == 0) break;
    }
    if (i >= MAXCLI) {
        if (fptcp)   fprintf(fptcp, "accsock: too many clients sock=%d\n", tcpsvr->svr.sock);
        return 0;
    }
    if ((sock = accept_nb(tcpsvr->svr.sock, (struct sockaddr*)&addr, &len)) == (socket_t)-1) {
        err = errsock();
        sprintf(msg, "accept error (%d)", err);
        //trace(1, "accsock: accept error sock=%d err=%d\n", tcpsvr->svr.sock, err);
        closesocket(tcpsvr->svr.sock);
        tcpsvr->svr.state = 0;
        return 0;
    }
    ...
    if (fptcp)   fprintf(fptcp, "accsock: connected sock=%d addr=%s i=%d\n", tcpsvr->cli[i].sock, tcpsvr->cli[i].saddr, i);
...
{
    if (fptcp)   fprintf(fptcp, "waittcpsvr: sock=%d state=%d\n", tcpsvr->svr.sock, tcpsvr->svr.state);
...
static int readtcpsvr(tcpsvr_t* tcpsvr, uint8_t* buff, int n, char* msg)
{
    ...
    if (fptcp)   fprintf(fptcp, "readtcpsvr: state=%d\n", tcpsvr->svr.state);
    ...
    for (i = 0; i < MAXCLI; i++) {
        ...
        if ((nr = recv_nb(tcpsvr->cli[i].sock, buff, n)) == -1) {
            if ((err = errsock())) {
                if (fptcp)   fprintf(fptcp, "readtcpsvr: recv error sock=%d err=%d\n", tcpsvr->cli[i].sock, err);
            }
...
static int writetcpsvr(tcpsvr_t* tcpsvr, uint8_t* buff, int n, char* msg)
{
    ...
    if (fptcp)   fprintf(fptcp, "writetcpsvr: state=%d n=%d\n", tcpsvr->svr.state, n);
    ...
    for (i = 0; i < MAXCLI; i++) {
        ...
        if ((ns = send_nb(tcpsvr->cli[i].sock, buff, n)) == -1) {
            if ((err = errsock())) {
                if (fptcp)   fprintf(fptcp, "writetcpsvr: send error i=%d sock=%d err=%d\n", i, tcpsvr->cli[i].sock, err);
            }
            discontcp(&tcpsvr->cli[i], ticonnect);
            updatetcpsvr(tcpsvr, msg);
        }
...
static int consock(tcpcli_t* tcpcli, char* msg)
{
    ...
    if ((stat = connect_nb(tcpcli->svr.sock, (struct sockaddr*)&tcpcli->svr.addr,
        ...
        if (fptcp)   fprintf(fptcp, "%s,consock: connect error sock=%d err=%d\n", s, tcpcli->svr.sock, err);
    ...
    if (fptcp)   fprintf(fptcp, "%s,consock: connected sock=%d addr=%s\n", s, tcpcli->svr.sock, tcpcli->svr.saddr);
    tcpcli->svr.state = 2;
...
static tcpcli_t* opentcpcli(const char* path, char* msg)
{
    ...
    if (fptcp)   fprintf(fptcp, "opentcpcli: path=%s\n", path);
    ...
    *tcpcli = tcpcli0;
    ...
    if (sscanf(port, "%d", &tcpcli->svr.port) < 1) {
        sprintf(msg, "port error: %s", port);
        if (fptcp)   fprintf(fptcp, "opentcp: port error port=%s\n", port);
        free(tcpcli);
        return NULL;
    }
    tcpcli->svr.tcon = 0;
...
{
    if (fptcp)   fprintf(fptcp, "closetcpcli: sock=%d\n", tcpcli->svr.sock);
...
{
    //if (fptcp)   fprintf(fptcp,"waittcpcli: sock=%d state=%d\n", tcpcli->svr.sock, tcpcli->svr.state);
    gtime_t time;
    char s[64] = { 0 };
    ...
    if (tcpcli->svr.state == 2) { /* connect */
        if (tcpcli->toinact > 0 &&
            ...
            time = timeget();
            time2str(time, s, 0);
            if (fptcp)   fprintf(fptcp, "%s:waittcpcli: inactive timeout sock=%d\n", s, tcpcli->svr.sock);
...
static int readtcpcli(tcpcli_t* tcpcli, uint8_t* buff, int n, char* msg)
{
    ...
    //if (fptcp)   fprintf(fptcp,"readtcpcli: sock=%d\n", tcpcli->svr.sock);

    if (!waittcpcli(tcpcli, msg)) return 0;

    if ((nr = recv_nb(tcpcli->svr.sock, buff, n)) == -1) {
        if ((err = errsock())) {
            if (fptcp)   fprintf(fptcp, "readtcpcli: recv error sock=%d err=%d\n", tcpcli->svr.sock, err);
    ...
    //if (fptcp)   fprintf(fptcp,"readtcpcli: exit sock=%d nr=%d\n", tcpcli->svr.sock, nr);
    return nr;
}
/* write tcp client ----------------------------------------------------------*/
static int writetcpcli(tcpcli_t* tcpcli, uint8_t* buff, int n, char* msg)
{
    ...
    // if (fptcp)   fprintf(fptcp,"writetcpcli: sock=%d state=%d n=%d\n", tcpcli->svr.sock, tcpcli->svr.state, n);

    if (!waittcpcli(tcpcli, msg)) return 0;

    if ((ns = send_nb(tcpcli->svr.sock, buff, n)) == -1) {
        if ((err = errsock())) {
            if (fptcp)   fprintf(fptcp, "writetcp: send error sock=%d err=%d\n", tcpcli->svr.sock, err);
    ...
    if (fptcp)   fprintf(fptcp, "writetcpcli: exit sock=%d ns=%d\n", tcpcli->svr.sock, ns);
    return ns;
}
...
static int encbase64(char* str, const uint8_t* byte, int n)
{
    ...
    // if (fptcp)   fprintf(fptcp,"encbase64: n=%d\n", n);
    ...
    while (j & 0x3) str[j++] = '=';
    str[j] = '\0';
    // if (fptcp)   fprintf(fptcp,"encbase64: str=%s\n", str);
    return j;
}
...
static int reqntrip_s(ntrip_t* ntrip, char* msg)
{
    ...
    int returnValue, len;
    if (fptcp)   fprintf(fptcp, "reqntrip_s: state=%d\n", ntrip->state);
    ...
    returnValue = writetcpcli(ntrip->tcp, (uint8_t*)buff, p - buff, msg);
    len = p - buff;
    if (fptcp)   fprintf(fptcp, "reqntrip_s writetcpcli return %d len=%d state=%d\n", returnValue, len, ntrip->state);
    if (returnValue != len) {
        return 0;
    }
    // if (writetcpcli(ntrip->tcp, (uint8_t*)buff, p - buff, msg) != p - buff) return 0;

    if (fptcp)   fprintf(fptcp, "reqntrip_s: send request state=%d ns=%d\n", ntrip->state, p - buff);
    if (fptcp)   fprintf(fptcp, "reqntrip_s: n=%d buff=\n%s\n", p - buff, buff);
    ntrip->state = 1;
    return 1;
}
...
static int reqntrip_c(ntrip_t* ntrip, char* msg)
{
    ...
    int returnValue, len;
    if (fptcp)   fprintf(fptcp, "reqntrip_c: state=%d\n", ntrip->state);
    ...
    returnValue = writetcpcli(ntrip->tcp, (uint8_t*)buff, p - buff, msg);
    len = p - buff;
    if (fptcp)   fprintf(fptcp, "reqntrip_c writetcpcli return %d len=%d state=%d\n", returnValue, len, ntrip->state);
    if (returnValue != len) {
        return 0;
    }
    // if (writetcpcli(ntrip->tcp, (uint8_t*)buff, p - buff, msg) != p - buff) return 0;

    if (fptcp)   fprintf(fptcp, "reqntrip_c: send request state=%d ns=%d\n", ntrip->state, p - buff);
    if (fptcp)   fprintf(fptcp, "reqntrip_c: n=%d buff=\n%s\n", p - buff, buff);
    ntrip->state = 1;
    return 1;
}
...
static int rspntrip_s(ntrip_t* ntrip, char* msg)
{
    ...
    if (fptcp)   fprintf(fptcp, "rspntrip_s: state=%d nb=%d\n", ntrip->state, ntrip->nb);
    ntrip->buff[ntrip->nb] = '0';
    if (fptcp)   fprintf(fptcp, "rspntrip_s: n=%d buff=\n%s\n", ntrip->nb, ntrip->buff);
    ...
    if ((p = strstr((char*)ntrip->buff, NTRIP_RSP_OK_SVR))) { /* ok */
        ...
        if (fptcp)   fprintf(fptcp, "rspntrip_s: response ok nb=%d\n", ntrip->nb);
        return 1;
    }
        ...
        if (fptcp)   fprintf(fptcp, "rspntrip_s: %s nb=%d\n", msg, ntrip->nb);
        ntrip->nb = 0;
        ntrip->buff[0] = '\0';
        ntrip->state = 0;
        ...
        if (fptcp)   fprintf(fptcp, "rspntrip_s: response overflow nb=%d\n", ntrip->nb);
        ntrip->nb = 0;
        ntrip->buff[0] = '\0';
        ntrip->state = 0;
        discontcp(&ntrip->tcp->svr, ntrip->tcp->tirecon);
    }
    if (fptcp)   fprintf(fptcp, "rspntrip_s: exit state=%d nb=%d\n", ntrip->state, ntrip->nb);
    return 0;
}
...
static int rspntrip_c(ntrip_t* ntrip, char* msg)
{
    ...
    if (fptcp)   fprintf(fptcp, "rspntrip_c: state=%d nb=%d\n", ntrip->state, ntrip->nb);
    ntrip->buff[ntrip->nb] = '0';
    if (fptcp)   fprintf(fptcp, "rspntrip_c: n=%d buff=\n%s\n", ntrip->nb, ntrip->buff);
    ...
    if ((p = strstr((char*)ntrip->buff, NTRIP_RSP_OK_CLI))) { /* ok */
        ...
        ntrip->state = 2;
        sprintf(msg, "%s/%s", ntrip->tcp->svr.saddr, ntrip->mntpnt);
        if (fptcp)   fprintf(fptcp, "rspntrip_c: response ok nb=%d\n", ntrip->nb);
        return 1;
    }
    if ((p = strstr((char*)ntrip->buff, NTRIP_RSP_SRCTBL))) { /* source table */
        if (!*ntrip->mntpnt) { /* source table request */
            ...
            if (fptcp)   fprintf(fptcp, "rspntrip_c: receive source table nb=%d\n", ntrip->nb);
            return 1;
        }
        sprintf(msg, "no mountp. reconnect...");
        if (fptcp)   fprintf(fptcp, "rspntrip_c: no mount point nb=%d\n", ntrip->nb);
        ntrip->nb = 0;
        ntrip->buff[0] = '\0';
        ntrip->state = 0;
        discontcp(&ntrip->tcp->svr, ntrip->tcp->tirecon);
    ...
    if (fptcp)   fprintf(fptcp, "rspntrip_c: exit state=%d nb=%d\n", ntrip->state, ntrip->nb);
    return 0;
}
...
static int waitntrip(ntrip_t* ntrip, char* msg)
{
    ...
    //if (fptcp)   fprintf(fptcp,"waitntrip: state=%d nb=%d\n", ntrip->state, ntrip->nb);
    ...
    if (ntrip->state == 1) { /* read response */
        ...
        if ((n = readtcpcli(ntrip->tcp, (uint8_t*)p, NTRIP_MAXRSP - ntrip->nb - 1, msg)) == 0) {
            if (fptcp)   fprintf(fptcp, "waitntrip: readtcp n=%d\n", n);
            return 0;
        }
...
static ntrip_t* openntrip(const char* path, int type, char* msg)
{
    ...
    if (fptcp)   fprintf(fptcp, "openntrip: path=%s type=%d\n", path, type);
    ...
    if (!(ntrip->tcp = opentcpcli(tpath, msg))) {
        if (fptcp)   fprintf(fptcp, "openntrip: opentcp error\n");
        free(ntrip);
        return NULL;
    }
    return ntrip;
...
{
    if (fptcp)   fprintf(fptcp, "closentrip: state=%d\n", ntrip->state);
...
static int readntrip(ntrip_t* ntrip, uint8_t* buff, int n, char* msg)
{
    ...
    gtime_t time;
    char s1[64];
    //if (fptcp)   fprintf(fptcp,"readntrip:\n");
    ...
    time.time += 3600 * 8;
    time2str(time, s1, 3);
    readntripLen = readtcpcli(ntrip->tcp, buff, n, msg);
    if (readntripLen != 0) {
        if (fptcp)   fprintf(fptcp, "%s readtcpcli index=%d len=%d\n", s1, 0, readntripLen);
        trace(4, "%s readtcpcli index=%d len=%d\n", s1, 0, readntripLen);
    }
    return readntripLen;
}
/* write ntrip ---------------------------------------------------------------*/
static int writentrip(ntrip_t* ntrip, uint8_t* buff, int n, char* msg)
{
    ...
    gtime_t time;
    char s1[64];
    //if (fptcp)   fprintf(fptcp,"writentrip: n=%d\n", n);
    ...
    len = writetcpcli(ntrip->tcp, buff, n, msg);
    if (len != 0) {
        if (fptcp)   fprintf(fptcp, "%s writetcpcli len=%d\n", s1, len);
        trace(4, "%s writetcpcli len=%d\n", s1, len);
    }
    return len;
}
/* get state ntrip -----------------------------------------------------------*/
...
static ntripc_t* openntripc(const char* path, char* msg)
{
    ...
    if (fptcp)   fprintf(fptcp, "openntripc: path=%s\n", path);
    ...
    if (!*ntripc->mntpnt) {
        if (fptcp)   fprintf(fptcp, "openntripc: no mountpoint path=%s\n", path);
        free(ntripc);
        return NULL;
    }
    /* use default port if no port specified */
    ...
    if (!(ntripc->tcp = opentcpsvr(tpath, msg))) {
        if (fptcp)   fprintf(fptcp, "openntripc: opentcpsvr error port=%d\n", port);
        free(ntripc);
        return NULL;
    }
    return ntripc;
...
{
    if (fptcp)   fprintf(fptcp, "closentripc: state=%d\n", ntripc->state);
...
{
    if (fptcp)   fprintf(fptcp, "discon_ntripc: i=%d\n", i);
...
static void rsp_ntripc(ntripc_t* ntripc, int i)
{
    ...
    if (fptcp)   fprintf(fptcp, "rspntripc_c i=%d\n", i);
    con->buff[con->nb] = '\0';
    if (fptcp)   fprintf(fptcp, "rspntripc_c: n=%d,buff=\n%s\n", con->nb, con->buff);

    if (con->nb >= NTRIP_MAXRSP - 1) { /* buffer overflow */
        if (fptcp)   fprintf(fptcp, "rsp_ntripc_c: request buffer overflow\n");
    ...
    if (!(p = strstr((char*)con->buff, "GET")) || !(q = strstr(p, "\r\n")) ||
        ...
        if (fptcp)   fprintf(fptcp, "rsp_ntripc_c: NTRIP request error\n");
        discon_ntripc(ntripc, i);
        return;
    }
    /* test protocol */
    if (sscanf(p, "GET %255s %255s", url, proto) < 2 ||
        ...
        if (fptcp)   fprintf(fptcp, "rsp_ntripc_c: NTRIP request error proto=%s\n", proto);
        discon_ntripc(ntripc, i);
        return;
    }
    ...
    if (!*mntpnt || strcmp(mntpnt, ntripc->mntpnt)) {
        if (fptcp)   fprintf(fptcp, "rsp_ntripc_c: no mountpoint %s\n", mntpnt);
    ...
    if (*ntripc->passwd) {
        ...
        if (!(p = strstr((char*)con->buff, "Authorization:")) ||
            ...
            if (fptcp)   fprintf(fptcp, "rsp_ntripc_c: authroziation error\n");
...
static void wait_ntripc(ntripc_t* ntripc, char* msg)
{
    ...
    if (fptcp)   fprintf(fptcp, "wait_ntripc\n");
    ...
    for (i = 0; i < MAXCLI; i++) {
        ...
        if ((n = recv_nb(ntripc->tcp->cli[i].sock, buff, nmax)) == -1) {
            if ((err = errsock())) {
                if (fptcp)   fprintf(fptcp, "wait_ntripc: recv error sock=%d err=%d\n", ntripc->tcp->cli[i].sock, err);
            }
            discon_ntripc(ntripc, i);
            continue;
        }
...
static int readntripc(ntripc_t* ntripc, uint8_t* buff, int n, char* msg)
{
    ...
    if (fptcp)   fprintf(fptcp, "readntripc:\n");
    ...
    for (i = 0; i < MAXCLI; i++) {
        ...
        if (nr < 0) {
            if ((err = errsock())) {
                if (fptcp)   fprintf(fptcp, "readntripc: recv error i=%d sock=%d err=%d\n", i, ntripc->tcp->cli[i].sock, err);
            }
            discon_ntripc(ntripc, i);
        }
        else if (nr > 0) {
...
static int writentripc(ntripc_t* ntripc, uint8_t* buff, int n, char* msg)
{
    ...
    if (fptcp)   fprintf(fptcp, "writentripc: n=%d\n", n);
    ...
    for (i = 0; i < MAXCLI; i++) {
        ...
        if (ns < n) {
            if ((err = errsock())) {
                if (fptcp)   fprintf(fptcp, "writentripc: send error i=%d sock=%d err=%d\n", i, ntripc->tcp->cli[i].sock, err);
            }
            discon_ntripc(ntripc, i);
        }
        else {
...
static udp_t* genudp(int type, int port, const char* saddr, char* msg)
{
    ...
    if (fptcp)   fprintf(fptcp, "genudp: type=%d\n", type);
    ...
    if (setsockopt(udp->sock, SOL_SOCKET, SO_RCVBUF, (const char*)&bs, sizeof(bs)) == -1 ||
        ...
        if (fptcp)   fprintf(fptcp, "genudp: setsockopt error sock=%d err=%d bs=%d\n", udp->sock, errsock(), bs);
        sprintf(msg, "sockopt error: bufsiz");
    }
    ...
    if (!udp->type) { /* udp server */
        ...
        if (bind(udp->sock, (struct sockaddr*)&udp->addr, sizeof(udp->addr)) == -1) {
            if (fptcp)   fprintf(fptcp, "genudp: bind error sock=%d port=%d err=%d\n", udp->sock, port, errsock());
        ...
        if (!strcmp(saddr, "255.255.255.255") &&
            setsockopt(udp->sock, SOL_SOCKET, SO_BROADCAST, (const char*)&opt,
                sizeof(opt)) == -1) {
            if (fptcp)   fprintf(fptcp, "genudp: setsockopt error sock=%d err=%d\n", udp->sock, errsock());
...
{
    if (fptcp)   fprintf(fptcp, "closeudpsvr: sock=%d\n", udpsvr->sock);
...
static int readudpsvr(udp_t* udpsvr, uint8_t* buff, int n, char* msg)
{
    ...
    if (fptcp)   fprintf(fptcp, "readudpsvr: sock=%d n=%d\n", udpsvr->sock, n);
...
static udp_t* openudpcli(const char* path, char* msg)
{
    ...
    if (sscanf(sport, "%d", &port) < 1) {
        sprintf(msg, "port error: %s", sport);
        if (fptcp)   fprintf(fptcp, "openudpcli: port error port=%s\n", sport);
        return NULL;
    }
...
{
    if (fptcp)   fprintf(fptcp, "closeudpcli: sock=%d\n", udpcli->sock);
...
{
    if (fptcp)   fprintf(fptcp, "writeudpcli: sock=%d n=%d\n", udpcli->sock, n);
    ...
    char* passwd, int* topts)
{
    ...
    if (fptcp)   fprintf(fptcp, "decodeftpath: path=%s\n", path);
...
static gtime_t nextdltime(const int* topts, int stat)
{
    ...
    if (fptcp)   fprintf(fptcp, "nextdltime: topts=%d %d %d %d stat=%d\n", topts[0], topts[1], topts[2], topts[3], stat);
...
static int rtk_uncompress(const char* file, char* uncfile)
{
    int stat = 0;
...
#endif
{
    ...
    if (fptcp)   fprintf(fptcp, "ftpthread:\n");

    if (!*localdir) {
        if (fptcp)   fprintf(fptcp, "no local directory\n");
        ftp->error = 11;
        ftp->state = 3;
        return 0;
    }
    ...
    if ((fp = fopen(tmpfile, "rb"))) {
        ...
        if (fptcp)   fprintf(fptcp, "ftpthread: file exists %s\n", ftp->local);
        ftp->state = 2;
        return 0;
    }
    ...
    if ((ret = execcmd(cmd))) {
        remove(local);
        if (fptcp)   fprintf(fptcp, "execcmd error: cmd=%s ret=%d\n", cmd, ret);
        ftp->error = ret;
        ftp->state = 3;
        return 0;
    }
    ...
    if ((p = strrchr(local, '.')) &&
        ...
        if (rtk_uncompress(local, tmpfile)) {
            ...
            if (fptcp)   fprintf(fptcp, "file uncompact error: %s\n", local);
            ftp->error = 12;
            ftp->state = 3;
            return 0;
        }
    ...
    if (fptcp)   fprintf(fptcp, "ftpthread: complete cmd=%s\n", cmd);
    return 0;
}
/* open ftp ------------------------------------------------------------------*/
static ftp_t* openftp(const char* path, int type, char* msg)
{
    ...
    if (fptcp)   fprintf(fptcp, "openftp: path=%s type=%d\n", path, type);
    msg[0] = '\0';
...
{
    if (fptcp)   fprintf(fptcp, "closeftp: state=%d\n", ftp->state);
...
static int readftp(ftp_t* ftp, uint8_t* buff, int n, char* msg)
{
    ...
    if (fptcp)   fprintf(fptcp, "readftp: n=%d\n", n);
...
#endif
            if (fptcp)   fprintf(fptcp, "readftp: ftp thread create error\n");
            ftp->state = 3;
...
static membuf_t* openmembuf(const char* path, char* msg)
{
    ...
    if (fptcp)   fprintf(fptcp, "openmembuf: path=%s\n", path);
...
{
    if (fptcp)   fprintf(fptcp, "closemembufp\n");
...
static int readmembuf(membuf_t * membuf, uint8_t * buff, int n, char* msg)
{
    ...
    if (fptcp)   fprintf(fptcp, "readmembuf: n=%d\n", n);
...
static int writemembuf(membuf_t * membuf, uint8_t * buff, int n, char* msg)
{
    ...
    if (fptcp)   fprintf(fptcp, "writemembuf: n=%d\n", n);
...
extern void strinitcom(void)
{
    ...
    WSADATA data;
#endif
    //if (fptcp)   fprintf(fptcp,"strinitcom:\n");
#ifdef WIN32
...
{
    if (fptcp)   fprintf(fptcp, "strinit:\n");
...
{
    //if (fptcp)   fprintf(fptcp,"stropen: type=%d mode=%d path=%s\n", type, mode, path);
...
{
    if (fptcp)   fprintf(fptcp, "strclose: type=%d mode=%d\n", stream->type, stream->mode);
...
extern int strread(stream_t * stream, uint8_t * buff, int n)
{
    ...
    //if (fptcp)   fprintf(fptcp,"strread: n=%d\n", n);
...
extern int strwrite(stream_t * stream, uint8_t * buff, int n)
{
    ...
    //if (fptcp)   fprintf(fptcp,"strwrite: n=%d\n", n);
...
extern int strstat(stream_t * stream, char* msg)
{
    ...
    if (fptcp)   fprintf(fptcp, "strstat:\n");
...
extern int strstatx(stream_t * stream, char* msg)
{
    ...
    if (fptcp)   fprintf(fptcp, "strstatx:\n");
...
{
    if (fptcp)   fprintf(fptcp, "strsetopt: opt=%d %d %d %d %d %d %d %d\n", opt[0], opt[1], opt[2], opt[3], opt[4], opt[5], opt[6], opt[7]);
...
extern void strsettimeout(stream_t * stream, int toinact, int tirecon)
{
    ...
    if (fptcp)   fprintf(fptcp, "strsettimeout: toinact=%d tirecon=%d\n", toinact, tirecon);
...
{
    if (fptcp)   fprintf(fptcp, "strsetdir: dir=%s\n", dir);
...
{
    if (fptcp)   fprintf(fptcp, "strsetproxy: addr=%s\n", addr);
...
//    int n;
//
//    if (fptcp)   fprintf(fptcp,"strsendnmea: rr=%.3f %.3f %.3f\n", sol->rr[0], sol->rr[1], sol->rr[2]);
//
...
static int gen_hex(const char* msg, uint8_t * buff)
{
    ...
    if (fptcp)   fprintf(fptcp, "gen_hex: msg=%s\n", msg);
