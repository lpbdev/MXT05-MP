...
static double gettgd(gtime_t time, int sat)
{
	int i;
	for (i = 0; i < g_nav.n; i++) {
		if (g_nav.eph[i].sat != sat) continue;
...
static int solval(double* v, int nv, double* azel, int n, int* vsat, double* dop, const prcopt_t* opt)
{
	int i, ns;
	...
	if (nv > NXSPP)
	{
		...
		{
			trace(0x04, "chi square test failed  nv=%d tem=%lf  chisqr=%lf\n", nv, tmp, chisqr[nv - NXSPP - 1]);
			return -2;
		}
	}
	for (i = 0, ns = 0; i < n; i++)
...
static int rescode(int post, sol_t* sol, const obsd_t* obs, const int* svh, int n, double* rs,
	double* dts, double* azel, double* H, double* v, double* var_sat, double* var, double* resp,
	double* x, int* vsat, int* exc, double* lsqraim, const prcopt_t* opt, int* ns, double tt)
{
	...
	ecef2pos(rr, pos);
	lsqraim[1] = 0.0; lsqraim[2] = 0.0;

	for (i = *ns = 0; i < n; i++) {
		vsat[i] = 0; azel[i * 2] = azel[i * 2 + 1] = resp[i] = 0.0;
		if (svh[i])	continue;
		if (exc[obs[i].sat - 1] == 1)continue;
		...
		switch (sys) {
		case SYS_GPS: dtr = x[3]; break;
		case SYS_QZS: dtr = x[3]; break;
		case SYS_GLO: dtr = x[4]; break;
		case SYS_GAL: dtr = x[5]; break;
		case SYS_BDS: dtr = x[6]; break;
		...
		/* design matrix */
		for (j = 0; j < NXSPP; j++) H[j + nv * NXSPP] = j < 3 ? -e[j] : (j == 3 ? 0.0 : 0.0);
		/* time system and receiver bias offset correction */

		if (sys == SYS_GPS || sys == SYS_QZS) { H[3 + nv * NXSPP] = 1.0; mask[0] = 1; }
		else if (sys == SYS_GLO)
		{
			H[4 + nv * NXSPP] = 1.0; mask[1] = 1;
		}
		else if (sys == SYS_GAL) { H[5 + nv * NXSPP] = 1.0; mask[2] = 1; }
		else if (sys == SYS_BDS) { H[6 + nv * NXSPP] = 1.0; mask[3] = 1; }
		//else if (sys == SYS_QZS) { H[7 + nv * NXSPP] = 1.0; mask[4] = 1; }
		else mask[0] = 1;

		vsat[i] = 1; resp[i] = v[nv];
		var[nv] = varrLsq(opt, &obs[i], index, azel[1 + i * 2]) + var_sat[i] + var_ion + var_trop + var_P;
		//var[nv] = varerr_snr(opt, &obs[i], index, azel[1 + i * 2])+var_sat[i] + var_ion + var_trop + var_P;
		//var[nv] = varerr(opt, azel[1 + i * 2], sys) + var_sat[i] + var_ion + var_trop + var_P;
		//if (post == 0) {
		...
		//if (obs[i].rcv==1)
		//	printf("rcv=%d sys=%3d prn=%3d v[%3d]=%14.3f var=%14.3f P=%14.3f r=%14.3f ion=%5.2f trop=%5.2f dtr=%14.3f dts=%14.3f\n",
	...
	for (i = 0; i < NSYS; i++) {
		...
		v[nv] = 0.0;
		for (j = 0; j < NXSPP; j++) H[j + nv * NXSPP] = j == i + 3 ? 1.0 : 0.0;
		var[nv] = 0.01;
		nv++;
	}
	//printf("-------------------------------------\n");
...
static int estpos(const obsd_t* obs, int n, int* svh, double* rs, double* dts, double* resp,
	double* var_sat, double* azel, int* vsat, int* exc, double* lsqraim, const prcopt_t* opt, sol_t* sol, double tt)
{
	...
	double r1, r2;
	H = mat(NXSPP, n + 4); v = mat(n + 4, 1); var = mat(n + 4, 1);
	if ((!H) || (!v) || (!var))
	{
	...
	for (i = 0; i < MAXITR; i++) {
		nv = rescode(post, sol, obs, svh, n, rs, dts, azel, H, v, var_sat, var, resp, x, vsat, exc, lsqraim, opt, &ns, tt);
		if (i == 0) ns0 = ns;
		if (nv == -1) {
			i = 0;
			continue;
		...
		for (j = 0; j < nv; j++) {
			sig = sqrt(var[j]);
			v[j] /= sig;
			for (k = 0; k < NXSPP; k++) H[k + j * NXSPP] /= sig;
		}
		...
		if (norm(dx, NXSPP) < 1E-3) {
			...
			for (j = 0; j < 3; j++) sol->qr[j] = (float)Q[j + j * NXSPP];
			//for (j = 0; j < 3; j++) {
			...
			//if (sol->dtrwindowindex == 10) {
			//	sol->dtrwindowindex = 0;
			//}
			//for (j = 0; j < NSYS; j++) {
			//	sol->dtr[j] = 0;
			//	sol->dtrwindow[j][sol->dtrwindowindex] = x[j + 3]; /* receiver clock bias (s) */
			//}
			//for (j = 0; j < NSYS; j++) {
			//	for (k = 0; k < sol->dtrwindowcnt; k++) {
			//		sol->dtr[j] += sol->dtrwindow[j][k];
			//	}
			//	sol->dtr[j] = sol->dtr[j] / sol->dtrwindowcnt/ CLIGHT; /* receiver clock bias (s) */
			//}

			//sol->dtrwindowindex++;
			//if(sol->dtrwindowcnt<10)
			//	sol->dtrwindowcnt++;
			...
			sol->dop[0][0] = dop[0]; sol->dop[0][1] = dop[1]; sol->dop[0][2] = dop[2]; sol->dop[0][3] = dop[3];
			sol->ns[0] = ns;
			free(H); free(v); free(var);
			return stat;
		}
...
/* doppler residuals ---------------------------------------------------------*/
static int resdop(const obsd_t* obs, int n, const double* rs, const double* dts, const double* rr, const double* x,
	const double* azel, const int* vsat, double* v, double* H)
{
	...
	for (i = 0; i < n && i < MAXOBS; i++) {
		sys = satsys(obs[i].sat, &prn);
		if (sys == SYS_GPS || sys == SYS_QZS)	lam = g_gpsLam[0];
		else if (sys == SYS_GLO)    lam = g_gloLam[prn - 1][0];
		else if (sys == SYS_GAL)	lam = g_galLam[0];
		else lam = g_bdsLam[0];
...
//static int estvel(const obsd_t* obs, int n, const double* rs, const double* dts, const prcopt_t* opt, sol_t* sol,
...
//	return 1;
//}
static void estvel(const obsd_t* obs, int n, const double* rs, const double* dts, const prcopt_t* opt, sol_t* sol, const double* azel, const int* vsat)
{
	double x[4] = { 0 }, dx[4], Q[16], * v, * H;
	int i, j, nv;
...
static void update_state(const prcopt_t* opt, int base, int n, obsd_t* obs, ssat_t* ssat, int* vsat, double* resp, double* azel_) {
	...
	for (i = 0; i < n; i++)
	{
		...
		if (sys == SYS_GPS || sys == SYS_QZS) {
			for (j = 0; j < NFREQ; j++) lam[j] = g_gpsLam[j];
		}
		else if (sys == SYS_GLO) {
			for (j = 0; j < NFREQ; j++) lam[j] = g_gloLam[prn - 1][j];
		}
		else if (sys == SYS_GAL) {
			for (j = 0; j < NFREQ; j++) lam[j] = g_galLam[j];
		}
		else
			for (j = 0; j < NFREQ; j++) lam[j] = g_bdsLam[j];
		...
		if (base == 0) {
			...
			if (fabs(obs[i].P[index] - obs[i].L[index] * lam[index]) > 1000) {
				trace(0x04, "spp reject sys=%3d prn=%3d P[%d]=%14.3lf L[%d]=%14.3f P-L=%14.3f\n",
	...
	double* azel, ssat_t* ssat, const prcopt_t* opt, double tt)
{
	...
	opt_.sateph = EPHOPT_BRDC;  //#define EPHOPT_SBAS 2                   /* ephemeris option: broadcast + SBAS */
	opt_.ionoopt = IONOOPT_BRDC; //#define IONOOPT_SBAS 2                  /* ionosphere option: SBAS model */
	...
	while (1) {
		stat = estpos(obs, n, svh, rs, dts, resp, var_sat, azel_, vsat, exc, lsqraim, &opt_, sol, tt);
		//printf("-----------------------------------\n");
		if (stat == -2 && (sol->ns[0] > (5 + SysCnt)) && lsqraim[0] >= 1) {
			sat = (int)lsqraim[0];
			exc[sat - 1] = 1;
			sys = satsys(sat, &prn);
			trace(0x04, "rcv:%d spp reject sys=%d prn=%d v=%lf stdv=%lf\n", base + 1, sys, prn, lsqraim[1], lsqraim[2]);
			for (i = 0; i < 3; i++) lsqraim[i] = 0.0;
		}
