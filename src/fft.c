        ...
        for( l=1; l<=lb; l++ )
        {
            ...
            for(n=0; n<ld; n++)
            {
                ...
                w.imag   =  FFTSinTable[r];
                 ...
                 //tmp.real:10bit
                tmp.real = CONVERGENT_ROUND_SHIFT( (OutPara[lc].real*w.real -
                                        OutPara[lc].imag*w.imag),(TWIDDLE_FFT_WIDTH-2) ) ;
                tmp.imag = CONVERGENT_ROUND_SHIFT( (OutPara[lc].real*w.imag  +
                                        OutPara[lc].imag*w.real),(TWIDDLE_FFT_WIDTH-2) ) ;
