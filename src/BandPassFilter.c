...
int BpfCoef[24] = {-5, 9,   8,   -7, -5, -2,  -10, 20, 31, -42, -50, 53,
                   53, -50, -42, 31, 20, -10, -2,  -5, -7, 8,   9,   -5};
...
void BandPassFilterProcess(pBPF pbpf, S32* InputI, S32* InputQ)
{
    ...
    *InputQ = 0;
    ...
    {
        *InputI += (pbpf->FilterInI[LpfCnt] + pbpf->FilterInI[BPF_LEN - (LpfCnt + 1)]) *
                   pbpf->FilterCoef[LpfCnt];
        *InputQ += (pbpf->FilterInQ[LpfCnt] + pbpf->FilterInQ[BPF_LEN - (LpfCnt + 1)]) *
                   pbpf->FilterCoef[LpfCnt];
    }
}
