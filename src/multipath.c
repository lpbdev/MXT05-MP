...
static int test_update_hmp()
{
    ...
    if (hmp.n != 10 || fabs(hmp.mean - 4.50000) > 1E-6 || fabs(hmp.var - 8.25000) > 1E-6 ||
    ...
    if (hmp.n != 20 || fabs(hmp.mean - 5.50000) > 1E-6 || fabs(hmp.var - 8.25000) > 1E-6 ||
        fabs(sqrt(hmp.var) - 2.872281) > 1E-6)
    {
        return 1;
    }
...
static int get_res_seg(satres_t *satres, satseg_t *satseg, int *index) {
    ...
    for (i = *index; i < satres->epochnum-1; i++) {
        if (satres->res[i+1].time - satres->res[i].time <= satres->timeintv) {
            ...
            satseg->res[satseg->nepoch].azel[0] = (satres->res[i].azel[0] + satres->res[i +
1].azel[0]) / 2; satseg->res[satseg->nepoch].azel[1] = (satres->res[i].azel[1] + satres->res[i +
1].azel[1]) / 2; satseg->nepoch++;
        }
        else {
            break;
        }
...
extern int satres_insert(satres_t* satres, resdata_t* data)
{
    ...
    for (i = 0; i <= minNum; i++)
    {
        ...
        if (satres->data[minIndex].sod < 1E-5)
        {
            break;
        }
...
static int print_satres(satres_t* satres)
{
    ...
    //     printf("index = %02d, sod = %03.2f, res = %5.4f\n", minIndex, satres->data[minIndex].sod,
    //     satres->data[minIndex].res);
    // }

    for (i = 0; i < satres->nmax; i++)
    {
        // minIndex = (satres->n - i + satres->nmax) % satres->nmax;
        if (satres->data[i].sod < 1E-5)
        {
            continue;
        }
        printf(
            "index = %02d, sod = %03.2f, res = %5.4f\n", i, satres->data[i].sod, satres->data[i].res
        );
    }
...
extern int mkfpssat(rtk_t* rtk)
{
    ...
    for (i = 0; i < MAXSAT; i++)
    {
        ...
        else if (rtk->mpflag == 2)
        {  // load old MP cache data
