...
extern int matinvLambda(rtk_t* rtk, double* A, int n)
{
	...
	indx = imat(n, 1); B = rtk->I;
	if (!indx)
		return -1;
	matcpy(B, A, n, n);
	if (ludcmpLambda(B, n, indx, &d)) { free(indx); return -2; }
	for (j = 0; j < n; j++) {
...
*          X can be same as Y
*-----------------------------------------------------------------------------*/
static int solveLambda(rtk_t* rtk, const char* tr, double* A, const double* Y, int n,
	int m, double* X)
{
	//double *B=mat(n,n);
	int info;
...
extern int lambda(rtk_t* rtk, int n, int m, const double* a, const double* Q, double* f, double* s, int lcopt)
{
	...
	Z = rtk->H;
	//E = rtk->F;
	for (i = 0; i < n; i++) Z[i + i * n] = 1.0;
	/* LD factorization */
	if (!(info = LD(rtk, n, Q, L, D))) { //F
		/* lambda reduction */
