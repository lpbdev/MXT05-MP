...
double g_bdsLam[NFREQ] = {
    CLIGHT / FREQ1_CMP,   CLIGHT / FREQ2_CMP,
...
int GetHead(obsqueue_t *Q) {
  ...
  if (g_baseObsSyncIndex == 255)
    return 0;
  if (Q->data[0].data[0].time.time >
      g_baseObsSync[g_baseObsSyncIndex - 1][0].time.time)
    return 0;
  return 1;
}
...
static void updatesvr(rtksvr_t *svr, int ret, obs_t *obs, int index, int iobs) {
  ...
  if (ret == 1) { /* observation data */
    ...
    // svr->nmsg[index][0]++;
  } else if (ret == 5 && svr->rtk.opt.useRtcmPosFlag ==
                             1) { /* antenna postion parameters */
    if (index == 1) {
      for (i = 0; i < 3; i++) {
...
extern int decoderaw(rtksvr_t *svr, int index) {
  ...
  for (i = 0; i < svr->nb[index]; i++) {
    if (svr->format[index] == STRFMT_RTCM3) {
      ...
      if (ret != 0) {
        trace(2, "index,%d,time,%s,obs->n,%d\n", index,
              time_str(obs[0].data[0].time, 0), obs->n);
...
#ifdef MULBASE
static void outDnyResult(rtksvr_t *svr, char **pbuff, char *s1,
                         unsigned char iniEnuFlag, int fixCnt) {
  if (svr->rtk.opt.kMode == 0) {
    if (svr->rtk.opt.timeIntervalSolution != 0 && fixCnt > 0) {
      svr->rtk.sol.stat = 1;
      trace(4, "outDnyResult set stat 1\n");
    }
    if (svr->rtk.opt.masterSlaveBaseFlag == 0 &&
        svr->rtk.opt.masterXyz[0] != 0 && svr->rtk.opt.slaveXyz[0] != 0) {
      if (svr->rtk.dr[0] == 0.0)
        return;
      *pbuff += sprintf(
          *pbuff,
          ...
          svr->rtk.sol.rr_original[2] + svr->rtk.dr[2],
          svr->rtk.sol.enu_original[0], svr->rtk.sol.enu_original[1],
          svr->rtk.sol.enu_original[2],
          svr->rtk.sol.rr_filer[0] + svr->rtk.dr[0],
          svr->rtk.sol.rr_filer[1] + svr->rtk.dr[1],
          svr->rtk.sol.rr_filer[2] + svr->rtk.dr[2], svr->rtk.sol.enu[0],
          svr->rtk.sol.enu[1], svr->rtk.sol.enu[2], svr->rtk.sol.vel[0],
          svr->rtk.sol.vel[1], svr->rtk.sol.vel[2], svr->rtk.sol.acc[0],
          svr->rtk.sol.acc[1], svr->rtk.sol.acc[2], svr->rtk.opt.masterXyz[0],
          svr->rtk.opt.masterXyz[1], svr->rtk.opt.masterXyz[2], iniEnuFlag);
    } else {
      *pbuff += sprintf(
          *pbuff,
          "pos,%s,%d,%d,%.9lf,%.9lf,%.9lf,%.9lf,%.9lf,%.9lf,%.9lf,%.9lf,%.9lf,%"
          ...
          "4lf,%d;",
          s1, svr->rtk.sol.stat, svr->rtk.sol.ns[0],
          svr->rtk.sol.rr_original[0], svr->rtk.sol.rr_original[1],
          svr->rtk.sol.rr_original[2], svr->rtk.sol.enu_original[0],
          svr->rtk.sol.enu_original[1], svr->rtk.sol.enu_original[2],
          svr->rtk.sol.rr_filer[0], svr->rtk.sol.rr_filer[1],
          svr->rtk.sol.rr_filer[2], svr->rtk.sol.enu[0], svr->rtk.sol.enu[1],
          svr->rtk.sol.enu[2], svr->rtk.sol.vel[0], svr->rtk.sol.vel[1],
          svr->rtk.sol.vel[2], svr->rtk.sol.acc[0], svr->rtk.sol.acc[1],
          svr->rtk.sol.acc[2], svr->rtk.opt.masterXyz[0],
          svr->rtk.opt.masterXyz[1], svr->rtk.opt.masterXyz[2], iniEnuFlag);
    }
  } else {
    if (svr->rtk.opt.timeIntervalSolution != 0 && fixCnt > 0) {
    ...
    if (svr->rtk.opt.masterSlaveBaseFlag == 0 &&
      ...
      *pbuff += sprintf(
          *pbuff,
          ...
          svr->rtk.sol.rr_filer[1] + svr->rtk.dr[1],
          svr->rtk.sol.rr_filer[2] + svr->rtk.dr[2], svr->rtk.sol.enu[0],
          svr->rtk.sol.enu[1], svr->rtk.sol.enu[2],
          svr->rtk.sol.rr_filer[0] + svr->rtk.dr[0],
          svr->rtk.sol.rr_filer[1] + svr->rtk.dr[1],
          svr->rtk.sol.rr_filer[2] + svr->rtk.dr[2], svr->rtk.sol.enu[0],
          svr->rtk.sol.enu[1], svr->rtk.sol.enu[2], svr->rtk.sol.vel[0],
      ...
      *pbuff += sprintf(
          *pbuff,
          ...
          svr->rtk.sol.rr_filer[1], svr->rtk.sol.rr_filer[2],
  ...
  if (svr->rtk.opt.senceopt == 1) {
    *pbuff += sprintf(*pbuff, "fft,%.4f,%.4f;", svr->rtk.fftFrq[2],
                      svr->rtk.fftPower[2]);
  } else {
    if ((svr->rtk.sol.time.time + ROUND(svr->rtk.sol.time.frac)) %
            (int)writeConfigTime ==
        0) {
      *pbuff += sprintf(*pbuff, "\n");
      if (svr->rtk.opt.masterSlaveBaseFlag == 0 &&
        ...
        *pbuff += sprintf(
            *pbuff,
            "cof,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%d,%d,%d,%.4f,%.4f,%.4f;",
            svr->rtk.opt.masterXyz[0], svr->rtk.opt.masterXyz[1],
            svr->rtk.opt.masterXyz[2], svr->rtk.sol.enu[0], svr->rtk.sol.enu[1],
            svr->rtk.sol.enu[2], svr->rtk.enuWindwoIndex[0],
        ...
        *pbuff += sprintf(
            *pbuff,
            ...
            svr->rtk.enuWindwoIndex[2], svr->rtk.sol.rr_filer[0],
            svr->rtk.sol.rr_filer[1], svr->rtk.sol.rr_filer[2]);
      }
    }
  }
  ...
  if (svr->rtk.opt.kMode == 0) {
    ...
    *pbuff += sprintf(
        *pbuff,
        ...
        svr->rtk.sol.enu_original[0], svr->rtk.sol.enu_original[1],
        svr->rtk.sol.enu_original[2], svr->rtk.sol.rr_filer[0],
        svr->rtk.sol.rr_filer[1], svr->rtk.sol.rr_filer[2], svr->rtk.sol.enu[0],
        svr->rtk.sol.enu[1], svr->rtk.sol.enu[2], svr->rtk.sol.vel[0],
        svr->rtk.sol.vel[1], svr->rtk.sol.vel[2], svr->rtk.sol.acc[0],
        svr->rtk.sol.acc[1], svr->rtk.sol.acc[2], svr->rtk.rb[0],
        svr->rtk.rb[1], svr->rtk.rb[2], iniEnuFlag);
  } else {
    if (svr->rtk.opt.timeIntervalSolution != 0 && fixCnt > 0) {
      svr->rtk.sol.stat = 1;
      trace(4, "outDnyResult set stat 1\n");
  ...
  if (svr->rtk.opt.senceopt == 1) {
    *pbuff += sprintf(*pbuff, "fft,%.4f,%.4f;", svr->rtk.fftFrq[2],
    ...
    if ((svr->rtk.sol.time.time + ROUND(svr->rtk.sol.time.frac)) %
        ...
        0) {
      *pbuff += sprintf(*pbuff, "\n");
      *pbuff += sprintf(*pbuff, "cof,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%d,%d,%d;",
                        svr->rtk.rb[0], svr->rtk.rb[1], svr->rtk.rb[2],
                        svr->rtk.sol.enu[0], svr->rtk.sol.enu[1],
                        svr->rtk.sol.enu[2], svr->rtk.enuWindwoIndex[0],
                        svr->rtk.enuWindwoIndex[1], svr->rtk.enuWindwoIndex[2]);
    }
  }
...
extern int get_json_int(char *idStr, cJSON *json, char *item, stream_t *stream,
                        int *out) {
  cJSON *json_data_tmp;
...
extern int get_json_uint(char *idStr, cJSON *json, char *item, stream_t *stream,
                         uint8_t *out) {
  cJSON *json_data_tmp;
...
extern int get_json_double(char *idStr, cJSON *json, char *item,
                           stream_t *stream, double *out) {
  cJSON *json_data_tmp;
  json_data_tmp = cJSON_GetObjectItem(json, item);
...
static int decodeConfig(rtksvr_t *svr, cfgopt_t *cfgOpt, char *buff) {
  ...
  if (json_data_tmp == NULL) {
    ...
    if (str != NULL) {
      ...
      level_trace = (json_data_tmp->valueint > 7
                         ? 0xFF
                         : (0xFF >> (8 - json_data_tmp->valueint)));
    }
  }
  ...
  if (configflag == 1) {
    ...
    if (g_cfgOpt.masterSlaveBaseFlag == 0 ||
      ...
      svr->rtk.masterEnu[0] = svr->rtk.masterEnu[1] = svr->rtk.masterEnu[2] =
          0.0;
      ...
      if (g_cfgOpt.masterSlaveBaseFlag == 0) {
        ...
        svr->rtk.sol.fixxyz[0] = svr->rtk.sol.fixxyz[1] =
            svr->rtk.sol.fixxyz[1] = 0.0;
        svr->rtk.sol.rr_smooth_cnt = 0;
        svr->rtk.iniCnt = 0;
...
static int traceconfig(void) {
  ...
  trace(2,
        ...
        "detectSensitivity:%d\n",
        g_cfgOpt.kMode, g_cfgOpt.freq, g_cfgOpt.iono, g_cfgOpt.trop,
        g_cfgOpt.tides, g_cfgOpt.sys, g_cfgOpt.senceopt,
        g_cfgOpt.detectSensitivity);
  ...
  trace(2,
        ...
        g_cfgOpt.typeSol, g_cfgOpt.timeIntervalSolution, g_cfgOpt.minFixSat,
        g_cfgOpt.maxDelSat, g_cfgOpt.param, g_cfgOpt.gpsMask,
        g_cfgOpt.qzssMask);

  trace(2,
        "glonassMask:%ld galieoMask:%ld bdsMask:%ld minSatRes:%.2f "
        "iggiiik0:%.2f iggiiik1:%.2f smoothWindowsTime:%.2f\n",
        g_cfgOpt.glonassMask, g_cfgOpt.galieoMask, g_cfgOpt.bdsMask,
        g_cfgOpt.minSatRes, g_cfgOpt.iggiiik0, g_cfgOpt.iggiiik1,
        g_cfgOpt.smoothWindowsTime);

  trace(2,
        "initEnuTime:%.2f diffAgeMax:%d buffSize:%d cn0Min:%.2f elevMin:%.2f "
        "gdopThld:%.2f postResThld:%.2f\n",
        g_cfgOpt.initEnuTime, g_cfgOpt.diffAgeMax, g_cfgOpt.buffSize,
        g_cfgOpt.cn0Min, g_cfgOpt.elevMin, g_cfgOpt.gdopThld,
        g_cfgOpt.postResThld);
...
#endif
{
  int i, j, f, k, m, n, cnt, dtMinIndex, cputime, fobs[2] = {0}, fnobs, size,
                                                  ouInterval, fixCnt = 0;
  ...
  for (cycle = 0; svr->state; cycle++) {
    ...
    if (g_cfgOpt.initEnuTime > g_cfgOpt.smoothWindowsTime) {
      svr->rtk.opt.initEnuTime = g_cfgOpt.initEnuTime =
          g_cfgOpt.smoothWindowsTime;
    }
    if (ouInterval != 0) {
    ...
    for (i = 0; i < fobs[0]; i++) {
      ...
      for (j = 0; j < MAXSAT; j++) {
        ...
        svr->rtk.ssat[j].SNR[0] = svr->rtk.ssat[j].SNR[1] =
            svr->rtk.ssat[j].SNR[2] = svr->rtk.ssat[j].SNR[3] =
                svr->rtk.ssat[j].SNR[4] = svr->rtk.ssat[j].SNR[5] = 0.0;
      }
      for (j = 0; j < n; j++) {
      ...
      if (!pntpos(0, obs, n, &svr->rtk.sol, NULL, svr->rtk.ssat, &svr->rtk.opt,
                  0)) { // 0:is ok  -1 eoror
        memset(buff, 0, DEBUG_BUFF_LEN);
        pbuff = buff;
    ...
    while (GetHead(&Qrover)) {
      ...
      if (svr->rtk.opt.masterSlaveBaseFlag == 0 &&
        ...
        if (svr->rtk.opt.masterXyz[0] == 0.0 &&
            svr->rtk.opt.masterXyz[1] == 0.0 &&
      ...
      if (rtkReturnValue != 0) {
        ...
        pbuff += sprintf(pbuff, "\n");
        if (svr->rtk.opt.typeSol == 0 &&
            svr->rtk.opt.timeIntervalSolution == 0) {
          strwrite(&svr->stream[2], (uint8_t *)buff, strlen(buff));
        }
        ...
        if (svr->rtk.enuWindwoIndex[0] * (double)svr->rtk.opt.timeInterval ==
          ...
          // rebootFlag=1;
          trace(4,
                "enuWindwoIndex:%d,timeInterval:%.2f,smoothWindowsTime:%.2f\n",
        ...
        if (outsol(NULL, &svr->rtk, &svr->rtk.sol, svr->rtk.rb, &sopt)) {
          ...
          if (svr->rtk.sol.vel_pre[0] != 0.0 &&
              svr->rtk.sol.vel_pre[1] != 0.0 &&
...
int main(int argc, char **argv) {
  ...
  if (argc < 26) {
    if (argc == 19) // Without Configuration Parameters
    {
      ...
      printf("enuWindow:%14.4f %14.4f %14.4f\n", g_cfgOpt.enuWindow[0],
             g_cfgOpt.enuWindow[1], g_cfgOpt.enuWindow[2]);
      ...
      printf("enuWindowIndex:%d %d %d\n", g_cfgOpt.enuWindowIndex[0],
             g_cfgOpt.enuWindowIndex[1], g_cfgOpt.enuWindowIndex[2]);
  ...
  for (i = 0; i < 2; i++) {
    ...
    if (!(svr.buff[i] =
              (unsigned char *)calloc(sizeof(unsigned char), BUFFSIZE)) ||
        !(svr.pbuf[i] =
              (unsigned char *)calloc(sizeof(unsigned char), BUFFSIZE))) {
      return 0;
    }
