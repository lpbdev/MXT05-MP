...
static int readseribuff(serial_t* serial, uint8_t* buff, int nmax)
{
    ...
    unlock(&serial->lock);
    // trace(4, "readseribuff: ns=%d rp=%d wp=%d\n", ns, serial->rp, serial->wp);
    return ns;
}
static int writeseribuff(serial_t* serial, uint8_t* buff, int n)
{
    ...
    // trace(4, "writeseribuff: dev=%d n=%d\n", serial->dev, n);
    ...
    for (ns = 0; ns < n; ns++)
    {
        ...
        {
            // trace(4, "serial buffer overflow: size=%d\n", serial->buffsize);
            break;
        }
    }
    unlock(&serial->lock);
    // trace(4, "writeseribuff: ns=%d rp=%d wp=%d\n", ns, serial->rp, serial->wp);
    return ns;
}
#endif /* WIN32 */
...
static serial_t* openserial(const char* path, int mode, char* msg)
{
    ...
    int            rw   = 0;
#endif
    // trace(4, "openserial: path=%s mode=%d\n", path, mode);
    ...
    // trace(4, "openserial: dev=%d\n", serial->dev);
    return serial;
}
/* close serial --------------------------------------------------------------*/
static void closeserial(serial_t* serial)
{
    // trace(4, "closeserial: dev=%d\n", serial->dev);
...
#endif
    // trace(4, "readserial: exit dev=%d nr=%d\n", serial->dev, nr);
...
#endif
    // trace(4, "writeserial: exit dev=%d ns=%d\n", serial->dev, ns);
    return ns;
}
/* get state serial ----------------------------------------------------------*/
...
static int openfile_(file_t* file, gtime_t time, char* msg)
{
    ...
    // trace(4, "openfile_: path=%s time=%s\n", file->path, time_str(time, 0));
    ...
    // trace(4, "openfile_: open file %s (%s)\n", file->openpath, rw);
    ...
    { /* output/sync time-tag */
        ...
        // trace(4, "openfile_: open tag file %s (%s)\n", tagpath, rw);
...
static file_t* openfile(const char* path, int mode, char* msg)
{
    ...
    // trace(4, "openfile: path=%s mode=%d\n", path, mode);
    ...
    if (!(file = (file_t*)calloc(sizeof(file_t), 1)))
    {
        return NULL;
    }
...
static void swapfile(file_t* file, gtime_t time, char* msg)
{
    ...
    {
        // trace(4, "swapfile: no need to swap %s\n", openpath);
        return;
    }
    ...
    if (file->fp_tag)
    {
        ...
        while ((int)(file->tick_n - t) <= 0)
        {
            if (fread(&file->tick_n, sizeof(tick), 1, file->fp_tag) < 1 ||
                fread(
                    (file->size_fpos == 4) ? (void*)&fpos_4B : (void*)&fpos_8B, file->size_fpos, 1,
                    file->fp_tag
                ) < 1)
            {
...
static int writefile(file_t* file, uint8_t* buff, int n, char* msg)
{
    ...
    // trace(4, "writefile: fp=%d ns=%d tick=%5d fpos=%d\n", file->fp, ns, tick,
    // fpos);
...
static int gentcp(tcp_t* tcp, int type, char* msg)
{
    ...
    { /* server socket */
        ...
        if (bind(tcp->sock, (struct sockaddr*)&tcp->addr, sizeof(tcp->addr)) == -1)
        {
            ...
            // trace(1, "gentcp: bind error port=%d err=%d\n", tcp->port, errsock());
    ...
    { /* client socket */
        if (!(hp = gethostbyname(tcp->saddr)))
        {
            ...
            // trace(1, "gentcp: gethostbyname error addr=%s err=%d\n", tcp->saddr,
            // errsock());
            closesocket(tcp->sock);
            tcp->state = 0;
            tcp->tcon  = ticonnect;
    ...
    tcp->state = 1;
    tcp->tact  = tickget();
    // trace(4, "gentcp: exit sock=%d\n", tcp->sock);
    return 1;
}
/* disconnect tcp ------------------------------------------------------------*/
static void discontcp(tcp_t* tcp, int tcon)
{
    // trace(4, "discontcp: sock=%d tcon=%d\n", tcp->sock, tcon);
...
static tcpsvr_t* opentcpsvr(const char* path, char* msg)
{
    ...
    *tcpsvr = tcpsvr0;
    ...
    if (sscanf(port, "%d", &tcpsvr->svr.port) < 1)
    {
        ...
        // trace(1, "opentcpsvr: port error port=%s\n", port);
        free(tcpsvr);
        return NULL;
    }
...
static int accsock(tcpsvr_t* tcpsvr, char* msg)
{
    ...
    if (i >= MAXCLI)
    {
        // trace(4, "accsock: too many clients sock=%d\n", tcpsvr->svr.sock);
        return 0;
    }
    ...
    // trace(4, "accsock: connected sock=%d addr=%s i=%d\n", tcpsvr->cli[i].sock,
...
{
    // trace(4, "waittcpsvr: sock=%d state=%d\n", tcpsvr->svr.sock,
    // tcpsvr->svr.state);
...
static int readtcpsvr(tcpsvr_t* tcpsvr, uint8_t* buff, int n, char* msg)
{
    ...
    for (i = 0; i < MAXCLI; i++)
    {
        ...
        if ((nr = recv_nb(tcpsvr->cli[i].sock, buff, n)) == -1)
        {
            if ((err = errsock()))
            {
                // trace(4, "readtcpsvr: recv error sock=%d
...
static int writetcpsvr(tcpsvr_t* tcpsvr, uint8_t* buff, int n, char* msg)
{
    ...
    // trace(4, "writetcpsvr: state=%d n=%d\n", tcpsvr->svr.state, n);
    ...
    for (i = 0; i < MAXCLI; i++)
    {
        ...
        if ((ns = send_nb(tcpsvr->cli[i].sock, buff, n)) == -1)
        {
            if ((err = errsock()))
            {
                // trace(4, "writetcpsvr: send error i=%d sock=%d err=%d\n", i,
                // tcpsvr->cli[i].sock, err);
            }
            discontcp(&tcpsvr->cli[i], ticonnect);
            updatetcpsvr(tcpsvr, msg);
...
static int consock(tcpcli_t* tcpcli, char* msg)
{
    ...
    /* non-block connect */
    if ((stat = connect_nb(
             tcpcli->svr.sock, (struct sockaddr*)&tcpcli->svr.addr, sizeof(tcpcli->svr.addr)
         )) == -1)
    {
        err = errsock();
        sprintf(msg, "connect error (%d)", err);
        // trace(4, "consock: connect error sock=%d err=%d\n", tcpcli->svr.sock,
        // err);
    ...
    // trace(4, "consock: connected sock=%d addr=%s\n", tcpcli->svr.sock,
    // tcpcli->svr.saddr);
    tcpcli->svr.state = 2;
...
static tcpcli_t* opentcpcli(const char* path, char* msg)
{
    ...
    *tcpcli = tcpcli0;
    ...
    {
        sprintf(msg, "port error: %s", port);
        // trace(4, "opentcp: port error port=%s\n", port);
        free(tcpcli);
        return NULL;
    }
    tcpcli->svr.tcon = 0;
...
static int waittcpcli(tcpcli_t* tcpcli, char* msg)
{
    ...
    { /* connect */
        ...
        {
            sprintf(msg, "timeout");
            // trace(4, "waittcpcli: inactive timeout sock=%d\n", tcpcli->svr.sock);
...
static int readtcpcli(tcpcli_t* tcpcli, uint8_t* buff, int n, char* msg)
{
    ...
    if ((nr = recv_nb(tcpcli->svr.sock, buff, n)) == -1)
    {
        if ((err = errsock()))
        {
            // trace(4, "readtcpcli: recv error sock=%d err=%d\n", tcpcli->svr.sock,
            // err);
    ...
    // trace(4, "readtcpcli: exit sock=%d nr=%d\n", tcpcli->svr.sock, nr);
    return nr;
}
/* write tcp client ----------------------------------------------------------*/
static int writetcpcli(tcpcli_t* tcpcli, uint8_t* buff, int n, char* msg)
{
    ...
    // trace(4, "writetcpcli: sock=%d state=%d n=%d\n", tcpcli->svr.sock,
    // tcpcli->svr.state, n);
    ...
    if ((ns = send_nb(tcpcli->svr.sock, buff, n)) == -1)
    {
        if ((err = errsock()))
        {
            // trace(4, "writetcp: send error sock=%d err=%d\n", tcpcli->svr.sock,
            // err);
    ...
    // trace(4, "writetcpcli: exit sock=%d ns=%d\n", tcpcli->svr.sock, ns);
    return ns;
}
...
static int reqntrip_s(ntrip_t* ntrip, char* msg)
{
    ...
    // trace(4, "reqntrip_s: send request state=%d ns=%d\n", ntrip->state, p -
    // buff); trace(4, "reqntrip_s: n=%d buff=\n%s\n", p - buff, buff);
    ntrip->state = 1;
    return 1;
}
...
static int reqntrip_c(ntrip_t* ntrip, char* msg)
{
    ...
    // trace(4, "reqntrip_c: send request state=%d ns=%d\n", ntrip->state, p -
    // buff); trace(4, "reqntrip_c: n=%d buff=\n%s\n", p - buff, buff);
    ntrip->state = 1;
    return 1;
}
...
static int rspntrip_s(ntrip_t* ntrip, char* msg)
{
    ...
    // trace(4, "rspntrip_s: state=%d nb=%d\n", ntrip->state, ntrip->nb);
    ntrip->buff[ntrip->nb] = '0';
    // trace(4, "rspntrip_s: n=%d buff=\n%s\n", ntrip->nb, ntrip->buff);
    ...
    { /* ok */
        ...
        // trace(4, "rspntrip_s: response ok nb=%d\n", ntrip->nb);
        return 1;
    }
    ...
    { /* buffer overflow */
        ...
        // trace(4, "rspntrip_s: response overflow nb=%d\n", ntrip->nb);
        ntrip->nb      = 0;
        ntrip->buff[0] = '\0';
        ntrip->state   = 0;
        discontcp(&ntrip->tcp->svr, ntrip->tcp->tirecon);
    }
    // trace(4, "rspntrip_s: exit state=%d nb=%d\n", ntrip->state, ntrip->nb);
    return 0;
}
...
static int rspntrip_c(ntrip_t* ntrip, char* msg)
{
    ...
    // trace(4, "rspntrip_c: state=%d nb=%d\n", ntrip->state, ntrip->nb);
    ntrip->buff[ntrip->nb] = '0';
    // trace(4, "rspntrip_c: n=%d buff=\n%s\n", ntrip->nb, ntrip->buff);
    ...
    { /* ok */
        ...
        ntrip->state = 2;
        sprintf(msg, "%s/%s", ntrip->tcp->svr.saddr, ntrip->mntpnt);
        // trace(4, "rspntrip_c: response ok nb=%d\n", ntrip->nb);
        return 1;
    }
    ...
    { /* source table */
        ...
        { /* source table request */
            ...
            // trace(4, "rspntrip_c: receive source table nb=%d\n", ntrip->nb);
            return 1;
        }
        sprintf(msg, "no mountp. reconnect...");
        // trace(4, "rspntrip_c: no mount point nb=%d\n", ntrip->nb);
        ntrip->nb      = 0;
        ntrip->buff[0] = '\0';
        ntrip->state   = 0;
        discontcp(&ntrip->tcp->svr, ntrip->tcp->tirecon);
    ...
    // trace(4, "rspntrip_c: exit state=%d nb=%d\n", ntrip->state, ntrip->nb);
    return 0;
}
...
static ntrip_t* openntrip(const char* path, int type, char* msg)
{
    ...
    // trace(4, "openntrip: path=%s type=%d\n", path, type);
    ...
    {
        // trace(4, "openntrip: opentcp error\n");
        free(ntrip);
        return NULL;
    }
    return ntrip;
...
static ntripc_t* openntripc(const char* path, char* msg)
{
    ...
    if (!*ntripc->mntpnt)
    {
        // trace(4, "openntripc: no mountpoint path=%s\n", path);
        free(ntripc);
        return NULL;
    }
    /* use default port if no port specified */
    ...
    {
        // trace(4, "openntripc: opentcpsvr error port=%d\n", port);
        free(ntripc);
        return NULL;
    }
    return ntripc;
...
static void rsp_ntripc(ntripc_t* ntripc, int i)
{
    ...
    // trace(4, "rspntripc_c i=%d\n", i);
    con->buff[con->nb] = '\0';
    // trace(4, "rspntripc_c: n=%d,buff=\n%s\n", con->nb, con->buff);

    if (con->nb >= NTRIP_MAXRSP - 1)
    { /* buffer overflow */
        // trace(4, "rsp_ntripc_c: request buffer overflow\n");
    ...
    {
        // trace(4, "rsp_ntripc_c: NTRIP request error\n");
        discon_ntripc(ntripc, i);
        return;
    }
    /* test protocol */
    ...
    {
        // trace(4, "rsp_ntripc_c: NTRIP request error proto=%s\n", proto);
        discon_ntripc(ntripc, i);
        return;
    }
    if ((p = strchr(url, '/')))
    ...
    {
        // trace(4, "rsp_ntripc_c: no mountpoint %s\n", mntpnt);
    ...
    if (*ntripc->passwd)
    {
        ...
        {
            // trace(4, "rsp_ntripc_c: authroziation error\n");
...
static void wait_ntripc(ntripc_t* ntripc, char* msg)
{
    ...
    for (i = 0; i < MAXCLI; i++)
    {
        ...
        if ((n = recv_nb(ntripc->tcp->cli[i].sock, buff, nmax)) == -1)
        {
            if ((err = errsock()))
            {
                // trace(4, "wait_ntripc: recv error sock=%d
...
static int readntripc(ntripc_t* ntripc, uint8_t* buff, int n, char* msg)
{
    ...
    for (i = 0; i < MAXCLI; i++)
    {
        ...
        if (nr < 0)
        {
            if ((err = errsock()))
            {
                // trace(4, "readntripc: recv error i=%d sock=%d err=%d\n",
...
static int writentripc(ntripc_t* ntripc, uint8_t* buff, int n, char* msg)
{
    ...
    for (i = 0; i < MAXCLI; i++)
    {
        ...
        if (ns < n)
        {
            if ((err = errsock()))
            {
                // trace(4, "writentripc: send error i=%d sock=%d err=%d\n", i,
                // ntripc->tcp->cli[i].sock, err);
            }
            discon_ntripc(ntripc, i);
        }
...
static udp_t* genudp(int type, int port, const char* saddr, char* msg)
{
    ...
    {
        // trace(4, "genudp: setsockopt error sock=%d err=%d bs=%d\n", udp->sock,
        // errsock(), bs);
        sprintf(msg, "sockopt error: bufsiz");
    }
    ...
    { /* udp server */
        ...
        {
            // trace(4, "genudp: bind error sock=%d port=%d err=%d\n", udp->sock,
            // port, errsock());
    ...
    { /* udp client */
        ...
        {
            // trace(4, "genudp: setsockopt error sock=%d err=%d\n", udp->sock,
            // errsock());
...
static udp_t* openudpsvr(const char* path, char* msg)
{
    ...
    if (sscanf(sport, "%d", &port) < 1)
    {
        ...
        // trace(4, "openudpsvr: port error port=%s\n", port);
        return NULL;
    }
...
static udp_t* openudpcli(const char* path, char* msg)
{
    ...
    {
        sprintf(msg, "port error: %s", sport);
        // trace(4, "openudpcli: port error port=%s\n", sport);
        return NULL;
    }
...
{
    // trace(4, "writeudpcli: sock=%d n=%d\n", udpcli->sock, n);

    return (int)sendto(
        udpcli->sock, (char*)buff, n, 0, (struct sockaddr*)&udpcli->addr, sizeof(udpcli->addr)
    );
}
...
/* decode ftp path -----------------------------------------------------------*/
static void decodeftppath(
    const char* path, char* addr, char* file, char* user, char* passwd, int* topts
)
{
    char buff[MAXSTRPATH], *p, *q;

    // trace(4, "decodeftpath: path=%s\n", path);
...
static gtime_t nextdltime(const int* topts, int stat)
{
    ...
    // trace(4, "nextdltime: topts=%d %d %d %d stat=%d\n", topts[0], topts[1],
...
#endif
{
    // trace(4, "ftpthread: complete cmd=%s\n", cmd);
    return 0;
}
/* open ftp ------------------------------------------------------------------*/
...
static int readftp(ftp_t* ftp, uint8_t* buff, int n, char* msg)
{
    ...
    { /* ftp/http not executed? */
        ...
        {
#endif
            // trace(4, "readftp: ftp thread create error\n");
            ftp->state = 3;
...
{
    // trace(4, "stropen: type=%d mode=%d path=%s\n", type, mode, path);
...
{
    // trace(4, "strclose: type=%d mode=%d\n", stream->type, stream->mode);
...
extern int strread(stream_t* stream, uint8_t* buff, int n)
{
    ...
    switch (stream->type)
    {
        ...
        case STR_NTRIPCLI:
            ...
            break;
            // case STR_NTRIPCAS: nr = readntripc((ntripc_t*)stream->port, buff, n,
            // msg); break; case STR_UDPSVR: nr = readudpsvr((udp_t*)stream->port, buff,
            // n, msg); break;
            // case STR_MEMBUF: nr = readmembuf((membuf_t*)stream->port, buff, n, msg);
            // break;
        case STR_FTP:
...
extern int strwrite(stream_t* stream, uint8_t* buff, int n)
{
    ...
    switch (stream->type)
    {
        ...
        case STR_NTRIPCLI:
            ...
            break;
            // case STR_NTRIPCAS: ns = writentripc((ntripc_t*)stream->port, buff, n,
            // msg); break; case STR_UDPCLI: ns = writeudpcli((udp_t*)stream->port,
            // buff, n, msg); break; case STR_MEMBUF: ns =
            // writemembuf((membuf_t*)stream->port, buff, n, msg); break;
        case STR_FTP:
        case STR_HTTP:
        default:
            strunlock(stream);
 ...
 * --------------------------------------------------- get extended stream
 * status args   : stream_t *stream I   stream char   *msg      IO  extended
 * status message return : status (-1:error,0:close,1:wait,2:connect,3:active)
 *-----------------------------------------------------------------------------*/
extern int strstatx(stream_t* stream, char* msg)
{
    ...
    switch (stream->type)
    {
        ...
        case STR_NTRIPCLI:
            ...
            break;
            // case STR_NTRIPCAS: state = statexntripc((ntripc_t*)stream->port, msg);
            // break;
            ...
            // case STR_MEMBUF: state = statexmembuf((membuf_t*)stream->port, msg);
            // break;
        case STR_FTP:
...
{
    // trace(4, "strsetopt: opt=%d %d %d %d %d %d %d %d\n", opt[0], opt[1],
...
extern void strsettimeout(stream_t* stream, int toinact, int tirecon)
{
    ...
    // trace(4, "strsettimeout: toinact=%d tirecon=%d\n", toinact, tirecon);
...
//    int n;
//
//    //trace(4, "strsendnmea: rr=%.3f %.3f %.3f\n", sol->rr[0], sol->rr[1],
//    sol->rr[2]);
//
...
extern int strgetsel(stream_t* stream, char* sel)
{
    *sel = '\0';
    ...
    if (!stream->port || (stream->type != STR_NTRIPC_S && stream->type != STR_NTRIPC_C))
    {
        return 0;
    }
    strlock(stream);
...
extern int strsetsrctbl(stream_t* stream, const char* file)
{
    ...
    if (!(fp = fopen(file, "rb")))
    {
        trace(2, "strsetsrctbl: file open error %s\n", file);
        strunlock(stream);
        return 0;
    }
