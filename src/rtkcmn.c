...
gtime_t epoch2time(const double* ep)
{
    ...
    int days = (year - 1970) * 365 + doy[month - 1] + (((year % 4 == 0) && (month > 2)) ? 1 : 0) +
               day - 1 +
               (year - 1969) / 4;  //(year-1-1968)/4: leap days since 1970, ignore current year.
    sec       = (int)ep[5];
...
extern double ionmodel(gtime_t t, const double* ion, const double* pos, const double* azel)
{
    ...
    return CLIGHT * f *
           (fabs(x) < 1.57 ? 5E-9 + amp * (1.0 + x * x * (-0.5 + x * x / 24.0)) : 5E-9);
}
/* compute dops ----------------------------------------------------------------
...
extern void time2str(gtime_t t, char* s, int n)
{
    ...
    time2epoch(t, ep);
    sprintf(
        s, "%04.0f/%02.0f/%02.0f %02.0f:%02.0f:%0*.*f", ep[0], ep[1], ep[2], ep[3], ep[4],
...
extern int execcmd(const char* cmd)
{
    ...
    if (!CreateProcess(
            NULL, (LPTSTR)cmds, NULL, NULL, FALSE, CREATE_NO_WINDOW, NULL, NULL, &si, &info
        ))
    {
        return -1;
    }
