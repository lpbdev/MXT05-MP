...
static char codepris[7][MAXFREQ][16] = {
    ...
    {"CPYWMNSL", "PYWCMNDSLX", "IQX", "PYWCMNDSLX", "PYWCMNDSLX", "",
     ""},                                                            /* GPS */
    {"PC", "PC", "IQX", "", "", "", ""},                             /* GLO */
...
const static double leaps[][7] =
    {/* leap seconds {y,m,d,h,m,s,gpst-utc,...} */
...
gtime_t epoch2time(const double *ep) {
  gtime_t temp;
  int doy[12] = {0,   31,  59,  90,  120, 151,
                 181, 212, 243, 273, 304, 334}; // distance of first day
  // of months from January 1st in common year
  int year = (int)ep[0], month = (int)ep[1], day = (int)ep[2], sec;

  int days =
      (year - 1970) * 365 + doy[month - 1] +
      (((year % 4 == 0) && (month > 2)) ? 1 : 0) + day - 1 +
      (year - 1969) /
          4; //(year-1-1968)/4: leap days since 1970, ignore current year.
  sec = (int)ep[5];
...
extern gtime_t gpst2utc(gtime_t t) { return timeadd(t, -leaps[0][6]); }
...
extern void matmul33(const char *tr, const double *A, const double *B,
  ...
  matmul(tr_, n, m, q, 1.0, T, C, 0.0, D);
  free(T);
}
...
extern double satazel(const double *pos, const double *e, double *azel) {
  ...
  if (pos[2] > -RE_WGS84) {
    ecef2enu(pos, e, enu);
    az =
        enu[0] * enu[0] + enu[1] * enu[1] < 1E-12 ? 0.0 : atan2(enu[0], enu[1]);
    if (az < 0.0)
      az += 2 * PI;
    el = asin(enu[2]);
  }
 ...
 * order (fortran convention) if state x[i]==0.0, not updates state
 * x[i]/P[i+i*n]
 *-----------------------------------------------------------------------------*/
...
/* interpolate antenna phase center variation --------------------------------*/
static double interpvar0(unsigned char sat, double ang, const double *var,
                         int bsat) {
  int i, limit = 18;
  double a;
  unsigned char sys;
...
extern void antmodel_s(unsigned char sat, const pcv_t *pcv, double nadir,
  ...
  for (i = 0; i < NFREQ; i++) {
    ...
    if (sys == SYS_GPS) {
      ...
      if (i == 2) {
        dant[i] =
            interpvar0(sat, nadir * R2D * 5.0, pcv->var[1 + 2 * NFREQ], 1);
      }
    } else if (sys == SYS_GAL) {
      dant[i] = interpvar0(sat, nadir * R2D * 5.0, pcv->var[i + 3 * NFREQ], 1);
      if (i == 2) {
        dant[i] =
            interpvar0(sat, nadir * R2D * 5.0, pcv->var[1 + 3 * NFREQ], 1);
      }
    } else if (sys == SYS_QZS) {
      dant[i] = interpvar0(sat, nadir * R2D * 5.0, pcv->var[i + 4 * NFREQ], 1);
      if (i == 2) {
        dant[i] =
            interpvar0(sat, nadir * R2D * 5.0, pcv->var[1 + 4 * NFREQ], 1);
      }
    }
  }
}
