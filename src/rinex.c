...
static void set_index(
    double ver, unsigned char sys, const char* opt, char tobs[MAXOBSTYPE][4], sigind_t* ind
)
{
    const char* p;
    char        str[8], *optstr = "";
    ...
    for (i = n = 0; *tobs[i]; i++, n++)
    {
        ...
        if (sys == SYS_GAL)
        {
            if (ind->code[i] == CODE_L5Q)
            {
                ind->frq[i] = 3; /* E5A */
            }
            else if (ind->code[i] == CODE_L7Q)
            {
                ind->frq[i] = 2; /* E5B */
            }
        }
...
static int trim_obs(obs_t* obs, gtime_t ts, gtime_t te, int navsys)
{
    ...
    for (i = 0; i < obs->n; i++)
    {
        ...
        if ((ts.time != 0) && (timediff(ts, obs->data[i].time) > TMAX))
        {
            continue;
        }
        if ((te.time != 0) && (timediff(obs->data[i].time, te) > TMAX))
        {
            continue;
        }
...
)
{
    ...
    if (!(data = (obsd_t*)calloc(sizeof(obsd_t), MAXOBS)))
    {
        return 0;
    }

    /* read rinex obs data body */
    while ((n = readrnxobsb(fp, opt, ver, tsys, tobs, &flag, data, sta, lineCount, maxLine)) >= 0 &&
           stat >= 0)
    {
        for (i = 0; i < n; i++)
        {
...
/* decode ephemeris ----------------------------------------------------------*/
static int decode_eph(double ver, unsigned char sat, gtime_t toc, const double* data, eph_t* eph)
{
    eph_t         eph0 = {0};
    unsigned char sys;
...
static int decode_geph(double ver, unsigned char sat, gtime_t toc, double* data, geph_t* geph)
{
    geph_t  geph0 = {0};
    gtime_t tof;
    double  tow, tod;
...
static int readrnxnavb(
    FILE* fp, const char* opt, double ver, unsigned char sys, int* type, eph_t* eph, geph_t* geph,
...
extern int outprcopts(unsigned char* buff, const prcopt_t* opt)
{
    ...
    const char*         s1[] = {"single", "dgps",          "kinematic",  "static",    "moving-base",
                                "fixed",  "ppp-kinematic", "ppp-static", "ppp-fixed", ""};
    const char*         s2[] = {
        "L1", "L1+L2", "L1+L2+L5", "L1+L2+L5+L6", "L1+L2+L5+L6+L7", "L1+L2+L5+L6+L7+L8", ""
    };
    const char* s3[] = {"forward", "backward", "combined"};
    const char* s4[] = {"off", "broadcast", "sbas",    "iono-free", "estimation", "ionex tec",
                        "qzs", "lex",       "vtec_sf", "vtec_ef",   "gtec",       ""};
...
static void outrpos(FILE* fp, const double* r, const solopt_t* opt)
{
    ...
    if (opt->posf == SOLF_LLH || opt->posf == SOLF_ENU)
    {
        ...
        if (opt->degf)
        {
            ...
            fprintf(
                fp, "%3.0f%s%02.0f%s%08.5f%s%4.0f%s%02.0f%s%08.5f%s%10.4f", dms1[0], sep, dms1[1],
...
static int outsolheads(unsigned char* buff, const solopt_t* opt)
{
    ...
    { /* lat/lon/hgt */
        ...
        {
            p += sprintf(
                p, "%16s%s%16s%s%10s%s%3s%s%3s%s%8s%s%8s%s%8s%s%8s%s%8s%s%8s%s%6s%s%6s",
        ...
        {
            p += sprintf(
                p, "%14s%s%14s%s%10s%s%3s%s%3s%s%8s%s%8s%s%8s%s%8s%s%8s%s%8s%s%6s%s%6s",
                "latitude(deg)", sep, "longitude(deg)", sep, "height(m)", sep, "Q", sep, "ns", sep,
                "sdn(m)", sep, "sde(m)", sep, "sdu(m)", sep, "sdne(m)", sep, "sdeu(m)", sep,
        ...
        {
            p += sprintf(
                p, "%s%10s%s%10s%s%10s%s%9s%s%8s%s%8s%s%8s%s%8s%s%8s", sep, "vn(m/s)", sep,
                "ve(m/s)", sep, "vu(m/s)", sep, "sdvn", sep, "sdve", sep, "sdvu", sep, "sdvne", sep,
                "sdveu", sep, "sdvun"
            );
        }
    }
    ...
    { /* x/y/z-ecef */
        p += sprintf(
            p, "%14s%s%14s%s%14s%s%3s%s%3s%s%8s%s%8s%s%8s%s%8s%s%8s%s%8s%s%6s%s%6s", "x-ecef(m)",
            sep, "y-ecef(m)", sep, "z-ecef(m)", sep, "Q", sep, "ns", sep, "sdx(m)", sep, "sdy(m)",
            sep, "sdz(m)", sep, "sdxy(m)", sep, "sdyz(m)", sep, "sdzx(m)", sep, "age(s)", sep,
            "ratio"
        );
        ...
        {
            p += sprintf(
                p, "%s%10s%s%10s%s%10s%s%9s%s%8s%s%8s%s%8s%s%8s%s%8s", sep, "vx(m/s)", sep,
                "vy(m/s)", sep, "vz(m/s)", sep, "sdvx", sep, "sdvy", sep, "sdvz", sep, "sdvxy", sep,
                "sdvyz", sep, "sdvzx"
            );
        }
    }
    ...
    { /* e/n/u-baseline */
        p += sprintf(
            p, "%14s%s%14s%s%14s%s%3s%s%3s%s%8s%s%8s%s%8s%s%8s%s%8s%s%8s%s%6s%s%6s",
            "e-baseline(m)", sep, "n-baseline(m)", sep, "u-baseline(m)", sep, "Q", sep, "ns", sep,
            "sde(m)", sep, "sdn(m)", sep, "sdu(m)", sep, "sden(m)", sep, "sdnu(m)", sep, "sdue(m)",
            sep, "age(s)", sep, "ratio"
        );
    }
    p += sprintf(p, "\n");
...
/* read rinex file -----------------------------------------------------------*/
static int readrnxfp(
    FILE* fp, gtime_t ts, gtime_t te, double tint, const char* opt, int flag, int index, char* type,
...
)
{
    ...
    /* read rinex header */
    if (!readrnxh(fp, &ver, type, &sys, &tsys, tobs, nav, sta, &num, 0))
    {
        return 0;
    }
...
static int readrnxfile(
    const char* file, gtime_t ts, gtime_t te, double tint, const char* opt, int flag, int index,
