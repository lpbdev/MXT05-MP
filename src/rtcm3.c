...
static int decode_type1020(rtcm_t* rtcm)
{
    ...
    if (fabs(timediff(geph.toe, g_nav.geph[prn-1].toe)) < 1.0 && geph.svh == g_nav.geph[prn-1].svh) {
        //pthread_mutex_unlock(&mutex_nav);
        return 0; /* unchanged */
    }
...
static int decode_type1045(rtcm_t* rtcm)
{
    ...
    if (rtcm->outtype) {
        msg = rtcm->msgtype + strlen(rtcm->msgtype);
        sprintf(msg, " prn=%2d iode=%3d week=%d toe=%6.0f toc=%6.0f hs=%d dvs=%d rsv=%d",
            prn, eph.iode, week, eph.toes, toc, e5a_hs, e5a_dvs, rsv);
    }
    sat = satno(sys, prn);
    if (!sat) {
...
static int decode_type63(rtcm_t* rtcm)
{
   ...
   // printf("sat=%d,week=%d,eph week=%d,rtcm week=%d\n",sat,week,eph.week,rtcm->week);
    eph.ttr = rtcm->time;
    eph.A = sqrtA * sqrtA;
    if (timediff(eph.toe, g_nav.eph[sat - 1].toe) == 0.0 && eph.iode == g_nav.eph[sat - 1].iode && eph.iodc == g_nav.eph[sat - 1].iodc) {
        return 0; /* unchanged */
    }
    g_nav.eph[sat - 1] = eph;
    rtcm->ephsat = sat;
...
static int decodeMsmHead(rtcm_t* rtcm, int sys, int* sync, int* iod,
    msm_h_t* h, int* hsize)
{
    msm_h_t h0 = { 0 };
    double tow, tod;
...
static double satwavelen(unsigned char sat, int frq, const nav_t* nav)
{
    ...
    if (sys == SYS_GLO) {
        ...
        else if (frq == 2) return CLIGHT / FREQB2a_CMP; /* B3 */
        else if (frq == 3) return CLIGHT / FREQB2b_CMP; /* B1C */
        else if (frq == 4) return CLIGHT / FREQB1C_CMP; /* B2a */
        else if (frq == 5) return CLIGHT / FREQ3_CMP ; /* B2b */
    }
    else if (sys == SYS_GAL) {
...
static void saveMsmObs(rtcm_t* rtcm, int sys, msm_h_t* h, const double* r,
    ...
    const int* ex, const int* half)
{
    ...
    //            }
    //            if (sys == SYS_GPS) {
    //                //printf("1sat=%d gps ind[k]=%d freq=%d %lf %lf\n",sat,ind[k],freq[k]-1,g_gpsLam[ind[k]],wl);
    ...
    //            }
    //            else if (sys == SYS_GAL) {
    //                //printf("sat=%d gal ind[k]=%d freq=%d %lf %lf\n",sat,ind[k],freq[k]-1,g_galLam[ind[k]],wl);
    ...
    //                trace(4, "21 p=%lf r=%lf pr=%lf\n", rtcm->obs.data[index].P[ind[k]], r[i], pr[i]);
    //            }
    ...
    for (i = 0; i < h->nsig; i++) {
        ...
        if (freq[i] == 5) freq[i] = 2; /* B2 */
            else if (freq[i] == 4) freq[i] = 6; /* B3 */
            else if (freq[i] == 2) freq[i] = 5; /* B1c */
            else if (freq[i] == 3) freq[i] = 3; /* B2a*/
            else if (freq[i] == 6) freq[i] = 4; /* B2b */
            //printf("i=%d\n", h->sigs[i]);  //2 14 23 25
        }
        ...
        for (k = 0; k < h->nsig; k++) {
            ...
            if (sat && index >= 0 && ind[k] >= 0) {
                ...
                if (sys == SYS_GPS || sys == SYS_QZS) {
                    //printf("1sat=%d gps ind[k]=%d freq=%d %lf %lf\n",sat,ind[k],freq[k]-1,g_gpsLam[ind[k]],wl);
                    if (g_gpsLam[ind[k]] == 0.0)    g_gpsLam[ind[k]] = wl;
                    else    wl = g_gpsLam[ind[k]];
                }
                else if (sys == SYS_GAL) {
                    //printf("sat=%d gal ind[k]=%d freq=%d %lf %lf\n",sat,ind[k],freq[k]-1,g_galLam[ind[k]],wl);
                    if (g_galLam[ind[k]] == 0.0)    g_galLam[ind[k]] = wl;
                    else    wl = g_galLam[ind[k]];
                }
                else if (sys == SYS_BDS) {
                    if (g_bdsLam[ind[k]] == 0.0)    g_bdsLam[ind[k]] = wl;
                    else    wl = g_bdsLam[ind[k]];
                }
...
static int decode_msm4(rtcm_t* rtcm, int sys)
{
    ...
    /* decode msm header */
    if ((ncell = decodeMsmHead(rtcm, sys, &sync, &iod, &h, &i)) < 0) return -1;
...
static int decode_msm5(rtcm_t* rtcm, int sys)
{
    ...
    if (i + h.nsat * 36 + ncell * 63 > rtcm->len * 8) {
        trace(0x01, "rtcm3 %d length error: nsat=%d ncell=%d len=%d\n", type, h.nsat,
            ncell, rtcm->len);
        return -1;
    }
    for (j = 0; j < h.nsat; j++) {
...
extern int decode_rtcm3(rtcm_t* rtcm)
{
    ...
    case 1042: ret = decode_type63(rtcm); break; /* beidou ephemeris (tentative mt) */
    ...
    //if(type!=1019 && type != 1019&& type != 1020 && type != 1042 && type != 1046 && type != 1044 & type != 1005)
    //printf("decode_rtcm3:rcv=%d len=%3d type=%d time=%d ret=%d\n", rtcm->rcv, rtcm->len, type,rtcm->time.time,ret);
    return ret;
}
