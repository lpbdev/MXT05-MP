...
// static double varerr_gamit(unsigned char sat, unsigned char sys, double el,
// double bl, int f, const prcopt_t* opt)
// {
...
// static double varrL(const obsd_t* obs, double el, double bl, int f, const
// prcopt_t* opt)
// {
//     double a, b, c = 0 * bl / 1E4;
//     double sinel, var;
...
static int selsat(const obsd_t *obs, int nu, int nr, int *sat, int *iu,
                  int *ir) {
  int i, j, k = 0, tmp;
  for (i = 0; i < nu; i++) {
    tmp = (obs[i].sat);
...
static int rescode(rtk_t *rtk, int post, const obsd_t *obs, int n, int nu,
                   const int *svh, double *rs, double *dts, double *H,
                   double *v, double *var_sat, double *var, double *x,
                   unsigned char *exc, double *lsqraim, const prcopt_t *opt,
                   unsigned char ns, unsigned char *sat, unsigned char *iu,
                   unsigned char *ir, double *stdv, unsigned char *nvSatMask) {
  ...
  for (m = 0; m < NSYS; m++) {
    for (f = 0; f < nf; f++) {
      ...
      if (rtk->ssat[sat[j] - 1].vs != 1)
        continue;
      if (rtk->ssat[sat[i] - 1].fix_amb[f] == 9999.9)
        continue;
      if ((rtk->ssat[sat[i] - 1].slip[f] & 1) ||
          (rtk->ssat[sat[i] - 1].slip[f] & 2)) {
        rtk->ssat[sat[i] - 1].fix_amb[f] = 9999.9;
        continue;
      }
      satsys(sat[i], &prni);

      if ((rb1 = geodist(rs + index1 * 6, rr1, eb1)) <= 0)
        continue; // 参考星至接收机几何距离
      if ((rb2 = geodist(rs + index2 * 6, rr2, eb2)) <= 0)
        continue; // 参考星至基准站几何距离
      ...
      if (rtk->opt.mode != 4) {
        ...
        rb1 += tropmapf(obs[0].time, pos1, rtk->ssat[sat1 - 1].azel[0], NULL) *
               zhd;

        zhd = tropmodel(obs[0].time, pos2, zazel, 0.0);
        rb2 += tropmapf(obs[0].time, pos2, rtk->ssat[sat1 - 1].azel[1], NULL) *
               zhd;
      }

      for (j = 0; j < ns; j++) {
        // vsat[j+f*ns] = 0;azel[j * 2] = azel[j * 2 + 1] = resp[j+ f * ns] =
        // 0.0;
        ...
        if (!test_sys(sys, m))
          continue;
        if (obs[iu[i]].L[f] == 0.0 || obs[ir[i]].L[f] == 0.0 ||
            obs[iu[j]].L[f] == 0.0 || obs[ir[j]].L[f] == 0.0)
          continue;
        for (k = 0; k < nu; k++) {
          if (obs[k].sat == sat[j]) {
            index1 = k;
        ...
        if (rtk->ssat[sat[j] - 1].fix_amb[f] == 9999.9)
          continue;
        if ((r1 = geodist(rs + index1 * 6, rr1, e1)) <= 0)
          continue; // 非参考星至接收机几何距离
        if ((r2 = geodist(rs + index2 * 6, rr2, e2)) <= 0)
          continue; // 非参考星至基准站几何距离
        ...
        if (rtk->opt.mode != 4) {
          zhd = tropmodel(obs[0].time, pos1, zazel, 0.0);
          r1 += tropmapf(obs[0].time, pos1, rtk->ssat[sat2 - 1].azel[0], NULL) *
                zhd;

          zhd = tropmodel(obs[0].time, pos2, zazel, 0.0);
          r2 += tropmapf(obs[0].time, pos2, rtk->ssat[sat2 - 1].azel[1], NULL) *
                zhd;
        }
        ...
        for (k = 0; k < rtk->nsLsq[f]; k++) {
          if (sat[j] == rtk->satLsq[f][k]) {
            ...
            amb = rtk->ssat[sat[i] - 1].fix_amb[f] * lami[f] -
                  rtk->ssat[sat[j] - 1].fix_amb[f] * lamj[f];
        ...
        v[nv] = Lb1 - Lb2 - (L1 - L2) - amb -
                0 * (rtk->ssat[sat[j] - 1].ddion - rtk->ssat[sat[j] - 1].ddtrp);
        ...
        if (rtk->mpflag == 1 && f == 0) {
          ...
          trace(2, "$EPH, %s,%s, %.2f,%d\n", tstr, id, epochlen,
                (int)(epochlen / rtk->opt.timeInterval));
          ctime.time = ctime.time - ((int)(epochlen / rtk->opt.timeInterval)) *
                                        rtk->opt.timeInterval;
          int offset2 =
              calOffset(obs[j].sat, ctime, (int)rtk->opt.timeInterval);
        ...
        if (post == 0) {
          trace(2, "RES1, %s, %s, %d, %.4f, %d,%f,%d,%s, %d,%f, %f\n", tstr, id,
                f, v[nv], obs[j].sat, amb, obs[i].sat, id2, base_sat,
                rtk->ssat[sat[i] - 1].fix_amb[f],
                rtk->ssat[sat[j] - 1].fix_amb[f]);

          trace(2, "BASEPRN, %s, %d, %d \n", tstr, base_sat,
                rtk->base_prn_pre[m][f]);
          rtk->base_prn_pre[m][f] = base_sat;
        }

        // if (post == 0)
        //     printf("*****f=%d v=%10.2f sat1=%3d sat2=%3d N1=%10.2f N2=%10.2f
        //     N1-N2=%10.2f\n",
        //         f,v[nv],sat[j], sat[i], rtk->ssat[sat[j] - 1].fix_amb[f],
        //         rtk->ssat[sat[i] - 1].fix_amb[f], rtk->ssat[sat[j] -
        //         1].fix_amb[f] - rtk->ssat[sat[i] - 1].fix_amb[f]);
        ...
        // Ri = varerr_gamit(sat[i], sys, rtk->ssat[sat1 - 1].azel[0][1],
        // bl,f,opt); Rj = varerr_gamit(sat[j], sys, rtk->ssat[sat2 -
        // 1].azel[0][1], bl,f,opt);
        ...
        trace(2, "RiRj: f,%d,%10.6f, %10.6f, %10.6f, %10.6f\n", Ri, Rj,
              rtk->ssat[sat1 - 1].Ri, rtk->ssat[sat2 - 1].Ri),
...
extern int rtkLsq(rtk_t *rtk, const obsd_t *obs, int n, int nu, const int *svh,
  ...
  unsigned char i, j, k, f, info, sys, prn, nsobs = 0, nbase = 0, post = 0,
                                            nv = 0, respMaxSat = 0;
  int nf = rtk->opt.nf;
  ...
  for (i = 0; i < 3; i++)
    x[i] = rtk->sol.rr[i];
  for (i = 0; i < 10; i++) {
    nv = rescode(rtk, post, obs, n, nu, svh, rs, dts, H, v, var_sat, var, x,
                 exc, lsqraim, &rtk->opt, ns, sat, iu, ir, stdv, nvSatMask);
    ...
    if (norm(dx, 3) < 1E-3) {
      ...
      rtk->sol.ns[1] = nsobs + nbase;
      for (j = 0; j < 3; j++)
        rtk->sol.rr[j] = j < 3 ? x[j] : 0.0;
      for (j = 0; j < 3; j++)
        rtk->sol.qr[j] = (float)Q[j + j * 3];
