...
static int rescode(
    rtk_t* rtk, int post, const obsd_t* obs, int n, int nu, const int* svh, double* rs, double* dts,
    double* H, double* v, double* var_sat, double* var, double* x, unsigned char* exc,
    double* lsqraim, const prcopt_t* opt, unsigned char ns, unsigned char* sat, unsigned char* iu,
    ...
    for (m = 0; m < NSYS; m++)
    {
        for (f = 0; f < nf; f++)
        {
            ...
            for (j = 0; j < ns; j++)
            {
                ...
                if (obs[iu[i]].L[f] == 0.0 || obs[ir[i]].L[f] == 0.0 || obs[iu[j]].L[f] == 0.0 ||
                    obs[ir[j]].L[f] == 0.0)
                {
                    continue;
                }
                ...
                if (rtk->mpflag == 1 && f == 0)
                {
                    ...
                    ctime.time = ctime.time -
                                 ((int)(epochlen / rtk->opt.timeInterval)) * rtk->opt.timeInterval;
                ...
                {
                    trace(
                        2, "RES1, %s, %s, %d, %.4f, %d,%f,%d,%s, %d,%f, %f\n", tstr, id, f, v[nv],
                        obs[j].sat, amb, obs[i].sat, id2, base_sat,
                ...
                // var[nv] = Ri + Rj;
                trace(
                    2, "RiRj: f,%d,%10.6f, %10.6f, %10.6f, %10.6f\n", Ri, Rj,
...
{
    unsigned char i, j, k, f, info, sys, prn, nsobs = 0, nbase = 0, post = 0, nv = 0,
                                              respMaxSat = 0;
    int            nf                                    = rtk->opt.nf;
    ...
    {
        nv = rescode(
            rtk, post, obs, n, nu, svh, rs, dts, H, v, var_sat, var, x, exc, lsqraim, &rtk->opt, ns,
            sat, iu, ir, stdv, nvSatMask
        );
