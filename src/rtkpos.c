    ...
    const unsigned char* ir, int ns, double* standard_v, unsigned char* robust, int* vflg)
{
    ...
    for (f = opt->mode > PMODE_DGPS ? 0 : nf; f < nf * 2; f++) {
        ...
        for (m = 0; m < NSYS; m++) {
            ...
            for (j = 0; j < ns; j++) {
                ...
                if (f < nf && post ==0)
                   trace(2,"f=%d nv=%3d sys=%3d prn=%3d x[%3d]=%7.2f x[%3d]=%7.2f dbias=%7.2f v[%2d]=%10.4lf\n",
    ...
    trace(2, "replos, m, %d, nf, %d, ns,%d,opt->nf, %d, nv,%d,f,%d,NY,%d\n", m, nf, ns, opt->nf,nv,f,NY);
...
extern int relpos(rtk_t* rtk, obsd_t* obs, unsigned char nu, unsigned char nr,
    ...
    trace(2, "kalman xyz after while , %s, %14.4f, %14.4f, %14.4f, %d,%d, %14.4f, %14.4f, %14.4f,\n",
        ts, rtk->x [0],rtk->x[1],rtk->x[2], rtk->sol.ns[1],ilterCout,rtk->rb[0],rtk->rb[1],rtk->rb[2]);
    ...
    trace(2, "kalman xyz rtkAR , %s, %14.4f, %14.4f, %14.4f, %d,%d, %14.4f, %14.4f, %14.4f, %14.4f, %14.4f, %14.4f,\n",
          ts, rtk->x[0], rtk->x[1], rtk->x[2], rtk->sol.ns[1], ilterCout, rtk->rb[0], rtk->rb[1], rtk->rb[2],
