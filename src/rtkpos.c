...
static double varerr_gamit(int sat, int sys, double el, double bl, double dt,
...
static void initxp(rtk_t *rtk, double xi, double var, int i) {
  ...
  rtk->xp[i] = xi;
  for (j = 0; j < rtk->nx; j++) {
    rtk->Pp[i + j * rtk->nx] = rtk->Pp[j + i * rtk->nx] =
        ((i == j) ? var : 0.0);
  }
}
...
static double gfobs(const obsd_t *obs, int index, int i, int j,
                    const double *lam) {
  double pi = sdobs(obs, i, j, 0) * lam[0],
         pj = sdobs(obs, i, j, index) * lam[index];
...
static void udion(rtk_t *rtk, const obsd_t *obs, const unsigned char *iu,
                  const unsigned char *ir, double tt, double bl,
                  const unsigned char *sat, unsigned char ns) {
  gtime_t time;
  double el, fact, ep[6];
  ...
  if (0) {
    ...
    for (i = 0; i < ns; i++) {
      ...
      if (rtk->x[j] == 0.0 || (fabs(timediff(rtk->ssat[sat[i] - 1].ddionTime,
                                             rtk->sol.time)) > 60)) {
        // if (rtk->x[j] == 0.0) {
...
static void detslp_dop(rtk_t *rtk, const obsd_t *obs, int i, int rcv) {
  ...
  for (f = 0; f < rtk->opt.nf; f++) {
    ...
    trace(0x08,
          "detslp_dop detected (sat=%2d rcv=%d L%d=%.3f %.3f thres=%.3f)\n",
          sat, rcv, f + 1, dph, dpt, thres);
  }
}

static void udbias(rtk_t *rtk, double tt, const obsd_t *obs,
                   const unsigned char *sat, const unsigned char *iu,
...
static void udstate(rtk_t *rtk, const obsd_t *obs, const unsigned char *sat,
                    const unsigned char *iu, const unsigned char *ir,
                    unsigned char ns, double *r) {
...
static void zdres_sat(int base, double r, const obsd_t *obs, const double *azel,
...
static int zdres(rtk_t *rtk, int base, const obsd_t *obs, int n,
  ...
  for (f = 0, k = na; f < nf; f++) {
    for (m = 0; m < NSYS; m++) {
      ...
      for (j = k; j < k + rtk->nx; j++) {
        ...
        if (sati == 0 || f != frqi)
          continue;
        if (i == j || rtk->x[j] == 0.0 || !test_sys(sys, m) ||
            !rtk->ssat[sati - 1].vsat[f])
          continue;
...
static void holdamb(rtk_t *rtk, const double *xa) {
  ...
  int i, n, m, f, info, indexb, index[MAXSAT], nb = rtk->na, nv = 0,
                                               nf = rtk->opt.nf;
  unsigned char sati, frqi, sys;
    ...
    for (m = 0; m < NSYS; m++) {
      ...
      for (i = 0; i < n; i++) {
        if (index[i] == indexb)
          continue;
        rtk->v[nv] =
            (xa[indexb] - xa[index[i]]) - (rtk->x[indexb] - rtk->x[index[i]]);
  ...
  if (nv > 0) {
    ...
    if ((info = filter(rtk, rtk->xp, rtk->Pp, rtk->H, rtk->v, rtk->R, rtk->nx,
                       nv, rtk->x, rtk->P))) {
...
extern int resamb_LAMBDA(rtk_t *rtk, int lcopt) {
  ...
  double *D, *DP, *y, *Qy, *b, *db, *Qb, *Qab, *QQ, *bias, *Pp, s[2], thresar,
      ratio;
  // char buff[10240] = { 0 };
  // char *p = buff;
...
static int scanFloatState(rtk_t *rtk, unsigned char *floatSat,
                          unsigned char *floatSatFrq) {
  unsigned char i, j, sati, frqi, satj, frqj, flag,
      na = rtk->np + rtk->nt + rtk->ni;
  ...
  for (i = na; i < rtk->nx; i++) {
    ...
    if (flag == 0) {
      trace(0x02, "AR failed by sat=%3d f=%2d el=%7.2f %7.2f\n", sati, frqi,
            rtk->ssat[sati - 1].azel[0][1] * R2D,
            rtk->ssat[sati - 1].azel[1][1] * R2D);
...
static void rtkAR(rtk_t *rtk, obsd_t *obs, unsigned char *sat, int n) {
  int i, j, k, m, f, sati, frqi, lambdaReturn, nf = rtk->opt.nf,
                                               na = rtk->np + rtk->nt + rtk->ni;
      ...
      for (f = 0; f < NFREQ; f++) {
        ...
        for (j = 0; j < ns; j++) {
          ...
          if ((rtk->ssat[sat[j] - 1].slip[f] & 1) ||
              (rtk->ssat[sat[j] - 1].slip[f] & 2)) {
            rtk->ssat[sat[j] - 1].fix_amb[f] = 9999.9;
            continue;
          }
          rtk->ssat[sat[j] - 1].ddAmb[f] = rtk->ssat[sat[i] - 1].fix_amb[f] -
                                           rtk->ssat[sat[j] - 1].fix_amb[f];
        }
      }
    }
    int m, f;
    ...
    for (m = 0; m < NSYS; m++) {
      ...
      for (j = 0; j < ns; j++) {
        ...
        if (rtk->ssat[sat[j] - 1].fix_amb[0] == 9999.9)
          continue;
        if (rtk->ssat[sat[j] - 1].ph[0][0] == 0.0 ||
            rtk->ssat[sat[j] - 1].ph[1][0] == 0.0)
          continue;
        index1 = 0;
        index2 = 0;
        for (f = 1; f < NFREQ; f++) {
        ...
        ddL1 = (dLb1 - dL1) * lam[0];
        dL2 = rtk->ssat[sat[j] - 1].ph[0][index2] -
              rtk->ssat[sat[j] - 1].ph[1][index2];
        dLb2 = rtk->ssat[sat[i] - 1].ph[0][index2] -
               rtk->ssat[sat[i] - 1].ph[1][index2];
        ...
        ddtrp = (SQR(f1) / (SQR(f1) - SQR(f2))) * (ddL1 - lam[0] * ddN1) -
                (SQR(f2) / (SQR(f1) - SQR(f2))) * (ddL2 - lam[index2] * ddN2) -
                ddr;
...
static int check_res(rtk_t *rtk, const obsd_t *obs, const double *x,
                     unsigned char *sat, double *y, unsigned char *iu,
                     unsigned char *ir, int ns, double *azel) {
  unsigned char i, j, k, nv = 0, m, index1, index2, sati, satj, frqi, frqj,
                         flag, f, sysi, sysj, prni, prnj, nf = rtk->opt.nf;
  ...
  if (rtk->opt.tropopt == TROPOPT_EST) {
    for (i = 0; i < ns; i++) {
      tropu[i] =
          prectrop(rtk, posu, 0, azel + iu[i] * 2, &rtk->opt, x, dtdxu + i);
      tropr[i] =
          prectrop(rtk, posr, 1, azel + ir[i] * 2, &rtk->opt, x, dtdxr + i);
    }
  }
    ...
    for (f = 0; f < nf; f++) {
      ...
      for (j = 0; j < ns && flag == 1; j++) {
        ...
        if (rtk->opt.tropopt == TROPOPT_EST) {
          ...
          rtk->ssat[sat[j] - 1].ddtrp =
              (tropu[i] - tropu[j]) - (tropr[i] - tropr[j]);
        }
        if (sysj == SYS_GLO) {
        ...
        if (rtk->opt.ionoopt == IONOOPT_EST) {
          rtk->v[nv] -=
              C1 * x[rtk->np + rtk->nt + i] - C2 * x[rtk->np + rtk->nt + j];
        }
        ...
        trace(3, "nxRecord, check_res(), index1,%d, index2, %d, %d,%d\n",
              index1, index2, rtk->ssat[sat[i] - 1].xIndex[f],
...
static int ddres(int post, rtk_t *rtk, const obsd_t *obs, double dt,
                 const double *x, const double *P, const unsigned char *sat,
                 double *y, double *e, double *azel, const unsigned char *iu,
                 const unsigned char *ir, int ns, double *standard_v,
                 unsigned char *robust, int *vflg) {
  ...
  double *tropu, *tropr;
  double *dtdxu, *dtdxr;
  int i, j, k, sati, frqi, satj, frqj, index1, index2, m, f,
      nv = 0, nb[NFREQ * 4 * 2 + 2] = {0}, b = 0, sysi, sysj, nf = rtk->opt.nf;
  int vmax_sat = 1, stat = 1, vmaxj = 0, type, freq,
      na = rtk->np + rtk->nt + rtk->ni;
      ...
      for (j = 0; j < ns; j++) {
          ...
          if (rtk->opt.bl <= BSLTHRESHOLD && baseXyzError == 0) {
            if (post == 0 && f < nf && rtk->sol.fixxyz[0] != 0.0 &&
              ...
              rtk->ssat[sat[j] - 1].resc[f] =
                  rtk->ssat[sat[i] - 1].fbias[f] -
                  rtk->ssat[sat[j] - 1].fbias[f] -
                  ROUND(rtk->ssat[sat[i] - 1].fbias[f] -
                        rtk->ssat[sat[j] - 1].fbias[f]);
              ...
              if (fabs(rtk->ssat[sat[j] - 1].resc[f]) > 0.35 &&
                ...
                trace(0x04,
                      ...
                      obs[iu[i]].SNR[f % nf] / 4.0,
                      obs[iu[j]].SNR[f % nf] / 4.0);
                if (f == 0) {
                  for (k = na; k < rtk->nx; k++) {
                    if (rtk->nxRecordSat[k - na] == sat[j]) {
        ...
        if (post == 0 && fabs(rtk->v[nv]) > 50.0) {
          ...
          trace(0x04, "outlier rejected (sat=%3d-%3d %s%d v=%.3f)\n", sat[i],
                sat[j], f < nf ? "L" : "P", f % nf + 1, rtk->v[nv]);
          for (k = na; k < rtk->nx; k++) {
            if (rtk->nxRecordSat[k - na] == sat[j]) {
              initx(rtk, 0, 0, k);
            }
      ...
      if (standard_v[j] > K0) {
        ...
        if (vmax <= K0) {
          ...
          for (i = rtk->np + rtk->nt + rtk->ni; i < rtk->nx; i++) {
            if (rtk->nxRecordSat[i - (rtk->np + rtk->nt + rtk->ni)] ==
                vmax_sat) {
              initx(rtk, 0, 0, i);
            }
          }
        ...
        for (j = 0; j < nv; j++) {
          if (standard_v[j] > K0) {
            ...
            if (vmax <= K0) {
              ...
              rtk->Rj[j] = SQR((1 / p2));
              trace(8,
                    "post=%d robust1 sat=%d %s%d v=%.2f std=%.3f stdv=%.2f\n",
                    post, vmax_sat, stype, freq + 1, rtk->v[j],
                    sqrt(rtk->Ri[j]), vmax);
            } else {
              rtk->Rj[j] = 10E8;
              trace(8,
                    "post=%d robust2 sat=%d %s%d v=%.2f std=%.3f stdv=%.2f\n",
                    post, vmax_sat, stype, freq + 1, rtk->v[j],
                    sqrt(rtk->Ri[j]), vmax);
            }
          }
...
static int sat_ts_update(const obsd_t *obsd, satts_t *satts, double tint) {
  ...
  for (i = 0; i < NFREQ + NEXOBS; i++) {
    ...
    if (satts->vfcnt[i] < 3)
      continue;
    satts->L3d[i] =
        satts->data[0].L[i] + satts->data[2].L[i] - 2 * satts->data[1].L[i];
    satts->P3d[i] =
        satts->data[0].P[i] + satts->data[2].P[i] - 2 * satts->data[1].P[i];
    satts->D3d[i] =
        satts->data[0].D[i] + satts->data[2].D[i] - 2 * satts->data[1].D[i];
    satts->S3d[i] =
        satts->data[0].SNR[i] - satts->data[1].SNR[i]; // SNR 只用两点差分
  }
...
extern int relpos(rtk_t *rtk, obsd_t *obs, unsigned char nu, unsigned char nr,
  ...
  if (zdres(rtk, 1, obs + nu, nr, rs + nu * 6, dts + nu * 2, svh + nu, rtk->rb,
            &rtk->opt, 1, y + nu * nf * 2, e + nu * 3, azel + nu * 2, r + nu)) {
  ...
  if (zdres(rtk, 0, obs, nu, rs, dts, svh, rtk->sol.rr, &rtk->opt, 0, y, e,
            azel, r)) {
    free(y);
    free(e);
    free(azel);
  ...
  while (1) {
    ...
    if (zdres(rtk, 0, obs, nu, rs, dts, svh, rtk->x, &rtk->opt, 0, y, e, azel,
              r)) {
    ...
    nv = ddres(0, rtk, obs, dt, rtk->x, rtk->P, sat, y, e, azel, iu, ir, ns,
               standard_v, robust, vflg);
    if (nv == -2)
      continue;
    if (nv < 4) {
      trace(0x02, "nv=%d\n", nv);
    ...
    if (rtk->opt.mode == PMODE_DGPS) {
      if ((info = filter(rtk, rtk->x, rtk->P, rtk->H, rtk->v, rtk->R, 3, nv,
                         rtk->xp, rtk->Pp))) {
        trace(0x02, "filter error (info=%d)\n", info);
        rtk->sol.stat = SOLQ_NONE;
        break;
      ...
      if ((info = filter(rtk, rtk->x, rtk->P, rtk->H, rtk->v, rtk->R, rtk->nx,
                         nv, rtk->xp, rtk->Pp))) {
        trace(0x02, "filter error (info=%d)\n", info);
        rtk->sol.stat = SOLQ_NONE;
        break;
    ...
    if (zdres(rtk, 0, obs, nu, rs, dts, svh, rtk->xp, &rtk->opt, 0, y, e, azel,
              r)) {
      rtk->sol.stat = SOLQ_NONE;
      trace(0x02, "rover initial position error\n");
      break;
    }
    nv = ddres(1, rtk, obs, dt, rtk->xp, rtk->Pp, sat, y, e, azel, iu, ir, ns,
               standard_v, robust, vflg);
  ...
  trace(2,
        ...
        "%14.4f, %14.4f,\n",
        ts, rtk->x[0], rtk->x[1], rtk->x[2], rtk->sol.ns[1], ilterCout,
  ...
  trace(2,
        ...
        ts, rtk->x[0], rtk->x[1], rtk->x[2], rtk->sol.ns[1], ilterCout,
        rtk->rb[0], rtk->rb[1], rtk->rb[2], rtk->sol.rr[0], rtk->sol.rr[1],
        rtk->sol.rr[2]);
...
extern int getSatNum(rtk_t *rtk, const obsd_t *obs, unsigned char nu,
                     unsigned char nr, const prcopt_t *opt, unsigned char *sat,
                     unsigned char *iu, unsigned char *ir, double elMin,
                     int snrMin) {
      ...
      if (obs[i].sat == obs[j].sat) { /* elevation at base station */
        ...
        for (f = 0; f < nf; f++) {
          if (obs[i].P[f] != 0.0 && obs[j].P[f] != 0.0 && obs[i].L[f] != 0.0 &&
              obs[j].L[f] != 0.0 && obs[i].SNR[f] > snrMin &&
              obs[j].SNR[f] > snrMin) {
            flag = 1;
            break;
          }
...
static double gfmeas0(rtk_t *rtk, unsigned char sat, unsigned char rcv,
                      double *lam, int index1, int index2) {
  int i = index1, j = index2;
  if (lam[i] == 0.0 || lam[j] == 0.0 ||
      rtk->ssat[sat - 1].ph[rcv - 1][i] == 0.0 ||
...
static void detslp_gf(rtk_t *rtk, const obsd_t *obs, unsigned char n,
  ...
  for (i = 0; i < n; i++) {
    ...
    if (fabs(g1 - g0) > thres && fabs(age) < 7200.0) {
      trace(0x08, "%04.0f %02.0f %02.0f %02.0f %02.0f %02.0f\t", ep[0], ep[1],
            ep[2], ep[3], ep[4], ep[5]);
      trace(0x08,
            "L%dL%d\trcv=%-4d sys=%-4d prn=%-4d age=%-7.2lf el=%-7.2f "
            "thres=%-7.2lf value=%-10.2lf",
            index1 + 1, index2 + 1, obs[i].rcv, sys, prn, age,
            rtk->ssat[obs[i].sat - 1].azel[obs[i].rcv - 1][1] * R2D, thres,
            fabs(g1 - g0));
      trace(0x08, "L[%d]=%14.3lf L[%d]=%14.3lf\n", index1, obs[i].L[index1],
            index2, obs[i].L[index2]);
      for (j = 0; j < rtk->opt.nf; j++)
...
extern void detectSlip(rtk_t *rtk, obsd_t *obs, unsigned char nu,
                       unsigned char nr) {
...
extern int rtkpos(rtk_t *rtk, obsd_t *obs, int n) {
  ...
  unsigned char sat[MAXOBS] = {0}, iu[MAXOBS] = {0}, ir[MAXOBS] = {0},
                exc[MAXSAT] = {0};
  double *rs, *dts;
  double lsqraim[3] = {0};
  double *var;
  ...
  if (pntpos(0, obs, nu, &rtk->sol, NULL, rtk->ssat, &rtk->opt,
             rtk->tt)) { // 0:is ok  -1 eoror
    resetRtk(rtk, SOLQ_NONE);
  ...
  trace(0x04, "%-23s:%14.4f %14.4f %14.4f\n", "rover spp", rtk->sol.rr[0],
        rtk->sol.rr[1], rtk->sol.rr[2]);
  trace(2, "kalman rtk->x, 00, ");
  tracemat(2, rtk->x, 1, 6, 14, 4);

  if (rtk->opt.mode == PMODE_MOVEB || rtk->opt.mode == PMODE_KINEMA ||
      rtk->opt.mode == PMODE_STATIC) { /*  moving baseline */
    /* estimate position/velocity of base station */
    if (pntpos(1, obs + nu, nr, &rtk->solb, NULL, rtk->ssat, &rtk->opt,
               rtk->tt)) { // 0:is ok  -1 eoror
    ...
    trace(0x02,
          ...
          "baseXyzError=%d\n",
          rtk->rb[0], rtk->rb[1], rtk->rb[2], rtk->solb.rr[0], rtk->solb.rr[1],
          rtk->solb.rr[2], baseXyzError);
    ...
    if ((fabs(rtk->rb[0] - rtk->solb.rr[0]) > 100) ||
        ...
        (fabs(rtk->rb[2] - rtk->solb.rr[2]) > 100) &&
            rtk->opt.useRtcmPosFlag == 0) {
      baseXyzError++;
      if (baseXyzError > 10) {
    ...
    if ((fabs(rtk->rb[0] - rtk->solb.rr[0]) > 30) ||
        ...
        (fabs(rtk->rb[2] - rtk->solb.rr[2]) > 30) &&
            rtk->opt.useRtcmPosFlag == 0)
      baseXyzError++;
    else
      baseXyzError = 0;
  ...
  if (rtk->sol.stat == SOLQ_FIX) {
    if (rtk->sol.fixxyz[0] != 0) {
      if ((fabs(rtk->sol.fixxyz[0] - rtk->sol.rr[0]) > 0.2 ||
          ...
          rtk->sol.rr_smooth_cnt >
              (int)((3600.0 - 1) / rtk->opt.timeInterval)) {
        rtk->sol.stat = SOLQ_FLOAT;
        fixErrorCnt++;
        if (fixErrorCnt > 10) {
          rtk->sol.fixxyz[0] = 0.0;
