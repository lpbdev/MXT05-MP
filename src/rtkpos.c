...
static double varerr_gamit(int sat, int sys, double el, double bl, double dt, int f, const prcopt_t* opt)
{
...
static void getWavelength(unsigned char sys, unsigned char prn, double* lam)
{
	...
	{
		for (k = 0; k < NFREQ; k++)	lam[k] = g_gpsLam[k];
	}
	else if (sys == SYS_GLO)
	{
		for (k = 0; k < NFREQ; k++)	lam[k] = g_gloLam[prn - 1][k];
	}
	else if (sys == SYS_GAL)
	{
		for (k = 0; k < NFREQ; k++)	lam[k] = g_galLam[k];
	}
	else
	{
...
static void detslp_ll(rtk_t* rtk, const obsd_t* obs, int i, int rcv)
{
	...
	for (f = 0; f < rtk->opt.nf; f++) {
		...
		if (obs[i].LLI[f] & 1) {
			trace(0x08, "slip detected forward  (sat=%2d rcv=%d F=%d LLI=%x)\n", sat, rcv, f + 1, obs[i].LLI[f]);
...
static void udpos(rtk_t* rtk, double tt)
{
	...
	if (rtk->opt.dynamics == 2) {
		for (i = 0; i < 3; i++) for (j = 0; j < 3; j++) 	Q[i + j * 9] = Q[j + i * 9] = ((i == j) ? tt * tt * tt * tt / 20 : 0.0);
		for (i = 3; i < 6; i++)	for (j = 3; j < 6; j++)	Q[i + j * 9] = Q[j + i * 9] = ((i == j) ? tt * tt / 3 : 0.0);
		for (i = 6; i < 9; i++) for (j = 6; j < 9; j++)	Q[i + j * 9] = Q[j + i * 9] = ((i == j) ? 1.0 : 0.0);
		for (i = 3; i < 6; i++) for (j = 0; j < 3; j++) 	Q[i + j * 9] = Q[j + i * 9] = ((i - 3 == j) ? tt * tt * tt / 8 : 0.0);
		for (i = 6; i < 9; i++) for (j = 0; j < 3; j++)	Q[i + j * 9] = Q[j + i * 9] = ((i - 6 == j) ? tt * tt / 6 : 0.0);
		for (i = 6; i < 9; i++) for (j = 3; j < 6; j++)	Q[i + j * 9] = Q[j + i * 9] = ((i - 3 == j) ? tt / 2 : 0.0);
		xcc = norm(rtk->x + 6, 3);
		//sig = (4 - PI) / PI * (10 - xcc) * (10 - xcc) * 2 * tt / 100;
		for (i = 0; i < 9; i++) for (j = 0; j < 9; j++)  Q[i + j * 9] = Q[i + j * 9] * SQR(0.1);
...
static void udion(rtk_t* rtk, const obsd_t* obs, const unsigned char* iu,
	const unsigned char* ir, double tt, double bl, const unsigned char* sat, unsigned char ns)
{
	gtime_t time;
	double el, fact, ep[6];
	...
	if (0) {
		...
		for (i = 0; i < ns; i++) {
			//if (rtk->x[j] == 0.0 || rtk->sol.stat == 0 || (rtk->sol.stat != 1 && rtk->sol.stat != 2) || rtk->sol.ratio < 1.3) {
			//if (rtk->x[j] == 0.0 || rtk->sol.ratio < 1.3 || (rtk->sol.stat != 1 && rtk->sol.stat != 2)) {
			if (rtk->x[j] == 0.0 || (fabs(timediff(rtk->ssat[sat[i] - 1].ddionTime, rtk->sol.time)) > 60)) {
				//if (rtk->x[j] == 0.0) {
				...
				//else
				//	rtk->P[j + j * rtk->nx] += SQR(0.0001 * fact) * fabs(rtk->sol.age);
				j++;
			}
		}
	}
...
static void detslp_dop(rtk_t* rtk, const obsd_t* obs, int i, int rcv)
{
	...
	else	for (k = 0; k < NFREQ; k++)	lam[k] = g_bdsLam[k];

	for (f = 0; f < rtk->opt.nf; f++) {
		if (obs[i].L[f] == 0.0 || obs[i].D[f] == 0.0 || rtk->ssat[obs[i].sat - 1].ph[rcv - 1][f] == 0.0) {
			continue;
		}
		if (rtk->tt < DTTOL) continue;
		...
		trace(0x08, "detslp_dop detected (sat=%2d rcv=%d L%d=%.3f %.3f thres=%.3f)\n",
			sat, rcv, f + 1, dph, dpt, thres);
	}
}

static void udbias(rtk_t* rtk, double tt, const obsd_t* obs, const unsigned char* sat,
	const unsigned char* iu, const unsigned char* ir, unsigned char ns, double* r)
{
    ...
    for (f = 0; f < nf; f++) {
        ...
        for (i = 0; i < ns; i++) {
            ...
            if (sys == SYS_GPS || sys == SYS_QZS)	lam = g_gpsLam[f];
            else if (sys == SYS_GLO) {
                lam = g_gloLam[prn - 1][f];
            }
            else if (sys == SYS_GAL)	lam = g_galLam[f];
            else if (sys == SYS_BDS)	lam = g_bdsLam[f];
            else lam = g_gpsLam[f];
            cp = sdobs(obs, iu[i], ir[i], f); /* cycle */
            pr = sdobs(obs, iu[i], ir[i], f + NFREQ);
            if (cp == 0.0 || pr == 0.0 || lam <= 0.0) continue;

            slip = rtk->ssat[sat[i] - 1].slip[f];
            sys = satsys(sat[i], NULL);
            if (sys == SYS_GPS || sys == SYS_QZS)	nsat[0][f]++;
            else if (sys == SYS_GLO)	nsat[1][f]++;
            else if (sys == SYS_GAL)	nsat[2][f]++;
            else if (sys == SYS_BDS)	nsat[3][f]++;
            if (slip & 1) {
                rtk->x[nx2] = 0.0;
                if (sys == SYS_GPS || sys == SYS_QZS)	nslip[0][f]++;
                else if (sys == SYS_GLO)		nslip[1][f]++;
                else if (sys == SYS_GAL)	nslip[2][f]++;
                else if (sys == SYS_BDS)	nslip[3][f]++;
            }
            if (baseXyzError == 0)
                pr = r[iu[i]] - r[ir[i]];
            bias = cp - pr / lam;
        ...
        for (i = 0; i < ns; i++) {
            ...
            if (rtk->opt.ionoopt == IONOOPT_EST) {
                if (rtk->x[nx2] == 0.0) {
                    ...
                    trace(4, "ini bias:sat:sys=%3d prn=%3d f:%2d x=%7.2f stat=%2d ratio=%7.2f value:%7.2f el=%7.2f\n",
                        sys, prn, f, rtk->x[nx2], rtk->sol.stat, rtk->sol.ratio, bias, rtk->ssat[sat[i] - 1].azel[0][1] * R2D);
                    if (f == 0) {
                        rtk->ssat[sat[i] - 1].ionIndexCnt = 0;
                ...
                if (rtk->x[nx2] == 0.0 || rtk->sol.stat == 0 || (rtk->sol.stat != 1 && rtk->sol.stat != 2 && rtk->sol.stat != 4) || rtk->sol.ratio < 1.3) {
                    var = 900;
                    initx(rtk, bias, var, nx2);
                    trace(4, "ini bias:sat:sys=%3d prn=%3d f:%2d x=%7.2f stat=%2d ratio=%7.2f value:%7.2f el=%7.2f\n",
                        sys, prn, f, rtk->x[nx2], rtk->sol.stat, rtk->sol.ratio, bias, rtk->ssat[sat[i] - 1].azel[0][1] * R2D);
...
static void udstate(rtk_t* rtk, const obsd_t* obs, const unsigned char* sat,
	const unsigned char* iu, const unsigned char* ir, unsigned char ns, double* r)
{
...
static void zdres_sat(int base, double r, const obsd_t* obs, const double* azel, const prcopt_t* opt, double* y)
{
	...
	for (i = 0; i < nf; i++) {
		...
		if (obs->L[i] == 0.0 && obs->P[i] == 0.0) continue;
		if (obs->L[i] != 0.0) y[i] = obs->L[i] * lam[i] - r;
		if (obs->P[i] != 0.0) y[i + nf] = obs->P[i] - r;
	}
}
...
static int zdres(rtk_t* rtk, int base, const obsd_t* obs, int n, const double* rs, const double* dts,
	const int* svh, const double* rr, const prcopt_t* opt, int index, double* y,
	double* e, double* azel, double* rdist)
{
	double r, rr_[3], pos[3], disp[3];
	double zhd, zazel[] = { 0.0,90.0 * D2R };
	int i, sat, nf = rtk->opt.nf;
	for (i = 0; i < 2 * nf * n; i++) y[i] = 0.0;
	if (sos3(rr) <= 0.0) return -1;//no receiver pos
	for (i = 0; i < 3; i++) rr_[i] = rr[i];

	if (opt->tidecorr) {
		tidedisp(gpst2utc(obs[0].time), rr_, opt->tidecorr, &g_nav.erp,
			opt->odisp[base], disp);
	...
	for (i = 0; i < n; i++)
	{
		...
		r -= CLIGHT * dts[i * 2];
		//if (rtk->differHeight > 15 || rtk->bl > 1.1E4) {
		//if (rtk->differHeight > 15) {
...
static int ddmat(rtk_t* rtk, double* D, int* fixSatNum)
{
	...
	for (i = 0; i < na; i++) D[i + i * rtk->nx] = 1.0;
	for (f = 0, k = na; f < nf; f++) {
		for (m = 0; m < NSYS; m++)
		{
			flag = 0;
			...
			for (j = k; j < k + rtk->nx; j++) {
				...
				sys = satsys(sati, NULL);
				if (sati == 0 || f != frqi)continue;
				if (i == j || rtk->x[j] == 0.0 || !test_sys(sys, m) || !rtk->ssat[sati - 1].vsat[f]) continue;
...
{
	int i, n, f = 0, m, indexb, index[MAXSAT], nv = 0, nf = rtk->opt.nf, xa = rtk->np + rtk->nt + rtk->ni;
	int sati, frqi, sys;

	for (f = 0; f < nf; f++) for (m = 0; m < NSYS; m++) {
...
static void holdamb(rtk_t* rtk, const double* xa)
{
	...
	for (f = 0; f < nf; f++)  for (m = 0; m < NSYS; m++) {
		for (n = i = 0; i < nb; i++) {
			...
			sys = satsys(sati, NULL);
			if (sati == 0 || f != frqi)continue;
			if (!test_sys(sys, m) || rtk->ssat[sati - 1].fix[frqi] != 2) continue;
			index[n++] = i;
	...
	if (nv > 0) {
		for (i = 0; i < nv; i++) rtk->R[i + i * nv] = 0.001;
		/* update states with constraints */
		//if ((info = filter(rtk->x, rtk->P, H, v, R, rtk->nx, nv))) {
		if ((info = filter(rtk, rtk->xp, rtk->Pp, rtk->H, rtk->v, rtk->R, rtk->nx, nv, rtk->x, rtk->P))) {
...
{
	int i, j, k = 0, m = 0, n = 0, fixSat = 0, ny, nb, info, nx = rtk->nx, na = rtk->np + rtk->nt + rtk->ni;
	...
	//if ((!y) || (!Qy) || (!DP) || (!b) || (!db) || (!Qb) || (!Qab) || (!QQ) || (!bias) || (!Pp))
	//{
	...
	if (info == 0 || info == -2 || info == -3) {
		...
		if (lcopt == 0)
			rtk->sol.ratio = ratio;
		//printf("ratio=%f\n", rtk->sol.ratio);
		if (fixSat < 6)	thresar = 5.0;
		else if (fixSat >= 6 && fixSat < 10)	thresar = 3.0;
		else if (fixSat >= 10 && fixSat < 20)	thresar = 2.5;
		else		thresar = 2.0;
		//thresar = 1.5;
		if (lcopt == 3 || lcopt == 4) {
			thresar = 3.0;
		}
		//if (rtk->opt.ionoopt == IONOOPT_EST && fixSat>10 && rtk->sol.fixxyz[0] != 0.0) {
		//	thresar = 2.0;
		//}
		//if (lcopt == 4) {
		//	thresar = 1.3;
		//}
		//thresar = 1.0;
		trace(0x02, "lambda ratio=%f\n", ratio);
		//if (nb < 5) rtk->sol.ratio = 0.0;
		if ((s[0] <= 0.0 || (ratio >= thresar)) && info != -2 && info != -3) {
			for (i = 0; i < na; i++) {
				if (lcopt == 0) {
					for (j = 0; j < na; j++) Pp[i + j * na] = rtk->Pp[i + j * nx];
				}
			}
			for (i = 0; i < nb; i++) {
				bias[i] = b[i];
			...
			if (!matinv(Qb, nb)) {
				...
				Qab = rtk->F;
				QQ = rtk->I;
				for (i = 0; i < na; i++)	for (j = 0; j < nb; j++)	Qab[i + j * na] = Qy[i + (na + j) * ny]; //??Qab???
				...
				if (lcopt == 0) {
					...
					for (i = 0; i < 3; i++) {
						for (j = 0; j < 3; j++) rtk->Pp[i + j * nx] = Pp[i + j * na];
					}
					for (i = 0; i < 3; i++) {
						for (j = 0; j < 3; j++) rtk->Pp[i + j * nx] = Pp[i + j * na];
					}
...
static int scanFloatState(rtk_t* rtk, unsigned char* floatSat, unsigned char* floatSatFrq) {
	...
	if (rtk->nxFixNx <= 5) return 0;
	for (i = na; i < rtk->nx; i++) {
		sati = rtk->nxRecordSat[i - na];
		frqi = rtk->nxRecordFrq[i - na];
		if (rtk->ssat[sati - 1].vsat[frqi] == 0) continue;
		flag = 0;
		for (j = 0; j < rtk->nxFixNx; j++) {
			satj = rtk->nxFixSat[j];
			frqj = rtk->nxFixFrq[j];
			if (sati == satj && frqi == frqj) {
				if (rtk->ssat[satj - 1].slip[frqj] & 1) break;
		...
		if (flag == 0) {
			trace(0x02, "AR failed by sat=%3d f=%2d el=%7.2f %7.2f\n", sati, frqi, rtk->ssat[sati - 1].azel[0][1] * R2D, rtk->ssat[sati - 1].azel[1][1] * R2D);
...
static void rtkFloatState(rtk_t* rtk, unsigned char vsat[][NFREQ]) {
	...
	if (k > 0 && rtk->nxFixNx > 0) {
		...
		for (i = 0; i < k; i++)	rtk->ssat[floatSat[i] - 1].vsat[floatSatFrq[i]] = 0;
...
static void rtkAR(rtk_t* rtk, obsd_t* obs, unsigned char* sat, int n) {
	...
	double ratio = 0.0;
	for (i = 0; i < MAXSAT; i++)	for (j = 0; j < NFREQ; j++)	vsat[i][j] = rtk->ssat[i].vsat[j];
	matcpy(rtk->x, rtk->xp, NX, 1);
	matcpy(rtk->P, rtk->Pp, NX, NX);
	...
	if (rtk->sol.stat != SOLQ_FIX) rtk->nxFixNx = 0;
	...
	for (i = na; i < rtk->nx; i++) {
		...
		frqi = rtk->nxRecordFrq[i - na];
		sys = satsys(sati, &prn);
		//if (j != frqi)	trace(0x04, "\nf:%d pos sat:", frqi);
		//trace(0x04, "(%d:%3d) ", sys, prn);
		j = frqi;
	}
	//trace(0x04, "\n");
	...
	if (rtk->sol.stat == SOLQ_FIX) {
		...
		for (i = 0; i < rtk->nx; i++) rtk->x[i] = rtk->xp[i];
		...
		for (i = na; i < rtk->nx; i++) {
			...
			frqi = rtk->nxRecordFrq[i - na];
			sys = satsys(sati, &prn);
			if (rtk->ssat[sati - 1].slip[frqi] & 2) continue;
			if (rtk->ssat[sati - 1].vsat[frqi] == 0) continue;
			if (rtk->xp[i] != 0.0) {
				rtk->nxFixSat[k] = sati;
				rtk->nxFixFrq[k] = frqi;
		...
		if (rtk->opt.dynamics) { /* velocity and covariance */
			...
			if (rtk->opt.dynamics == 2) {
				for (i = 6; i < 9; i++) rtk->sol.rr[i] = rtk->xp[i];
			}
	...
	for (i = 0; i < MAXSAT; i++) for (j = 0; j < nf; j++) {
		if (rtk->ssat[i].fix[j] == 2 && rtk->sol.stat != SOLQ_FIX) rtk->ssat[i].fix[j] = 1;
	}
	...
	if (rtk->sol.stat == SOLQ_FIX) {
		...
		for (m = 0; m < NSYS; m++) {
			for (f = 0; f < NFREQ; f++) {
				...
				for (j = 0; j < ns; j++) {
					...
					if (sat[j] == 0 || sat[j] == sat[i]) continue;
					...
					rtk->ssat[sat[j] - 1].ddAmb[f] = rtk->ssat[sat[i] - 1].fix_amb[f] - rtk->ssat[sat[j] - 1].fix_amb[f];
				}
			}
		}
		int m, f;
		...
		for (m = 0; m < NSYS; m++) {
			...
			for (j = 0; j < ns; j++) {
				...
				if (!test_sys(sys, m))	continue;
				if (sat[j] == 0 || sat[j] == sat[i]) continue;
				if (rtk->ssat[sat[j] - 1].fix_amb[0] == 9999.9) continue;
				if (rtk->ssat[sat[j] - 1].ph[0][0] == 0.0 || rtk->ssat[sat[j] - 1].ph[1][0] == 0.0) continue;
				index1 = 0; index2 = 0;
				for (f = 1; f < NFREQ; f++) {
					if (rtk->ssat[sat[j] - 1].ph[0][f] != 0.0 && rtk->ssat[sat[j] - 1].ph[1][f] != 0.0) {
						index2 = f;
						break;
					}
				}
				...
				if (rtk->ssat[sat[i] - 1].ph[0][index2] == 0 || rtk->ssat[sat[i] - 1].ph[1][index2] == 0)
				{
				...
				ddion = (SQR(f2) / (SQR(f1) - SQR(f2))) * (ddL1 - ddL2 - (lam[0] * ddN1 - lam[index2] * ddN2));
...
static double prectrop(rtk_t* rtk, const double* pos, int r, const double* azel,
...
static int check_res(rtk_t* rtk, const obsd_t* obs, const double* x, unsigned char* sat,
	double* y, unsigned char* iu, unsigned char* ir, int ns, double* azel)
{
	unsigned char i, j, k, nv = 0, m, index1, index2, sati, satj, frqi, frqj, flag, f, sysi, sysj, prni, prnj, nf = rtk->opt.nf;
	...
	tropu = zeros(SELETE_SAT_NUM, 1); tropr = zeros(SELETE_SAT_NUM, 1);
	dtdxu = zeros(SELETE_SAT_NUM, 1); dtdxr = zeros(SELETE_SAT_NUM, 1);
	...
	for (m = 0; m < NSYS; m++) {
		for (f = 0; f < nf; f++) {
			...
			for (j = 0; j < ns && flag == 1; j++) {
				...
				if (rtk->ssat[sat[j] - 1].slip[f] & 1) continue;
				if (rtk->ssat[sat[j] - 1].slip[f] & 2) continue;
				if (rtk->ssat[sat[j] - 1].vsat[f] == 0)continue;
				if (!validobsLP(iu[j], ir[j], f, nf, y)) continue;
				...
				rtk->v[nv] = (y[f + iu[i] * nf * 2] - y[f + ir[i] * nf * 2]) - (y[f + iu[j] * nf * 2] - y[f + ir[j] * nf * 2]);
...
//static int findUseCntFlag(rtk_t* rtk,int f) {
//	int i, flag=0;
//	for (i = 0; i < MAXSAT; i++) {
//		if (rtk->ssat[i].useCnt[f] >= 1) {
//			flag = 1;
//			break;
//		}
//	}
...
static int ddres(int post, rtk_t* rtk, const obsd_t* obs, double dt, const double* x,
	const double* P, const unsigned char* sat, double* y, double* e, double* azel, const unsigned char* iu,
	const unsigned char* ir, int ns, double* standard_v, unsigned char* robust, int* vflg)
{
	...
	double* tropu, * tropr;
	double* dtdxu, * dtdxr;
	int i, j, k, sati, frqi, satj, frqj, index1, index2, m, f, nv = 0, nb[NFREQ * 4 * 2 + 2] = { 0 }, b = 0, sysi, sysj, nf = rtk->opt.nf;
	...
	for (f = opt->mode > PMODE_DGPS ? 0 : nf; f < nf * 2; f++) {
		...
		for (m = 0; m < NSYS; m++) {
			...
			for (i = -1, j = 0; j < ns; j++) {
				...
				if (rtk->ssat[sat[j] - 1].vs != 1) continue;
				//if (rtk->ssat[sat[j] - 1].useCnt[f%nf] < 1 && findUseCntFlag(rtk,f%nf)) continue;
				if (opt->mode > PMODE_DGPS) {
					if (rtk->allSlipFlag[m][0 % nf] == 0) {
						if (rtk->ssat[sat[j] - 1].slip[f % nf] & 1) continue;
						if (rtk->ssat[sat[j] - 1].slip[f % nf] & 2) continue;
					}
					if (!validobsLP(iu[j], ir[j], f, nf, y)) continue;
				}
				else {
			...
			for (j = 0; j < ns; j++) {
				...
				if (f < nf)
					if (lami[f] == 0.0 || lamj[f] == 0.0) continue;
				if (rtk->opt.mode == PMODE_DGPS)
					Hi = rtk->H + nv * 3;
				else
					Hi = rtk->H + nv * rtk->nx;
				...
				if (f < nf) {
					//rtk->v[nv] -= lami * rtk->ssat[sat[i] - 1].dbias[f] - lamj * rtk->ssat[sat[j] - 1].dbias[f];
					for (index1 = na; index1 < rtk->nx; index1++) {
						sati = rtk->nxRecordSat[index1 - na];
						frqi = rtk->nxRecordFrq[index1 - na];
						if (frqi != f) continue;
					...
					if (rtk->opt.bl <= BSLTHRESHOLD && baseXyzError==0) {
						if (post == 0 && f < nf && rtk->sol.fixxyz[0] != 0.0 && rtk->sol.rr_smooth_cnt>10) {
							...
							if (fabs(rtk->ssat[sat[j] - 1].resc[f]) > 0.35 && fabs(rtk->ssat[sat[j] - 1].resc[f]) < 0.75) {
                                ...
                                if (f == 0) {
                                    ...
                                    for (k = na; k < rtk->nx; k++) {
                                        if (rtk->nxRecordSat[k - na] == sat[j] && rtk->nxRecordFrq[k - na] == sat[j]) {
                                            initx(rtk, 0, 0, k);
                                        }
                                    }
				...
				if (post == 0 && fabs(rtk->v[nv]) > 50.0) {
					...
					memset(rtk->Ri, 0, sizeof(double) * NY);
					memset(rtk->Rj, 0, sizeof(double) * NY);
					trace(0x04, "outlier rejected (sat=%3d-%3d %s%d v=%.3f)\n", sat[i], sat[j], f < nf ? "L" : "P", f% nf + 1, rtk->v[nv]);
					for (k = na; k < rtk->nx; k++) {
						if (rtk->nxRecordSat[k - na] == sat[j]) {
							initx(rtk, 0, 0, k);
						}
				...
				//if (f < nf && post ==0)
				//	trace(4,"f=%d nv=%3d sys=%3d prn=%3d x[%3d]=%7.2f x[%3d]=%7.2f dbias=%7.2f v[%2d]=%10.4lf\n",
				//		f, nv, sysj, prnj,index1, x[index1],index1, x[index2], x[index1]- x[index2],nv, rtk->v[nv]);
				if (!post && robust[nv] == 0) {
					rtk->Rj[nv] = varrL(&obs[iu[j]], azel[1 + iu[j] * 2], bl, f, opt);
					//rtk->Rj[nv] = varerr_gamit(sat[j], sysj, azel[1 + iu[j] * 2], bl, dt, f, opt);
					if (f >= nf)	  rtk->Rj[nv] = rtk->Rj[nv] * SQR(500);
				}
	...
	if (rtk->opt.bl > BSLTHRESHOLD) {
		...
		if (post > 0 && stat == 0) {
			...
			if (standard_v[j] > K0) {
				...
				if (vmax <= K0) {
					...
					rtk->Rj[j] = SQR((1 / p2));
					trace(0x04, "post=%d robust1 sat=%d %s%d vmax=%f\n", post, vmax_sat, stype, freq + 1, vmax);
				}
				else if (vmax > 2 * K1) {
					...
					rtk->Rj[j] = 10E8;
					trace(0x04, "post=%d robust2 sat=%d %s%d vmax=%f\n", post, vmax_sat, stype, freq + 1, vmax);
				}
			}
		}
	}
		...
		if (post > 0 && stat == 0) {
			...
			if (vmax != 0) {
				rtk->ssat[vmax_sat - 1].vs = 0;
				trace(4, "outlier rejected sat=%d v=%.2f std=%.3f stdv=%.2f\n", vmax_sat, rtk->v[vmaxj], sqrt(rtk->Ri[vmaxj]), vmax);
				//if (rtk->opt.tropopt == TROPOPT_EST) {
				//	free(tropu); free(tropr);free(dtdxu);free(dtdxr);
				//}
				memset(rtk->Ri, 0, sizeof(double) * NY);
				...
				for (j = 0; j < nv; j++) {
					if (standard_v[j] > K0) {
						...
						if (vmax <= K0) {
							...
							p2 = p1 * (K0 / vmax) * SQR(((K1 - vmax) / (K1 - K0)));
							rtk->Rj[j] = SQR((1 / p2));
							trace(8, "post=%d robust1 sat=%d %s%d v=%.2f std=%.3f stdv=%.2f\n",
								post, vmax_sat, stype, freq + 1, rtk->v[j], sqrt(rtk->Ri[j]), vmax);
						}
						else {
							rtk->Rj[j] = 10E8;
							trace(8, "post=%d robust2 sat=%d %s%d v=%.2f std=%.3f stdv=%.2f\n",
								post, vmax_sat, stype, freq + 1, rtk->v[j], sqrt(rtk->Ri[j]), vmax);
						}
					}
				}
...
extern int relpos(rtk_t* rtk, obsd_t* obs, unsigned char nu, unsigned char nr,
	...
	azel = zeros(2 * SELETE_SAT_NUM * 2, 1); y = zeros(NFREQ * 2 * SELETE_SAT_NUM * 2, 1);
	e = zeros(3 * SELETE_SAT_NUM * 2, 1); standard_v = zeros(NY, 1); r = zeros(SELETE_SAT_NUM * 2, 1);
	...
	if (zdres(rtk, 1, obs + nu, nr, rs + nu * 6, dts + nu * 2, svh + nu, rtk->rb, &rtk->opt, 1, y + nu * nf * 2, e + nu * 3, azel + nu * 2, r + nu))
	{
		...
		free(y); free(e); free(azel); free(standard_v); free(r);
		rtk->sol.stat = SOLQ_NONE;
		return 1;
	}
	...
	while (1) {
		...
		if (ilterCout == 1) for (i = 0; i < NY; i++) robust[i] = 0;
		for (i = 0; i < NSYS; i++) 	for (f = 0; f < NFREQ * 2; f++) 	rtk->base_prn[i][f] = 0;
		...
		//memset(rtk->H, 0, sizeof(rtk->H));
		memset(rtk->H, 0, sizeof(double) * NX * NY);
		//printf("x:%14.4lf %14.4lf %14.4lf\n", rtk->x[0], rtk->x[1], rtk->x[2]);

		nv = ddres(0, rtk, obs, dt, rtk->x, rtk->P, sat, y, e, azel, iu, ir, ns, standard_v, robust, vflg);
		if (nv == -2) continue;
		if (nv < 4) {
			trace(0x02, "nv=%d\n", nv);
			rtk->sol.stat = SOLQ_NONE;
		...
		/*fprintf(fptest, "**************H****************\n");
		for (int i = 0; i < nv; i++) {
			for (int j = 0; j < rtk->nx; j++)	fprintf(fptest, "%14.6lf ", rtk->H[i * rtk->nx + j]);
			fprintf(fptest, "\n");
		}
		fprintf(fptest, "\n");
		fflush(fptest);
		fprintf(fptest, "**************R****************\n");
		for (int i = 0; i < nv; i++) {
			for (int j = 0; j < nv; j++)	fprintf(fptest, "%14.6lf ", rtk->R[i * nv + j]);
			fprintf(fptest, "\n");
		}
		fprintf(fptest, "\n");
		fflush(fptest);
		...
		if (rtk->opt.mode == PMODE_DGPS) {
			if ((info = filter(rtk, rtk->x, rtk->P, rtk->H, rtk->v, rtk->R, 3, nv, rtk->xp, rtk->Pp))) {
				trace(0x02, "filter error (info=%d)\n", info);
				rtk->sol.stat = SOLQ_NONE;
				break;
			}
			...
			if ((info = filter(rtk, rtk->x, rtk->P, rtk->H, rtk->v, rtk->R, rtk->nx, nv, rtk->xp, rtk->Pp))) {
				trace(0x02, "filter error (info=%d)\n", info);
				rtk->sol.stat = SOLQ_NONE;
				break;
			}
		...
		//fprintf(fptest, "**************XP****************\n");
		//for (int i = 0; i < rtk->nx; i++) {
		//	fprintf(fptest, "%14.6lf ", rtk->xp[i]);
		//}
		//fprintf(fptest, "\n");
		//fflush(fptest);
		//fprintf(fptest, "**************Pp****************\n");
		//for (int i = 0; i < rtk->nx; i++) {
		//	for (int j = 0; j < rtk->nx; j++)	fprintf(fptest, "%14.6lf ", rtk->Pp[i * rtk->nx + j]);
		//	fprintf(fptest, "\n");
		//}
		//fprintf(fptest, "\n");
		//fflush(fptest);
		...
		nv = ddres(1, rtk, obs, dt, rtk->xp, rtk->Pp, sat, y, e, azel, iu, ir, ns, standard_v, robust, vflg);
		//fprintf(fptest, "**************v****************\n");
		//for (int i = 0; i < 20; i++) {
		//	fprintf(fptest, "%14.6lf ", rtk->v[i]);
		//}
		//fprintf(fptest, "\n");
		//fflush(fptest);
		if (nv == -2) {
		...
		//if ((stdvMax > K0+0.01) && nv == 0 && ilterCout<20)	continue;
		else {
			//if (stdvMax > K1) {
			//	rtk->sol.stat = SOLQ_NONE;
			//	break;
	...
	if (rtk->sol.stat == SOLQ_FIX) {
		if (rtk->sol.fixxyz[0] != 0.0 && rtk->sol.rr_smooth_cnt > 10) {
			...
			if (fabs(rtk->x[0] - rtk->sol.fixxyz[0]) > 0.2 || fabs(rtk->x[1] - rtk->sol.fixxyz[1]) > 0.2 || fabs(rtk->x[2] - rtk->sol.fixxyz[2]) > 0.2 )
			{
				...
				for (i = 0; i < 3; i++) rtk->x[i] = rtk->sol.fixxyz[i];
				rtk->fixCheckCnt = 0;
			}
		}	
		else {
	...
	free(y); free(e); free(azel); free(standard_v); free(r);
	for (i = 0; i < NSYS; i++) 	for (f = 0; f < NFREQ * 2; f++) 	rtk->base_prn_fix[i][f] = rtk->base_prn[i][f];
...
extern int getSatNum(rtk_t* rtk, const obsd_t* obs, unsigned char nu, unsigned char nr,
	const prcopt_t* opt, unsigned char* sat, unsigned char* iu, unsigned char* ir, double elMin, int snrMin)
{
	...
	for (i = 0; i < nu; i++) {
		...
		if (rtk->ssat[obs[i].sat - 1].azel[0][1] < elMin || rtk->ssat[obs[i].sat - 1].azel[1][1] < elMin) continue;
		for (j = nu; j < nu + nr; j++) {
			if (obs[i].sat == obs[j].sat) { /* elevation at base station */
				flag = 0;
				for (f = 0; f < nf; f++) {
					if (obs[i].P[f] != 0.0 && obs[j].P[f] != 0.0 && obs[i].L[f] != 0.0 && obs[j].L[f] != 0.0
						&& obs[i].SNR[f] > snrMin && obs[j].SNR[f] > snrMin) {
						flag = 1;
						break;
					}
				}
...
static double gfmeas1(const obsd_t* obs, double* lam, int index1, int index2)
{
	...
	if (lam[i] == 0.0 || lam[j] == 0.0 || obs->L[i] == 0.0 || obs->L[j] == 0.0) return 0.0;
	return (lam[j] * obs->L[j] - lam[i] * obs->L[i]) / lam[i];
}
static double gfmeas0(rtk_t* rtk, unsigned char sat, unsigned char rcv, double* lam, int index1, int index2)
{
	int i = index1, j = index2;
	if (lam[i] == 0.0 || lam[j] == 0.0 || rtk->ssat[sat - 1].ph[rcv - 1][i] == 0.0 || rtk->ssat[sat - 1].ph[rcv - 1][j] == 0.0) return 0.0;
	return (lam[j] * rtk->ssat[sat - 1].ph[rcv - 1][j] - lam[i] * rtk->ssat[sat - 1].ph[rcv - 1][i]) / lam[i];
}
...
static void detslp_gf(rtk_t* rtk, const obsd_t* obs, unsigned char n, int index1, int index2)
{
	...
	for (i = 0; i < n; i++) {
		//if (rtk->ssat[obs[i].sat - 1].azel[obs[i].rcv - 1][1] * R2D < 10.0) continue;
		...
		if ((g1 = gfmeas1(obs + i, lam, index1, index2)) == 0.0) continue;
		...
		if (fabs(g1 - g0) > thres && fabs(age) < 7200.0) {
			trace(0x08, "%04.0f %02.0f %02.0f %02.0f %02.0f %02.0f\t", ep[0], ep[1], ep[2], ep[3], ep[4], ep[5]);
			trace(0x08, "L%dL%d\trcv=%-4d sys=%-4d prn=%-4d age=%-7.2lf el=%-7.2f thres=%-7.2lf value=%-10.2lf",
				index1 + 1, index2 + 1, obs[i].rcv, sys, prn, age, rtk->ssat[obs[i].sat - 1].azel[obs[i].rcv - 1][1] * R2D, thres, fabs(g1 - g0));
			trace(0x08, "L[%d]=%14.3lf L[%d]=%14.3lf\n", index1, obs[i].L[index1], index2, obs[i].L[index2]);
			for (j = 0; j < rtk->opt.nf; j++)
...
extern void detectSlip(rtk_t* rtk, obsd_t* obs, unsigned char nu, unsigned char nr) {
	...
	if (rtk->opt.nf >= 2) detslp_gf(rtk, obs, nu, 0, 1);
	if (rtk->opt.nf >= 3) detslp_gf(rtk, obs, nu, 0, 2);
	if (rtk->opt.nf >= 4) detslp_gf(rtk, obs, nu, 0, 3);
	if (rtk->opt.nf >= 5) detslp_gf(rtk, obs, nu, 0, 4);
	if (rtk->opt.nf >= 6) detslp_gf(rtk, obs, nu, 0, 5);
	if (nr > 0) {
		if (rtk->opt.nf >= 2) detslp_gf(rtk, obs + nu, nr, 0, 1);
		if (rtk->opt.nf >= 3) detslp_gf(rtk, obs + nu, nr, 0, 2);
		if (rtk->opt.nf >= 4) detslp_gf(rtk, obs + nu, nr, 0, 3);
		if (rtk->opt.nf >= 5) detslp_gf(rtk, obs + nu, nr, 0, 4);
		if (rtk->opt.nf >= 6) detslp_gf(rtk, obs + nu, nr, 0, 5);
	}
	for (i = 0; i < nu; i++)
	{
...
void static SmoothBasePosion(rtk_t* rtk) {
	...
	if (rtk->rb[0] == 0.0 || rtk->rb[1] == 0.0 || rtk->rb[2] == 0.0) {
		...
		smoothCnt++;
		for (i = 0; i < 3; i++) rtk->rb[i] = baseXyz[i];
		//}
	}
	//for (i = 0; i < 3; i++) {
	//	if (fabs(rtk->rb[i] - rtk->solb.rr[i]) > 15.0 && rtk->solb.ns[0]>10) {
	//		for (i = 0; i < 3; i++) rtk->rb[i] = rtk->solb.rr[i];
	//		smoothCnt = 0;
	//		return;
	//	}
	//}
...
extern int rtkpos(rtk_t* rtk, obsd_t* obs, int n) {
	...
	rtk->rejSatCnt = 0;
	for (m = 0; m < NSYS; m++)	for (i = 0; i < NFREQ; i++)	rtk->allSlipFlag[m][i] = 0;
	for (i = 0; i < n; i++) {
		if (obs[i].rcv != 1)
			break;
	}
	...
	//if (ROUND(rtk->tt) != ROUND(rtk->tt_pre)&&rtk->tt_pre!=0.0)
	//	rtk->sol.stat = SOLQ_NONE;
	rtk->tt_pre = rtk->tt;
	for (i = 0; i < 3; i++) {
	...
	if (pntpos(0, obs, nu, &rtk->sol, NULL, rtk->ssat, &rtk->opt, rtk->tt)) {    //0:is ok  -1 eoror
		resetRtk(rtk, SOLQ_NONE);
		trace(0x02, "rover position error\n");
		for (i = 0; i < 6; i++) rtk->sol.rr[i] = 0.0;
		free(rs); free(dts); free(var);
		return 2;
	}
	...
	/* estimate position/velocity of base station */
		if (pntpos(1, obs + nu, nr, &rtk->solb, NULL, rtk->ssat, &rtk->opt, rtk->tt)) { //0:is ok  -1 eoror
			...
			if (nr <= 4 || rtk->opt.mode == PMODE_MOVEB) {
				...
				for (i = 0; i < 6; i++) rtk->solb.rr[i] = 0.0;
				free(rs); free(dts); free(var);
				return 3;
			}
			else {
        ...
        if ((fabs(rtk->rb[0] - rtk->solb.rr[0]) > 100) || (fabs(rtk->rb[1] - rtk->solb.rr[1]) > 100) || (fabs(rtk->rb[2] - rtk->solb.rr[2]) > 100) && rtk->opt.useRtcmPosFlag == 0) {
            ...
            baseXyzError = 0;
        if ((fabs(rtk->rb[0] - rtk->solb.rr[0]) > 30) || (fabs(rtk->rb[1] - rtk->solb.rr[1]) > 30) || (fabs(rtk->rb[2] - rtk->solb.rr[2]) > 30) && rtk->opt.useRtcmPosFlag == 0) 
            baseXyzError++;
        else
             baseXyzError = 0;
		if (rtk->opt.mode == PMODE_KINEMA || rtk->opt.mode == PMODE_STATIC) {
				...
				if (fabs(rtk->sol.age) > rtk->opt.maxtdiff) {
					trace(0x02, "age=%f\t sol.time=%d\t solb.time=%d\t \n", rtk->sol.age, (int)rtk->sol.time.time, (int)rtk->solb.time.time);
					resetRtk(rtk, SOLQ_SINGLE);
					free(rs); free(dts); free(var);
					return 4;
				}
	...
	//if (ns > 15) {
	//	rtk->opt.elmin = 25.0 * D2R;
	//}
	//else {
	//	rtk->opt.elmin = 15 * D2R;
	//}
	...
	//rtk->sol.rr[0] =  -2742725.4400; rtk->sol.rr[1] = 5119456.7620; rtk->sol.rr[2] = 2627448.3937;
	//if (rtk->sol.rr_smooth[0] != 0) {
	//	rtk->sol.rr[0] = rtk->sol.rr_smooth[0];
	//	rtk->sol.rr[1] = rtk->sol.rr_smooth[1];
	//	rtk->sol.rr[2] = rtk->sol.rr_smooth[2];
	//}
	if (rtk->sol.fixxyz[0] != 0.0 && rtk->sol.rr_smooth_cnt > 10) {
		for (i = 0; i < 3; i++) rtk->sol.rr[i] = rtk->sol.fixxyz[i];
	}
	//base  pntpos : -2734040.5016   5124959.9521   2625762.3904 - 2734035.9264   5124950.6126   2625752.6819 smoothCnt = 1
	//rtk->sol.rr[0] = -2286264.6685;   rtk->sol.rr[1] = 5003472.2191;   rtk->sol.rr[2] = 3217169.1057;
	rtk->sol.stat = 2;
	...
	if (rtk->opt.mode == 2 && rtk->sol.stat == SOLQ_FIX && rtk->opt.ionoopt != IONOOPT_EST) {
		...
		if (rtk->fix_state == 1) {
			while (1) {
				lsqstat = rtkLsq(rtk, obs, 2 * ns, nu, svh, rs, dts, var, lsqraim, ns, sat, iu, ir, exc);
				if (lsqstat == -2 && rtk->sol.ns[1] > 5 && lsqraim[0] > 0) {
					delSat++;
					exc[(int)lsqraim[0] - 1] = 1;
					sys = satsys((int)lsqraim[0], &prn);
					rtk->ssat[(int)lsqraim[0] - 1].vs = 0;
					trace(0x04, "rtkls reject sys=%d prn=%d el=%lf\n", sys, prn, rtk->ssat[(int)lsqraim[0] - 1].azel[0][1] * R2D);
					for (i = 0; i < NFREQ; i++) {
