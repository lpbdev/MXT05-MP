...
extern void assignSatBias(rtk_t* rtk, gtime_t teph, gtime_t tepb, obsd_tmp_t* obs_tmp, int ns) {
    ...
    for (i = 0; i < ns; i++) {
        for (j = 0; j < NFREQ; j++) {
            ...
            sys = satsys(sat, &prn);
            switch (sys) {
            case SYS_GPS: m = 0; break;
            case SYS_GLO: m = 1; break;
            case SYS_GAL: m = 2; break;
            case SYS_BDS: m = 3; break;
            case SYS_QZS: m = 0; break;
            }
            if (rtk->base_prn[m][j] != sat) continue;
            if (rtk->base_prn[m][j] == 0) continue;
            P1 = obs_tmp[i].P[j];
            P2 = obs_tmp[i + ns].P[j];
            L1 = obs_tmp[i].L[j];
            L2 = obs_tmp[i + ns].L[j];
...
extern int invalidBDS(unsigned char sat) {
    ...
    char* invalidBDSList[17] = { "C31",\
        "C47", "C48", "C49", "C50", "C51", "C52", "C53", "C54","C55", \
        "C56", "C57", "C58",  "C61", "C62", "C63","C64" };
...
static int periodK(unsigned char sat) {
    ...
    switch ((sys = satsys(sat, &prn))) {
        case SYS_GAL: K = 17; break;
        case SYS_BDS: K = isMEO(sat) == 0 ? 1 : 13; break;
        case SYS_GPS: K = 2; break;
        default:      K = 0;     break;
    }
    return K;
}
...
extern int periodDay(unsigned char sat) {
    ...
    switch ((sys = satsys(sat, &prn))) {
        case SYS_GAL: K = 10; break;
        case SYS_BDS: K = isMEO(sat) == 0 ? 1 : 7; break;
        case SYS_GPS: K = 1; break;
        default:      K = 0;     break;
    }
    return K;
}
...
static double get_eph_period(eph_t* eph) {
    ...
    switch ((sys = satsys(eph->sat, &prn))) {
        ...
        case SYS_BDS: mu = MU_CMP; break;
        default:      mu = MU_GPS; break;
    }
    k = periodK(eph->sat);
