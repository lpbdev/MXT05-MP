...
static int outenu_dynamic(unsigned char* buff, const char* s, rtk_t* rtk, sol_t* sol,
    const double* rb, const solopt_t* opt)
{
    ...
    ecef2pos(sol->rr, pos); // 7.4053201057023443  1.5051875809804434 - 0.0026813676485921700
    ...
    if (rtk->aveEnu[0] != 0) {
        if (fabs(enu[0] - rtk->aveEnu[0]) > 0.2) {
            shiftCnt = 60 - (ROUND(fabs(enu[0] - rtk->aveEnu[0]) / 0.2)) * 10;
        }
        else if (fabs(enu[1] - rtk->aveEnu[1]) > 0.2) {
            shiftCnt = 60 - (ROUND(fabs(enu[1] - rtk->aveEnu[1]) / 0.2)) * 10;
        }
        else if (fabs(enu[2] - rtk->aveEnu[2]) > 0.2) {
            shiftCnt = 60 - (ROUND(fabs(enu[2] - rtk->aveEnu[2]) / 0.2)) * 10;
        }
        if (shiftCnt < 0) {
            shiftCnt = 10;
        }
...
#ifdef MULBASE
    if (rtk->opt.masterSlaveBaseFlag == 0 && rtk->opt.masterXyz[0] != 0 && rtk->opt.slaveXyz[0] != 0) {
        trace(4, "masterXyz:%.4f %.4f %.4f slaveXyz:%.4f %.4f %.4f masterEnu:%.4f %.4f %.4f\n",
            rtk->opt.masterXyz[0], rtk->opt.masterXyz[1], rtk->opt.masterXyz[2], rtk->opt.slaveXyz[0], rtk->opt.slaveXyz[1], rtk->opt.slaveXyz[2], rtk->masterEnu[0], rtk->masterEnu[1], rtk->masterEnu[2]);
        ...
        for (i = 0; i < 3; i++)
            rtk->slaveAveEnu[i] = (rtk->slaveAveEnu[i] * rtk->slaveAvCnt + enu2[i]) / (rtk->slaveAvCnt + 1);
        rtk->slaveAvCnt++;
    ...
    if (rtk->fixCheckCnt == 0) {
        ...
        for (i = 0; i < 3; i++)
            rtk->sol.fixxyz[i] = (sol->fixxyz[i] * sol->rr_smooth_cnt + rtk->sol.rr[i]) / (sol->rr_smooth_cnt + 1);
        sol->rr_smooth_cnt++;
    }
    if (sol->aveFixSatCnt > (int)((12 * 3600.0 - 1) / rtk->opt.timeInterval))
        sol->aveFixSatCnt = (int)((12 * 3600.0 - 1) / rtk->opt.timeInterval);
    rtk->sol.aveFixSat = (sol->aveFixSat * sol->aveFixSatCnt + rtk->sol.ns[1]) / (sol->aveFixSatCnt + 1);
    sol->aveFixSatCnt++;
    ...
    if (rtk->mvflag == 1) {
        ...
        logmsg(4, "aveEnu , %s, %10.4f, %10.4f, %10.4f\n", s, rtk->aveEnu[0], rtk->aveEnu[1], rtk->aveEnu[2]);
    }
    ...
    rtk->sol.nsFixPre = rtk->sol.ns[1];
    //if (sol->ori_var[0] > 0.0 && sol->ori_var[1] > 0.0 && sol->ori_var[2] > 0.0)
    //    trace(0x04, "pos std:%.3f %.3f %.3f\n", sqrt(sol->ori_var[0]), sqrt(sol->ori_var[1]), sqrt(sol->ori_var[2]));
    trace(0x04, "iniCnt:%d shiftCnt=%d\n", rtk->iniCnt, shiftCnt);
    if (rtk->iniCnt < rtk->maxSmoothPoint)
        rtk->iniCnt++;
    for (k = 0; k < 3; k++) {
        if (rtk->enuWindwoIndex[k] >= rtk->maxSmoothPoint) {//窗口满
            if (rtk->aveEnu[k] != 0.0 && fabs(enu[k] - rtk->aveEnu[k]) > detectSensitivity * rtk->stdEnu[k]
            ...
            if (sol->thresCnt1[k] + sol->thresCnt2[k] >= shiftCnt) {
                rtk->sol.enu_shift[k] = sol->enu_sum[k] / (sol->thresCnt1[k] + sol->thresCnt2[k]) - rtk->aveEnu[k];
                if (fabs(rtk->sol.enu_shift[k]) > 0.05 && rtk->opt.timeInterval >= 15) precent = 0.5;
                if ((sol->thresCnt1[k] > shiftCnt * precent && rtk->opt.detectSensitivity != 10) || (fabs(enu[k] - rtk->aveEnu[k]) > 0.2 && fabs(rtk->aveEnu[k]) != 0.0)) {
                    //rtk->sol.enu_shift[k] = sol->enu_sum[k] / (sol->thresCnt1[k] + sol->thresCnt2[k]) - rtk->aveEnu[k];
                    ...
                    if (k == 0 || k == 1) {
                        if (fabs(rtk->sol.enu_shift[k]) > 0.008) {
                            ...
                            for (i = 0; i < rtk->maxSmoothPoint; i++)
                            {
                                ...
                                rtk->sum_sqeun[k] = rtk->sum_sqeun[k] + rtk->enuWindow[k][i] * rtk->enuWindow[k][i];
                            }
                        }
                        else {
            ...
            rtk->sum_sqeun[k] = rtk->sum_sqeun[k] - SQR(rtk->enuWindow[k][rtk->delpoint[k]]) + enu[k] * enu[k];

            rtk->aveEnu[k] = rtk->sum_enu[k] / rtk->maxSmoothPoint;

            var = (rtk->sum_sqeun[k] - SQR(rtk->sum_enu[k]) / rtk->maxSmoothPoint) / (rtk->maxSmoothPoint - 1);
            if (var < 0)
                rtk->stdEnu[k] = 1E-7;
            else
                rtk->stdEnu[k] = sqrt((rtk->sum_sqeun[k] - SQR(rtk->sum_enu[k]) / rtk->maxSmoothPoint) / (rtk->maxSmoothPoint - 1));
            ...
            if (rtk->aveEnu[k] != 0.0 && fabs(enu[k] - rtk->aveEnu[k]) > detectSensitivity * rtk->stdEnu[k]
                && rtk->iniCnt == rtk->maxSmoothPoint || (fabs(enu[k] - rtk->aveEnu[k]) > 1 && fabs(rtk->aveEnu[k]) != 0.0)) {
                sol->thresCnt1[k]++;
                trace(0x04, "warnning a exceptional potin:%d\n", k);
                if (sol->thresCnt1[k] < shiftCnt / 2.0) {
                    trace(0x04, "changed enu:%d\n", k);
            ...
            if (rtk->enuWindwoIndex[k] == 1)
                ...
                if (var < 0)
                    ...
                    rtk->stdEnu[k] = sqrt((rtk->sum_sqeun[k] - SQR(rtk->sum_enu[k]) / rtk->enuWindwoIndex[k]) / (rtk->enuWindwoIndex[k] - 1));
            }
        }
    }
    ...
    for (i = 0; i < 3; i++) sol->rr_original[i] = r[i];
    ...
    for (i = 0; i < 3; i++) sol->rr[i] = r[i];
    for (i = 0; i < 3; i++) sol->rr_filer[i] = r[i];
    ...
    if (rtk->opt.masterSlaveBaseFlag == 0 && rtk->opt.masterXyz[0] != 0 && rtk->opt.slaveXyz[0] != 0 && rtk->dr[0] == 0.0) {
        rtk->dr[0] = rtk->masterRr[0] - sol->rr_filer[0];
        rtk->dr[1] = rtk->masterRr[1] - sol->rr_filer[1];
        rtk->dr[2] = rtk->masterRr[2] - sol->rr_filer[2];
    }
#endif
    //printf("rr_original:%14.4lf %14.4lf %14.4lf\n", sol->rr_original[0], sol->rr_original[1], sol->rr_original[2]);
    //printf("rr_filer:%14.4lf %14.4lf %14.4lf\n", sol->rr_filer[0], sol->rr_filer[1], sol->rr_filer[2]);

    trace(4, "%14.4lf %14.4lf %14.4lf %u %u %u\n", rtk->stdEnu[0], rtk->stdEnu[1], rtk->stdEnu[2], rtk->enuWindwoIndex[0], rtk->enuWindwoIndex[1], rtk->enuWindwoIndex[2]);
    ...
    trace(0x02, "enu1:%s%s%14.4f%s%14.4f%s%14.4f%s%3d%s%3d%s%8.4f%s%8.4f%s%8.4f%s%8.4f%s%8.4f%s%8.4f%s%6.2f%s%6.1f\n",
        s, sep, sol->enu_original[0], sep, sol->enu_original[1], sep, sol->enu_original[2], sep, sol->stat, sep, ns, sep,
        SQRT(Q[0]), sep, SQRT(Q[4]), sep, SQRT(Q[8]), sep, sqvar(Q[1]),
        sep, sqvar(Q[5]), sep, sqvar(Q[2]), sep, sol->age, sep, sol->ratio);

    trace(0x02, "enu2:%s%s%14.4f%s%14.4f%s%14.4f%s%3d%s%3d%s%8.4f%s%8.4f%s%8.4f%s%8.4f%s%8.4f%s%8.4f%s%6.2f%s%6.1f\n",
        s, sep, enu2[0], sep, enu2[1], sep, enu2[2], sep, sol->stat, sep, ns, sep,
        SQRT(Q[0]), sep, SQRT(Q[4]), sep, SQRT(Q[8]), sep, sqvar(Q[1]),
        sep, sqvar(Q[5]), sep, sqvar(Q[2]), sep, sol->age, sep, sol->ratio);


    p += sprintf(p, "%s%s%14.4f%s%14.4f%s%14.4f%s%3d%s%3d%s%8.4f%s%8.4f%s%8.4f%s%8.4f%s%8.4f%s%8.4f%s%6.2f%s%6.1f%s%.4f%s%.4f%s%.4f%s%.0f%s%.0f%s%.0f\n",
        s, sep, enu2[0], sep, enu2[1], sep, enu2[2], sep, sol->stat, sep, ns, sep,
        SQRT(Q[0]), sep, SQRT(Q[4]), sep, SQRT(Q[8]), sep, sqvar(Q[1]),
        sep, sqvar(Q[5]), sep, sqvar(Q[2]), sep, sol->age, sep, sol->ratio, sep, rtk->fftFrq[0], sep, rtk->fftFrq[1],
        sep, rtk->fftFrq[2], sep, rtk->fftPower[0], sep, rtk->fftPower[1], sep, rtk->fftPower[2]);

    printf("%s%s%14.4f%s%14.4f%s%14.4f%s%3d%s%3d%s%8.4f%s%8.4f%s%8.4f%s%8.4f%s%8.4f%s%8.4f%s%6.2f%s%6.1f\n",
        s, sep, sol->enu_original[0], sep, sol->enu_original[1], sep, sol->enu_original[2], sep, sol->stat, sep, ns, sep,
        SQRT(Q[0]), sep, SQRT(Q[4]), sep, SQRT(Q[8]), sep, sqvar(Q[1]),
        sep, sqvar(Q[5]), sep, sqvar(Q[2]), sep, sol->age, sep, sol->ratio);
    if (oFile.fpOut[0])
...
static int outenu2(unsigned char* buff, const char* s, rtk_t* rtk, sol_t* sol,
    const double* rb, const solopt_t* opt)
{
...
#endif
    for (i = 0; i < 3; i++) {
        if (enuInit[i] == 0.0) continue;
        if (i == 2) {
            if (fabs(enu2[i] - enuInit[i]) > 0.5 && unusualEnuCnt < thresCnt) {
                unusualEnuCnt++;
                trace(0x04, "warnning enu:%.2f %.2f %.2f enuIni:%.2f %.2f %.2f\n", enu2[i], enu2[i], enu2[i], enuInit[i], enuInit[i], enuInit[i]);
                return 0;
            }
        }
        else {
    ...
    /*
        ...
        if (index != -1) {
            ...
            for (i = 0; i < 3; i++) {
                enu2[i] = 0.5 * (weight1 * enu2[i] + (1 - weight1) * rtk->satMapEnu[index][i]) +
                    0.5 * (weight2 * enu2[i] + (1 - weight2) * rtk->satMapEnu[index][i]);
            }
            trace(0x04, "nfix=%d ns=%d weight1=%.2f weight2=%.2f enu=%.2f %.2f %.2f satMapEnu=%.2f %.2f %.2f\n",
                rtk->nfix, index + 1, weight1, weight2, rtk->satMapEnu[index][0], rtk->satMapEnu[index][1], rtk->satMapEnu[index][2], enu2[0], enu2[1], enu2[2]);
        }
    ...
    for (k = 0; k < 3; k++) {
        if (enuInit[0] != 0.0 || enuInit[1] != 0.0 || enuInit[2] != 0.0) {
            ...
            if (FFTMagMax > (FFTMagMean * FFTDectTh))
            {
                ...
                trace(0x10, "k=%d FFT %d, FFTMagMean %d, FFTMagMax %d, FFTMagMaxIdx %d, FFTDectFreq %f\n", k, count, FFTMagMean, FFTMagMax, FFTMagMaxIdx, FFTDectFreq);
                rtk->fftFrq[k] = FFTDectFreq;
                rtk->fftPower[k] = FFTMagMax;
                //printf("k=%d FFT %d, FFTMagMean %d, FFTMagMax %d, FFTMagMaxIdx %d, FFTDectFreq %f\n", k, count, FFTMagMean, FFTMagMax, FFTMagMaxIdx, FFTDectFreq);
            }
            else
            {
    ...
    const double* rb, const solopt_t* opt)
{
    ...
    if (rtk->nfix < nfix || sol->stat != SOLQ_FIX || rtk->sol.ns[1] < 10) {
        trace(0x04, "warnning:stat=%d nfix=%d ns[0]=%d ns[1]=%d sumPostCarV=%.2f\n", sol->stat, rtk->nfix, rtk->sol.ns[0], rtk->sol.ns[1], rtk->sumPostCarV);
        trace(0x04, "warnning:%s%s%14.4f%s%14.4f%s%14.4f%s%3d%s%3d%s%8.4f%s%8.4f%s%8.4f%s%8.4f%s%8.4f%s%8.4f%s%6.2f%s%6.1f\n",
            s, sep, enu2[0], sep, enu2[1], sep, enu2[2], sep, sol->stat, sep, ns, sep,
            SQRT(Q[0]), sep, SQRT(Q[4]), sep, SQRT(Q[8]), sep, sqvar(Q[1]),
            sep, sqvar(Q[5]), sep, sqvar(Q[2]), sep, sol->age, sep, sol->ratio);
        return 0;
...
static int outnmea_gga(unsigned char* buff, const sol_t* sol)
{
    ...
    if (time.frac >= 0.995) { time.time++; time.frac = 0.0; }
    time2epoch(time, ep);
    ecef2pos(sol->rr, pos);
    h = geoidh(pos);
    ...
    p += sprintf(p, "$GNGGA,%02.0f%02.0f%05.2f,%02.0f%010.7f,%s,%03.0f%010.7f,%s,%d,%02d,%.1f,%.3f,M,%.3f,M,%.1f,",
        ...
        sol->ns[1], dop, pos[2] - h, h, sol->age);
    printf("$GNGGA,%02.0f%02.0f%05.2f,%02.0f%010.7f,%s,%03.0f%010.7f,%s,%d,%02d,%.1f,%.3f,M,%.3f,M,%.1f,",
        ep[3], ep[4], ep[5], dms1[0], dms1[1] + dms1[2] / 60.0, pos[0] >= 0 ? "N" : "S",
        dms2[0], dms2[1] + dms2[2] / 60.0, pos[1] >= 0 ? "E" : "W", solq,
        sol->ns[1], dop, pos[2] - h, h, sol->age);
    ...
    const solopt_t* opt)
{
    ...
    p += sprintf(p, "%s%10.4f%s%3d%s%3d%s%8.4f%s%8.4f%s%8.4f%s%8.4f%s%8.4f%s%8.4f%s%6.2f%s%6.1f",
        sep, pos[2], sep, sol->stat, sep, ns, sep, SQRT(Q[4]), sep,
        SQRT(Q[0]), sep, SQRT(Q[8]), sep, sqvar(Q[1]), sep, sqvar(Q[2]),
        sep, sqvar(Q[5]), sep, sol->age, sep, sol->ratio);

    printf("%s%10.4f%s%3d%s%3d%s%8.4f%s%8.4f%s%8.4f%s%8.4f%s%8.4f%s%8.4f%s%6.2f%s%6.1f\n",
        sep, pos[2], sep, sol->stat, sep, ns, sep, SQRT(Q[4]), sep,
        SQRT(Q[0]), sep, SQRT(Q[8]), sep, sqvar(Q[1]), sep, sqvar(Q[2]),
        sep, sqvar(Q[5]), sep, sol->age, sep, sol->ratio);
...
static void outSpp(FILE* fp, int rcv, rtk_t* rtk, gtime_t time)
{
    ...
    p += sprintf(p, "%04d%s%02d%s%02d%s%02d%s%02d%s%02d%s%4d%s%9.2f%s%14.4f%s%14.4f%s%14.4f",
        ...
        sep, sow, sep, rr[0], sep, rr[1], sep, rr[2]);
    p += sprintf(p, "\n");
    n = p - (char*)buff;
    if (n > 0) {
        fwrite(buff, n, 1, fp);
...
static void outPdop(FILE* fp, rtk_t* rtk, gtime_t time, int index)
{
    ...
    p += sprintf(p, "%04d%s%02d%s%02d%s%02d%s%02d%s%02d%s%4d%s%9.2f%s%3d%s%2d%s%10.3f%s%10.3f%s%10.3f%s%10.3f",
        (int)ep[0], sep, (int)ep[1], sep, (int)ep[2], sep, (int)ep[3], sep, (int)ep[4], sep, (int)ep[5], sep, week,
        sep, sow, sep, nsat, sep, ns, sep, rtk->sol.dop[index][0], sep, rtk->sol.dop[index][1], sep, rtk->sol.dop[index][2], sep, rtk->sol.dop[index][3]);//G P H V
...
static void outResi(FILE* fp, rtk_t* rtk, gtime_t time, int frq, int itype)
{
    ...
    p += sprintf(p, "%04d%s%02d%s%02d%s%02d%s%02d%s%02d%s%9.2f%s", (int)ep[0], sep, (int)ep[1], sep,
    ...
    for (i = i0; i < i1; i++) {
        ...
        if (frq == 1) {
            ...
            if (itype == 0) p += sprintf(p, "%10.4f%s", resc_pos[3], sep);
            if (itype == 1) p += sprintf(p, "%10.4f%s", resp_pos[3], sep);
        }
        else if (frq == 5) {
            if (itype == 0) p += sprintf(p, "%10.4f%s", resc_pos[4], sep);
            if (itype == 1) p += sprintf(p, "%10.4f%s", resp_pos[4], sep);
        }
        else if (frq == 6) {
            if (itype == 0) p += sprintf(p, "%10.4f%s", resc_pos[5], sep);
            if (itype == 1) p += sprintf(p, "%10.4f%s", resp_pos[5], sep);
        }
...
static void outElev(FILE* fp, rtk_t* rtk, gtime_t time)
{
    ...
    p += sprintf(p, "%04d%s%02d%s%02d%s%02d%s%02d%s%02d%s%9.2f%s", (int)ep[0], sep, (int)ep[1],
        sep, (int)ep[2], sep, (int)ep[3], sep, (int)ep[4], sep, (int)ep[5], sep, sow, sep);
    ...
    const solopt_t* opt)
{
    ...
    switch (opt->posf) {
    case SOLF_LLH:  p += outpos(p, s1, sol, opt);   break;
        //case SOLF_XYZ:  p += outecef(p, s1,rtk, sol, opt);   break;
    case SOLF_ENU: {
        if (rtk->opt.mode == 3) {
            p += outenuSstatic(p, s1, rtk, sol, rb, opt); break;
        }
        else {
            if (rtk->opt.senceopt == 1) {
                p += outenu2(p, s1, rtk, sol, rb, opt); break;
            }
            else {
                p += outenu_dynamic(p, s1, rtk, sol, rb, opt); break;
            }
        }
    }
