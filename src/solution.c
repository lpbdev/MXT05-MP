...
static void medianFilter(double *enu, rtk_t *rtk) {
  int i = 0, j = 0, k = 0, index, mid = 0, segment, symboldenu[3], leveldenu[3],
      pickPoint = 3;
  int maxpoint = rtk->maxMedianFilterPoint;
  double *enuwindow[3], tmp, denu[3], sumENU[3], sumSQENU[3], stdENU[3],
      aveENU[3];
  ...
  if (index > (maxpoint - 1)) {
    ...
    stdENU[0] =
        sqrt((sumSQENU[0] - sumENU[0] * sumENU[0] / maxpoint) / (maxpoint - 1));
    stdENU[1] =
        sqrt((sumSQENU[1] - sumENU[1] * sumENU[1] / maxpoint) / (maxpoint - 1));
    stdENU[2] =
        sqrt((sumSQENU[2] - sumENU[2] * sumENU[2] / maxpoint) / (maxpoint - 1));
    ...
    for (i = 0; i < 3; i++) {
      ...
      if (fabs(denu[i]) > rtk->stdEnu[i] &&
          (rtk->enuWindwoIndex[i] < rtk->maxSmoothPoint ||
           rtk->enuWindowMedianShiftNum[i] == 0))
...
static double geoidh_gsi(const double *pos) {
  ...
  if (!fp_geoid || pos[1] < lon0 || lon1 < pos[1] || pos[0] < lat0 ||
      lat1 < pos[0]) {
...
static double geoidh(const double *pos) {
  ...
  if (posd[1] < 0.0 || 360.0 - 1E-12 < posd[1] || posd[0] < -90.0 ||
      90.0 < posd[0]) {
...
static int outenu_dynamic(unsigned char *buff, const char *s, rtk_t *rtk,
                          sol_t *sol, const double *rb, const solopt_t *opt) {
  ...
  trace(4, "rr:%14.4lf %14.4lf %14.4lf %14.4lf %14.4lf %14.4lf\n", sol->rr[0],
        sol->rr[1], sol->rr[2], enu2[0], enu2[1], enu2[2]);
  ...
  if (sol->stat != SOLQ_FIX || rtk->sol.ns[1] < rtk->opt.minFixSat) {
    trace(0x4, "warnning:stat=%d nfix=%d ns[0]=%d ns[1]=%d sumPostCarV=%.2f\n",
          sol->stat, rtk->nfix, rtk->sol.ns[0], rtk->sol.ns[1],
          rtk->sumPostCarV);
    trace(0x4,
          "warnning:%s%s%14.4f%s%14.4f%s%14.4f%s%3d%s%3d%s%8.4f%s%8.4f%s%8.4f%"
          "s%8.4f%s%8.4f%s%8.4f%s%6.2f%s%6.1f\n",
          s, sep, enu2[0], sep, enu2[1], sep, enu2[2], sep, sol->stat, sep, ns,
          sep, SQRT(Q[0]), sep, SQRT(Q[4]), sep, SQRT(Q[8]), sep, sqvar(Q[1]),
  ...
  if (rtk->opt.masterSlaveBaseFlag == 0 && rtk->opt.masterXyz[0] != 0 &&
    ...
    trace(4,
          ...
          rtk->opt.slaveXyz[0], rtk->opt.slaveXyz[1], rtk->opt.slaveXyz[2],
          rtk->masterEnu[0], rtk->masterEnu[1], rtk->masterEnu[2]);
    trace(4, "use slave base,enu:%.4f %.4f %.4f\n", enu2[0], enu2[1], enu2[2]);
    if (rtk->masterEnu[0] == 0.0 && rtk->masterEnu[1] == 0.0 &&
        rtk->masterEnu[2] == 0.0) {
    ...
    for (i = 0; i < 3; i++)
      rtk->slaveAveEnu[i] = (rtk->slaveAveEnu[i] * rtk->slaveAvCnt + enu2[i]) /
                            (rtk->slaveAvCnt + 1);
    rtk->slaveAvCnt++;
  ...
  if (rtk->fixCheckCnt == 0) {
    ...
    for (i = 0; i < 3; i++)
      rtk->sol.fixxyz[i] =
          (sol->fixxyz[i] * sol->rr_smooth_cnt + rtk->sol.rr[i]) /
          (sol->rr_smooth_cnt + 1);
    sol->rr_smooth_cnt++;
  }
  ...
  for (i = 0; i < 3; i++) {
    if (sol->window[0].jumpflag[i] == 1) {
      ...
      sol->window[2].ave[i] =
          sol->window[2].ave[i] + sol->jump[i]; // sol->window[0].ave[i];
  ...
  if (rtk->mvflag == 1) {
    ...
    //        sol->window[2].ave[0], sol->window[2].ave[1],
    //        sol->window[2].ave[2], sol->window[2].std[0],
  ...
  trace(4, "stdEnu          :%10.4f %10.4f %10.4f\n", rtk->stdEnu[0],
        rtk->stdEnu[1], rtk->stdEnu[2]);
  cnt = 0;

  if (rtk->opt.detectSensitivity == 10) {
    if (rtk->aveEnu[0] != 0.0 &&
        rtk->enuWindwoIndex[0] > 3600.0 * 1 / rtk->opt.timeInterval) {
      detectSensitivity = 4;
    }
  }
  rtk->sol.nsFixPre = rtk->sol.ns[1];
  ...
  for (k = 0; k < 3; k++) {
    if (rtk->enuWindwoIndex[k] >= rtk->maxSmoothPoint) { // 窗口满
      ...
      if (sol->thresCnt1[k] + sol->thresCnt2[k] >= shiftCnt) {
        rtk->sol.enu_shift[k] =
            sol->enu_sum[k] / (sol->thresCnt1[k] + sol->thresCnt2[k]) -
            rtk->aveEnu[k];
        if (fabs(rtk->sol.enu_shift[k]) > 0.05 && rtk->opt.timeInterval >= 15)
          precent = 0.5;
        if ((sol->thresCnt1[k] > shiftCnt * precent &&
             rtk->opt.detectSensitivity != 10) ||
            (fabs(enu[k] - rtk->aveEnu[k]) > 0.5 &&
             fabs(rtk->aveEnu[k]) != 0.0 && rtk->opt.detectSensitivity != 10)) {
          ...
          if (k == 0 || k == 1) {
            if (fabs(rtk->sol.enu_shift[k]) > 0.008) {
              ...
              for (i = 0; i < rtk->maxSmoothPoint; i++) {
                ...
                rtk->sum_sqeun[k] = rtk->sum_sqeun[k] +
                                    rtk->enuWindow[k][i] * rtk->enuWindow[k][i];
              }
            } else {
      ...
      rtk->sum_enu[k] =
          rtk->sum_enu[k] - rtk->enuWindow[k][rtk->delpoint[k]] + enu[k];
      rtk->sum_sqeun[k] = rtk->sum_sqeun[k] -
                          SQR(rtk->enuWindow[k][rtk->delpoint[k]]) +
                          enu[k] * enu[k];
      ...
      if (rtk->aveEnu[k] != 0.0 &&
              fabs(enu[k] - rtk->aveEnu[k]) >
                  detectSensitivity * rtk->stdEnu[k] &&
              rtk->iniCnt == rtk->maxSmoothPoint ||
      ...
      if (rtk->enuWindwoIndex[k] == 1)
        ...
        if (var < 0)
          ...
          rtk->stdEnu[k] =
              sqrt((rtk->sum_sqeun[k] -
                    SQR(rtk->sum_enu[k]) / rtk->enuWindwoIndex[k]) /
  ...
  trace(4, "%14.4lf %14.4lf %14.4lf %u %u %u\n", rtk->stdEnu[0], rtk->stdEnu[1],
        rtk->stdEnu[2], rtk->enuWindwoIndex[0], rtk->enuWindwoIndex[1],
        rtk->enuWindwoIndex[2]);
  trace(4, "rtk->tt=%.2f maxSmoothPoint=%d\n", rtk->tt, rtk->maxSmoothPoint);
  trace(4, "cnt1:  %02d  %02d  %02d\n", sol->thresCnt1[0], sol->thresCnt1[1],
        sol->thresCnt1[2]);
  trace(4, "cnt2:  %02d  %02d  %02d\n", sol->thresCnt2[0], sol->thresCnt2[1],
        sol->thresCnt2[2]);
  ...
  p += sprintf(
      ...
      SQRT(Q[0]), sep, SQRT(Q[4]), sep, SQRT(Q[8]), sep, sqvar(Q[1]), sep,
      sqvar(Q[5]), sep, sqvar(Q[2]), sep, sol->age, sep, sol->ratio, sep,
      rtk->fftFrq[0], sep, rtk->fftFrq[1], sep, rtk->fftFrq[2], sep,
      rtk->fftPower[0], sep, rtk->fftPower[1], sep, rtk->fftPower[2]);

  printf("%s%s%14.4f%s%14.4f%s%14.4f%s%3d%s%3d%s%8.4f%s%8.4f%s%8.4f%s%8.4f%s%8."
  ...
  if (oFile.fpOut[0])
    fprintf(oFile.fpOut[0],
            ...
            sep, sqvar(Q[2]), sep, sol->age, sep, sol->ratio, sol->dop[0][0],
...
static int outenu2(unsigned char *buff, const char *s, rtk_t *rtk, sol_t *sol,
  ...
  */
  ...
  for (i = 0; i < 3; i++) {
    sol->ori_ave[i] = (sol->ori_ave[i] * rtk->enuWindwoIndex[i] + enu2[i]) /
                      (rtk->enuWindwoIndex[i] + 1);
...
static int outenuSstatic(unsigned char *buff, const char *s, rtk_t *rtk,
                         sol_t *sol, const double *rb, const solopt_t *opt) {
...
static int outnmea_gga(unsigned char *buff, const sol_t *sol) {
  ...
  p += sprintf(p,
               ...
               pos[1] >= 0 ? "E" : "W", solq, sol->ns[1], dop, pos[2] - h, h,
               sol->age);
  printf("$GNGGA,%02.0f%02.0f%05.2f,%02.0f%010.7f,%s,%03.0f%010.7f,%s,%d,%02d,%"
         ".1f,%.3f,M,%.3f,M,%.1f,",
         ep[3], ep[4], ep[5], dms1[0], dms1[1] + dms1[2] / 60.0,
         pos[0] >= 0 ? "N" : "S", dms2[0], dms2[1] + dms2[2] / 60.0,
         pos[1] >= 0 ? "E" : "W", solq, sol->ns[1], dop, pos[2] - h, h,
...
extern int outpos(unsigned char *buff, const char *s, const sol_t *sol,
  ...
  if (opt->degf) {
    ...
    p += sprintf(p, "%s%s%14.9f%s%14.9f", s, sep, pos[0] * R2D, sep,
                 pos[1] * R2D);
  ...
  printf("%s%10.4f%s%3d%s%3d%s%8.4f%s%8.4f%s%8.4f%s%8.4f%s%8.4f%s%8.4f%s%6.2f%"
         "s%6.1f\n",
         sep, pos[2], sep, sol->stat, sep, ns, sep, SQRT(Q[4]), sep, SQRT(Q[0]),
         sep, SQRT(Q[8]), sep, sqvar(Q[1]), sep, sqvar(Q[2]), sep, sqvar(Q[5]),
  ...
  if (opt->outvel) { /* output velocity */
    ...
    covenu(pos, P, Q);
    p += sprintf(
        p, "%s%10.5f%s%10.5f%s%10.5f%s%9.5f%s%8.5f%s%8.5f%s%8.5f%s%8.5f%s%8.5f",
        sep, vel[1], sep, vel[0], sep, vel[2], sep, SQRT(Q[4]), sep, SQRT(Q[0]),
        sep, SQRT(Q[8]), sep, sqvar(Q[1]), sep, sqvar(Q[2]), sep, sqvar(Q[5]));
  }
  p += sprintf(p, "\n");
  return p - (char *)buff;
...
static void outSpp(FILE *fp, int rcv, rtk_t *rtk, gtime_t time) {
  ...
  p += sprintf(
      p,
      "%04d%s%02d%s%02d%s%02d%s%02d%s%02d%s%4d%s%9.2f%s%14.4f%s%14.4f%s%14.4f",
      (int)ep[0], sep, (int)ep[1], sep, (int)ep[2], sep, (int)ep[3], sep,
      (int)ep[4], sep, (int)ep[5], sep, week, sep, sow, sep, rr[0], sep, rr[1],
      sep, rr[2]);
  p += sprintf(p, "\n");
  n = p - (char *)buff;
  if (n > 0) {
...
static void outPdop(FILE *fp, rtk_t *rtk, gtime_t time, int index) {
  ...
  p += sprintf(p,
               ...
               (int)ep[0], sep, (int)ep[1], sep, (int)ep[2], sep, (int)ep[3],
               sep, (int)ep[4], sep, (int)ep[5], sep, week, sep, sow, sep, nsat,
               sep, ns, sep, rtk->sol.dop[index][0], sep,
               rtk->sol.dop[index][1], sep, rtk->sol.dop[index][2], sep,
               rtk->sol.dop[index][3]); // G P H V
...
static void outResi(FILE *fp, rtk_t *rtk, gtime_t time, int frq, int itype) {
  ...
  p += sprintf(p, "%04d%s%02d%s%02d%s%02d%s%02d%s%02d%s%9.2f%s", (int)ep[0],
               sep, (int)ep[1], sep, (int)ep[2], sep, (int)ep[3], sep,
               (int)ep[4], sep, (int)ep[5], sep, sow, sep);
...
static void Record_gf(FILE *fp, rtk_t *rtk, gtime_t time) {
  ...
  p += sprintf(p, "%04d%s%02d%s%02d%s%02d%s%02d%s%02d%s%4d%s%9.2f%s",
               (int)ep[0], sep, (int)ep[1], sep, (int)ep[2], sep, (int)ep[3],
               sep, (int)ep[4], sep, (int)ep[5], sep, week, sep, sow, sep);
...
extern int outsols(unsigned char *buff, rtk_t *rtk, sol_t *sol,
  ...
  if (opt->timef) {
    ...
    sprintf(s2, "%4d%s%*.*f", week, sep, 6 + (timeu <= 0 ? 0 : timeu + 1),
            timeu, gpst);
  }
