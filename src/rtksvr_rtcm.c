...
//
//	cfgfile = argv[i++];
//	cfgOpt->useRtcmPosFlag = atoi(cfgfile);
//	printf("useRtcmPosFlag=%d\n", cfgOpt->useRtcmPosFlag);
//
//}
//
...
//	int nfile = 0;
//	sprintf(getfilecmd, "ls rtkLogPost -t |grep %s.*.tar.gz", &debugFile[9]);
//	printf("getfilecmd=%s\n", getfilecmd);
...
//				sprintf(dfcmd, "rm -rf rtkLogPost/%s", line);
//				system(dfcmd);
//				sleepms(5);
//			}
//		}
...
//
//#ifndef WIN32	
//		if (oFile.fpDebug && (svr->rtcm[0].time.time % 5 == 0)) {
...
//			time2str(time, s1, 3);
//
//			if (svr->rtk.sol.stat == 1)
//				fixCnt++;
//			
//			if (ouInterval != 0 && svr->rtk.opt.typeSol==1) {
//				if (time.time % ouInterval == 0 && fixCnt>10) {
...
//				pbuff = buff;
//				outDnyResult(svr, &pbuff, s1, iniEnuFlag);
//				if (svr->rtk.opt.typeSol == 0 && svr->rtk.opt.timeIntervalSolution == 0 && svr->rtk.sol.stat == 1) {
//					strwrite(&svr->stream[2], (uint8_t*)buff, strlen(buff));
//					fprintf(oFile.fpDebug, "%s\n", debugBuff);
//				}
//#ifdef WIN32
...
//	char* filename = NULL;
//	double ep[6] = { 0 };
//	if (argc == 18) //Without Configuration Parameters
//	{
//		gpdebugBuff = debugBuff;
//		for (i = 1; i < 4; i++)
//		{
...
//			else
//				strtype[i - 1] = STR_FILE;
//		}
//
//		filename=	strrchr(strpath[0], '//');
//		if(filename==NULL)
//			filename = strrchr(strpath[0], '\\');
//		if (filename == NULL)
//			filename = strrchr(strpath[0], '/ ');
...
//		}
//	
//		ep[0] = str2num(filename, 0, 4);
//		ep[1] = str2num(filename, 4, 2);
//		ep[2] = str2num(filename, 6, 2);
//		time2gpst(epoch2time(ep), &g_week);
//
//		cfgfile = argv[i++];
...
//	}
//
//	//char infile[6][MAXSTRPATH], fileDir[MAXSTRPATH] = "E:\\data\\E40225\\W\\1607";
//	
//	m = 0; n = 0;
//	
//	gpdebugBuff = debugBuff;
...
//
//#ifdef WIN32
//	if (access("./rtkLogPost", 0) != 0)
//		mkdir("./rtkLogPost");
//#else
//	if (access("./rtkLogPost", 0) != 0)
//		mkdir("./rtkLogPost", S_IRWXU);
//#endif	
//	strcpy(debugFile, "./rtkLogPost/");
//
...
//	while (1) {
//		ret = 0;
//		if ((n = strread(&svr.stream[2], buff2, DEBUG_BUFF_LEN)) > 0)
//		{
//			if (n != 0) {
//				ret=decodeConfig(&svr, &g_cfgOpt, buff2);
//#ifndef WIN32
//				del_dbglog();
//#endif // !WIN32
//			}
